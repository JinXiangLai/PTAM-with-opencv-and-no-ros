...
System::System(int camera_index)
    : mVideoSource(ImageDataSet(tumDataDir, tumRgbFile)),
      mGLWindow(mVideoSource.getSize(), "PTAM") {
    ...
    cv::Vec<float, NUMTRACKERCAMPARAMETERS> vTest =
    ...
    if (vTest == ATANCamera::mvDefaultParams) {
        cout << endl;
        cout << "! Camera.Parameters is not set, need to run the "
                "CameraCalibrator tool"
             << endl;
    ...
    cout << "Got passed the preliminary camera test... Now need to create: "
         << endl;
    cout << "a. The Map!" << endl;
    mpMap = new Map;
    cout << "DONE" << endl;
    ...
    mpTracker =
        new Tracker(mVideoSource.getSize(), *mpCamera, *mpMap, *mpMapMaker);
    cout << "DONE" << endl;
...
void System::Run() {
    while (!mbDone) {
        ...
        // Get a new frame
        mVideoSource.GetAndFillFrameBWandRGB(mimFrameRGB, mimFrameBW);
