...
void Map::deleteBadPoints() {
    ...
    for (unsigned int i = 0; i < vpPoints.size(); i++) {
        if (vpPoints.end() - (vpPoints.begin() + i) <= 0)
            continue;  // see if somehow we got
        // out of the vector's range (veeery unlikely)...
        MapPoint::Ptr pMP = vpPoints[i];

        if (pMP.use_count() ==
            0) {  // checking if the mappoint has been dereferenced (veery unlikely)
...
void Map::EmptyTrash() {
    ...
    vpPointsTrash.clear();
    cout << "DEBUG: Ditched " << vpPointsTrash.size()
         << " trashed map points! I hope..." << endl;
}
