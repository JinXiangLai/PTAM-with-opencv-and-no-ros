...
GLWindow2::GLWindow2(cv::Size2i irSize, string sTitle)
    ...
    if (sCommand == "GLWindow.AddMenu") {
        switch (vs.size()) {
            ...
            case 2:
                ...
                return;
            default:
                cout << "? AddMenu: need one or two params (internal menu "
                        "name, [caption])."
                     << endl;
                return;
        };
...
void GLWindow2::SetupVideoOrtho() {
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(-0.5, (double)mirVideoSize.width - 0.5,
            (double)mirVideoSize.height - 0.5, -0.5, -1.0, 1.0);
}
...
void GLWindow2::on_mouse_down(GLWindow& win, cv::Point2i where, int state,
                              int button) {
    bool bHandled = false;
...
pair<cv::Vec<float, 6>, cv::Vec<float, 6> > GLWindow2::GetMousePoseUpdate() {
    pair<cv::Vec<float, 6>, cv::Vec<float, 6> > result =
