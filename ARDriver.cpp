...
ARDriver::ARDriver(const ATANCamera& cam, GLWindow2& glw)
...
void ARDriver::Init() {
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA, mirFrameSize.width,
                 mirFrameSize.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    MakeFrameBuffer();
    mGame.Init();
}
...
void ARDriver::Render(cv::Mat& imFrame, SE3<> se3CfromW) {
    ...
    GLXInterface::glMultMatrix4x4(
        mCamera.MakeUFBLinearFrustumMatrix(0.005, 100));
...
void ARDriver::MakeFrameBuffer() {
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA, mirFBSize.width,
                 mirFBSize.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    ...
    glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT24,
                             mirFBSize.width, mirFBSize.height);
...
void ARDriver::DrawFBBackGround() {
    ...
    if (bFirstRun) {
        ...
        int nStepsY =
            (int)(nStepsX * ((double)mirFrameSize.width /
                             mirFrameSize.height));  // Scaled by aspect ratio
        ...
        for (int ystep = 0; ystep < nStepsY; ystep++) {
            ...
            for (int xstep = 0; xstep <= nStepsX; xstep++)
                     ...
                     yystep++)  // Two y-coords in one go - magic.
                {
                    ...
                    cv::Vec2f v2UFBUnDistorted = mCamera.UFBLinearProject(
                        mCamera.UFBUnProject(v2UFBDistorted));
                    glTexCoord2d(v2UFBDistorted[0] * mirFrameSize.width,
                                 v2UFBDistorted[1] * mirFrameSize.height);
            ...
            for (int xstep = 0; xstep <= nStepsX; xstep++)
                     ...
                     yystep++)  // Two y-coords in one go - magic.
                {
                    ...
                    cv::Vec2f v2UFBUnDistorted = mCamera.UFBLinearProject(
                        mCamera.UFBUnProject(v2UFBDistorted));
                    glTexCoord2d(
                        v2UFBUnDistorted[0] * mirFBSize.width,
                        (1.0 - v2UFBUnDistorted[1]) * mirFBSize.height);
...
void ARDriver::DrawFadingGrid() {
    ...
    for (int i = 0; i < nTot; i++)
        for (int j = 0; j < nTot; j++) {
            cv::Vec3f v3((i - nHalfCells) * 0.1, (j - nHalfCells) * 0.1, 0.0);
            aaVertex[i][j] = v3;
        }
