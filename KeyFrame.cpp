...
void KeyFrame::MakeKeyFrame_Lite(cv::Mat_<uchar>& im) {
    ...
    for (int i = 0; i < LEVELS; i++) {
        ...
        if (i != 0)
            CvUtils::halfSample(aLevels[i - 1].im,
                                lev.im);  // ALWAYS do this a la Rosten!!!!
                                          // DERECATED:
        ...
        switch (i) {
            ...
            case 2:
                ...
                break;
            default:
                b = 10;  // 10 for every level above-equal to 3 (if any that is...)
        }
        ...
        for (int r = 0; r < lev.im.rows; r++) {
            ...
            while (v < lev.vCorners.size() && r > lev.vCorners[v].y)
                v++;
...
void KeyFrame::MakeKeyFrame_Rest() {
    ...
    static pvar3<double> pvdCandidateMinSTScore(
    ...
    for (int l = 0; l < LEVELS; l++) {
        ...
        for (iCorner = lev.vMaxCorners.begin();
             iCorner != lev.vMaxCorners.end(); iCorner++) {

            if (!CvUtils::in_image_with_border(iCorner->y, iCorner->x, lev.im,
                                               10, 10))
                continue;
...
struct
    LevelHelpersFiller  // Code which should be initialised on init goes here; this runs before main()
{
    LevelHelpersFiller() {
        for (int i = 0; i < LEVELS; i++)
            switch (i) {
                case 0:
                    gavLevelColors[i] = cv::Vec3f(1.0, 0.0, 0.0);
                    break;
                case 1:
                    gavLevelColors[i] = cv::Vec3f(1.0, 1.0, 0.0);
                    break;
                case 2:
                    gavLevelColors[i] = cv::Vec3f(0.0, 1.0, 0.0);
                    break;
                case 3:
                    gavLevelColors[i] = cv::Vec3f(0.0, 0.0, 0.7);
                    break;
                default:
                    gavLevelColors[i] = cv::Vec3f(
                        1.0, 1.0, 0.7);  // In case I ever run with LEVELS > 4
            }
    }
};
