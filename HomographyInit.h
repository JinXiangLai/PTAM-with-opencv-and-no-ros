...
// Storage for each homography decomposition
{
    ...
    SE3<>
        se3SecondFromFirst;  // The rigid transformation in fashion [R|t] which basically transforms
...
class HomographyInit : public Initializer {
   ...
   public:
    ...
    bool Compute(std::vector<InitializerMatch> vMatches, double dMaxPixelError,
   ...
   protected:
    ...
    cv::Matx<float, 3, 3> HomographyFromMatches(
    ...
    bool IsHomographyInlier(cv::Matx<float, 3, 3> m3Homography,
                            InitializerMatch match);
    double MLESACScore(cv::Matx<float, 3, 3> m3Homography,
                       InitializerMatch match);
