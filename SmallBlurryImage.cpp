...
void SmallBlurryImage::MakeFromKF(KeyFrame& kf, double dBlur) {
    ...
    if (mirSize.width == -1)
        mirSize =
            cv::Size2i(kf.aLevels[3].im.cols / 2, kf.aLevels[3].im.rows / 2);
    ...
    int gkerSize = (int)ceil(
        dBlur * 3.0);  // where 3.0 is the default "sigmas" parameter in libCVD
...
void SmallBlurryImage::MakeGradients() {
    ...
    int r, c;
    float *tRowPtr00, *tRowPtr10,
        *tRowPtr_10;  // template row pointers at rows  r, r+1, r-1
...
pair<SE2<>, double> SmallBlurryImage::IteratePosRelToTarget(
    ...
    for (int it = 0; it < nIterations; it++) {
        ...
        cv::Vec2f v2Zero(0, 0);
        CvUtils::transform<>(mimTemplate, imWarped,
        ...
        float *iwRowPtr00, *iwRowPtr10,
            *iwRowPtr_10;  // r, r+1, r-1 row pointers to warped image
        ...
        for (r = 1; r < mirSize.height; r++) {
            ...
            otRowPtr =
                other.mimTemplate.ptr<float>(r);  // r row of other.mimTemplate
            ogRowPtr = other.mimImageGradients.ptr<cv::Vec<float, 2> >(
                r);  // r row of other.mimImageGradients
            ...
            for (c = 1; c < mirSize.width; c++) {
                ...
                if (left_ + right_ + up_ + down_ + here_ < -9999.9)
                    continue;

                //cout <<"DEBUG: l, r, u, d: "<<left_<<" , "<<right_<<" , "<<up_<<" , "<<down_<<endl;

                // now this is the gradient of TARGET image at the warped locations
                cv::Vec2f v2CurrentGrad(
                    right_ -
                        left_,  // Missing 0.5 factor (read below about the notorious lost factor...)
                    down_ - up_);
                ...
                cv::Vec2f v2GradAvg =
                    0.25 * (v2CurrentGrad +
                            ogRowPtr[c]);  // actualy, this is 0.5 * v2GradVg
                ...
                v4Jac[0] = v2GradAvg[0];
                v4Jac[1] = v2GradAvg[1];
                v4Jac[2] = -(r - irCenter.y) * v2GradAvg[0] +
                           (c - irCenter.x) * v2GradAvg[1];
                ...
                UT[0] += v4Jac[0] * v4Jac[0] / s;
                UT[1] += v4Jac[0] * v4Jac[1] / s;
                UT[2] += v4Jac[0] * v4Jac[2] / s;
                UT[3] += v4Jac[0] * v4Jac[3] / s;
                UT[4] += v4Jac[1] * v4Jac[1] / s;
                UT[5] += v4Jac[1] * v4Jac[2] / s;
                UT[6] += v4Jac[1] * v4Jac[3] / s;
                UT[7] += v4Jac[2] * v4Jac[2] / s;
                UT[8] += v4Jac[2] * v4Jac[3] / s;
...
SE3<> SmallBlurryImage::SE3fromSE2(SE2<> se2, ATANCamera camera) {
    ...
    av2Turned[0] =
        cv::Vec<float, 2>(mirSize.width / 2 + m2R(0, 0) * 5 + v2t[0],
    ...
    av3OrigPoints[0] = CvUtils::backproject(
        camera.UnProject(mirSize.width / 2 + 5, mirSize.height / 2));
    av3OrigPoints[1] = CvUtils::backproject(
        camera.UnProject(mirSize.width / 2 - 5, mirSize.height / 2));
    ...
    for (int it = 0; it < 3; it++) {
        ...
        cv::Matx<float, 3, 3> m3Omega =
            10.0 *
            cv::Matx<float, 3,
                     3>::eye();  // information matrix. initialize with 10*I3
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (int m = 0; m < 3; m++) {
                ...
                const cv::Vec<float, 3> v3Motion =
                    SO3<>::generator_field(m, v3Cam);
