...
MapMaker::MapMaker(Map& m, const ATANCamera& cam) : mMap(m), mCamera(cam) {
    ...
    PV3::Register(mpvdWiggleScale, "MapMaker.WiggleScale", 0.05,
...
void MapMaker::Reset() {
    ...
    mvFailureQueue
        .clear();  // clear the list of keyframes and failed observations

    while (!mqNewQueue.empty())
        mqNewQueue.pop();  // clearing the queue of newly detected points

    mMap.vpKeyFrames
        .clear();  // TODO: actually erase old keyframes - we'll see... no need for now...
...
void MapMaker::run() {
    ...
    while (
        !flag_StopRequest) {  // ShouldStop is a CVD::Thread func which return true if the thread is told to exit.
...
void MapMaker::HandleBadPoints() {
    ...
    for (unsigned int i = 0; i < mMap.vpPoints.size(); i++) {
        ...
        if (pMP->nMEstimatorOutlierCount > 20 &&
            pMP->nMEstimatorOutlierCount > pMP->nMEstimatorInlierCount) {
            pMP->bBad = true;
        }
    ...
    for (unsigned int i = 0; i < mMap.vpPointsTrash.size(); i++) {
        ...
        if (mMap.vpPointsTrash[i].use_count() == 0)
            continue;
        ...
        for (unsigned int j = 0; j < mMap.vpKeyFrames.size(); j++) {
            ...
            if (pKF.use_count() > 0)
                if (pKF->mMeasurements.count(pMP))
                    pKF->mMeasurements.erase(
                        pMP);  // just erase the bad point from the map!
...
cv::Vec<float, 3> MapMaker::ReconstructPoint2Views(
    SE3<float> se3AfromB,  // relative pose in m2 = R * m2 + t fashion
    const cv::Vec<float, 2>&
        v2A,  // Normalized Euclidean coordinates in the first view
    const cv::Vec<float, 2>&
        v2B)  // Normalized Euclidean coordinates in the second view
{
    ...
    float Q[6] = {
        RtW2[0][0] * r00 + RtW2[0][1] * r10 + RtW2[0][2] * r20 + 1,
    ...
    // 3. ksi = -R'*W2*t
    float ksi[3] = {
        -(RtW2[0][0] * t[0] + RtW2[0][1] * t[1] + RtW2[0][2] * t[2]),
    ...
    if (fabs(det) < 10E-6) {
        ...
        return cv::Vec3f(
            0, 0,
            -1);  // return negative depth so that the mapmaker drops the point
    }
    ...
    cv::Vec<float, 3> v3M(
        Qinv[0] * ksi[0] + Qinv[1] * ksi[1] + Qinv[2] * ksi[2],
...
void MapMaker::ThinCandidates(KeyFrame::Ptr pKF, int nLevel) {
    ...
    meas_it ipKF_Meas;
    for (ipKF_Meas = pKF->mMeasurements.begin();
         ipKF_Meas != pKF->mMeasurements.end(); ipKF_Meas++) {
        // we are only interested in measureements the chosen level (nlevel) or the next one (nlevel+1)
        if (!(ipKF_Meas->second.nLevel == nLevel ||
              ipKF_Meas->second.nLevel == nLevel + 1))
            continue;
        // Now register the merasurement location by scaling its root coordinates to match the chosen level scale
        irBusyLevelPos.push_back(CvUtils::roundIL((1.0 / LevelScale(nLevel)) *
                                                  ipKF_Meas->second.v2RootPos));
    }
    ...
    for (unsigned int candidateIndex = 0; candidateIndex < vCSrc.size();
         candidateIndex++) {
        ...
        for (unsigned int j = 0; j < irBusyLevelPos.size(); j++) {
            ...
            if ((irB.x - irC.x) * (irB.x - irC.x) +
                    (irB.y - irC.y) * (irB.y - irC.y) <
                nMinMagSquared) {
        ...
        if (bGood)
            vCGood.push_back(vCSrc[candidateIndex]);
    }
...
void MapMaker::AddKeyFrame(KeyFrame::Ptr pKF) {
    pKF->pSBI =
        NULL;  // Mapmaker uses a different SBI than the tracker, so will re-gen its own
...
void MapMaker::AddKeyFrameFromTopOfQueue() {
    ...
    for (meas_it ipMP_Meas = pKF->mMeasurements.begin();
         ipMP_Meas != pKF->mMeasurements.end(); ipMP_Meas++) {
...
// if a match is found.
bool MapMaker::AddPointEpipolar(KeyFrame::Ptr pKFSrc, KeyFrame::Ptr pKFTarget,
                                int nLevel, int nCandidate) {
    static cv::Mat_<cv::Vec<float, 2> >
        imUnProj;  // the normalized Euclidean projections
    ...
    if (!bMadeCache) {
        ...
        for (y = 0; y < imUnProj.rows; y++)
            for (x = 0; x < imUnProj.cols; x++)
                imUnProj(y, x) = mCamera.UnProject(
                    x,
                    y);  // very weird syntax. It makes sense in TooN notation though...
    ...
    cv::Vec<float, 3> v3Ray_SC =
        CvUtils::backproject(mCamera.UnProject(v2RootPos));
    // And normalize it...
    v3Ray_SC = CvUtils::normalize(v3Ray_SC);
    // Finally, express this direction vector in the TARGET keyframe coordibnates
    cv::Vec<float, 3> v3LineDirn_TC =
        pKFTarget->se3CfromW.get_rotation() *
        (pKFSrc->se3CfromW.get_rotation().inverse() * v3Ray_SC);
    ...
    cv::Vec<float, 3> v3CamCenter_TC =
        pKFTarget->se3CfromW * pKFSrc->se3CfromW.inverse().get_translation();
    ...
    if (v3RayStart_TC[2] <= 0.0)
        v3RayStart_TC +=
            (0.001 - v3RayStart_TC[2] / v3LineDirn_TC[2]) * v3LineDirn_TC;
    ...
    double dMinLen =
        min(v2AlongProjectedLine.dot(v2A), v2AlongProjectedLine.dot(v2B)) -
        0.05;
    double dMaxLen =
        max(v2AlongProjectedLine.dot(v2A), v2AlongProjectedLine.dot(v2B)) +
        0.05;
    ...
    vector<cv::Vec<float, 2> >& vv2Corners =
        pKFTarget->aLevels[nLevel].vImplaneCorners;
    ...
    if (!pKFTarget->aLevels[nLevel].bImplaneCornersCached) {
        ...
        for (unsigned int i = 0; i < vIR.size();
            ...
            vv2Corners.push_back(imUnProj(
                p2LevelPos.y,
                p2LevelPos
                    .x));  // n.b. "ir" is simply integer casting in libCVD....
        }
    ...
    for (unsigned int i = 0; i < vv2Corners.size();
        ...
        if (v2Im.dot(v2AlongProjectedLine) < dMinLen)
            continue;  // skip if not far enough along line
        if (v2Im.dot(v2AlongProjectedLine) > dMaxLen)
            continue;  // or too far
    ...
    //      Anyway...
    cv::Vec<float, 3> v3New_TC = ReconstructPoint2Views(
        pKFSrc->se3CfromW * pKFTarget->se3CfromW.inverse(),
        mCamera.UnProject(Finder.GetSubPixPos()), mCamera.UnProject(v2RootPos));
    ...
    pNew->v3OneRightFromCenter_NEC = CvUtils::backproject(
        mCamera.UnProject(v2RootPos[0] + nLevelScale, v2RootPos[1]));
    // Normalized Euclidean coordinates of the +1 pixel down.
    pNew->v3OneDownFromCenter_NEC = CvUtils::backproject(
        mCamera.UnProject(v2RootPos[0], v2RootPos[1] + nLevelScale));

    // Normalize the "batsignal" rays...
    pNew->v3Center_NEC = CvUtils::normalize(pNew->v3Center_NEC);
    pNew->v3OneDownFromCenter_NEC =
        CvUtils::normalize(pNew->v3OneDownFromCenter_NEC);
    pNew->v3OneRightFromCenter_NEC =
        CvUtils::normalize(pNew->v3OneRightFromCenter_NEC);
    ...
    KFMeasurement m;
    // Root measurement
    m.Source = KFMeasurement::
        SRC_ROOT;  // This is a Root feature (but from a higher pyramidal level most likely...)
    ...
    m.Source = KFMeasurement::
        SRC_EPIPOLAR;  // This feature was found with epipolar search
...
vector<std::shared_ptr<KeyFrame> > MapMaker::NClosestKeyFrames(KeyFrame::Ptr pk,
                                                               unsigned int N) {

    vector<pair<double, KeyFrame::Ptr> > vKFandScores;
    for (unsigned int i = 0; i < mMap.vpKeyFrames.size(); i++) {

        if (mMap.vpKeyFrames[i] == pk)
            continue;
...
KeyFrame::Ptr MapMaker::ClosestKeyFrame(KeyFrame::Ptr pk) {
    ...
    assert(nClosest != -1 &&
           "For some reason I ended up with negative closest distance! CHECK "
           "YOUR CODE!!!!");
...
bool MapMaker::NeedNewKeyFrame(KeyFrame::Ptr pKFCurrent) {
    ...
    if (dist > PV3::get<double>("MapMaker.MaxKFDistWiggleMult", 1.0, SILENT) *
...
void MapMaker::BundleAdjustAll() {
    ...
    for (unsigned int i = 0; i < mMap.vpKeyFrames.size(); i++) {
        ...
        if (mMap.vpKeyFrames[i]->bFixed)
            sKFsFixed.insert(mMap.vpKeyFrames[i]);
        else
    ...
    for (unsigned int i = 0; i < mMap.vpPoints.size(); i++)
        ...
        if (!mMap.vpPoints[i]
                 ->bBad)  // there could actually be bad points, so i added the check
            sMapPoints.insert(mMap.vpPoints[i]);

    // It is likely that points may not be enough...
    if (sMapPoints.size() < 6) {
        cout << "DEBUG: Too few Mappoints to bundle adjust ALL: "
             << sMapPoints.size() << endl;
        RequestReset();
        return;
    }
    if (sKFs2Adjust.size() == 0) {
        cout << "DEBUG: Too few KFs to bundle adjust ALL: "
             << sKFs2Adjust.size() << endl;
        RequestReset();
        return;
    }
...
void MapMaker::BundleAdjustRecent() {
    ...
    for (siKF = sKFs2Adjust.begin(); siKF != sKFs2Adjust.end(); siKF++) {
        ...
        map<MapPoint::Ptr, KFMeasurement>& mKFMeasurements =
            (*siKF)->mMeasurements;
        meas_it ipMP_Meas;
        for (ipMP_Meas = mKFMeasurements.begin();
             ipMP_Meas != mKFMeasurements.end(); ipMP_Meas++)
    ...
    for (viKF = mMap.vpKeyFrames.begin(); viKF != mMap.vpKeyFrames.end();
         viKF++) {

        // if the KF is already in the list, then skip the rest...
        if (sKFs2Adjust.count(*viKF))
            continue;

        //bool bInclude = false; // why on earth do we need to use this???
        meas_it ipMP_Meas;
        for (ipMP_Meas = (*viKF)->mMeasurements.begin();
             ipMP_Meas != (*viKF)->mMeasurements.end(); ipMP_Meas++)
...
void MapMaker::BundleAdjust(set<KeyFrame::Ptr> sAdjustSet,
                            set<KeyFrame::Ptr> sFixedSet,
                            set<MapPoint::Ptr> sMapPoints, bool bRecent) {
    ...
    for (unsigned int KF_Index = 0; KF_Index < mMap.vpKeyFrames.size();
         KF_Index++) {

        // if there is no bundle camera index for this keyframe, skip...
        if (mView_BundleID.count(mMap.vpKeyFrames[KF_Index]) == 0)
            continue;
        ...
        for (meas_it ipMP_Meas =
                 mMap.vpKeyFrames[KF_Index]->mMeasurements.begin();
             ipMP_Meas != mMap.vpKeyFrames[KF_Index]->mMeasurements.end();
             ipMP_Meas++) {
            // if there is NO bundle entry for this point, skip (a *MapPoint stored in "first" of the measurement pair)
            if (mPoint_BundleID.count(ipMP_Meas->first) == 0)
                continue;
            ...
            double sigma = LevelScale(
                ipMP_Meas->second
                    .nLevel);  // using level scale as standard deviation
            ba.AddMeasurement(nKF_BundleID, nMP_BundleID,
                              ipMP_Meas->second.v2RootPos, sigma * sigma);
        }
    }

    // Run the bundle adjuster. This returns the number of successful iterations
    cout << "DEBUG: Attempting BA with " << mMap.vpPoints.size()
         << " points in the map..." << endl;
    ...
    if (nAccepted > 0) {
        ...
        map<MapPoint::Ptr, int>::iterator
            ipMP_ID;  // point-index post-BA iterator

        // update points
        for (ipMP_ID = mPoint_BundleID.begin();
             ipMP_ID != mPoint_BundleID.end(); ipMP_ID++) {

            ipMP_ID->first->v3WorldPos = ba.GetPointCoords(ipMP_ID->second);
        }

        // update keyframe coord. frames
        map<KeyFrame::Ptr, int>::iterator
            ipKF_ID;  // cam-index post-BA iterator
        for (ipKF_ID = mView_BundleID.begin(); ipKF_ID != mView_BundleID.end();
             ipKF_ID++) {
    ...
    for (unsigned int pairIndex = 0; pairIndex < vOutliers.size();
         pairIndex++) {
        ...
        if (pMP->pMMData->GoodMeasCount() <= 2 ||
            m.Source == KFMeasurement::SRC_ROOT)
            pMP->bBad = true;
        else {

            // Do we retry it? Depends where it came from!!
            if (m.Source == KFMeasurement::SRC_TRACKER ||
                m.Source == KFMeasurement::SRC_EPIPOLAR)
                mvFailureQueue.push_back(
                    pair<KeyFrame::Ptr, MapPoint::Ptr>(pKF, pMP));
            else
...
bool MapMaker::ReFind_Common(KeyFrame::Ptr pKF, MapPoint::Ptr pMP) {
    ...
    if (pMP->pMMData->sMeasurementKFs.count(pKF) ||
        pMP->pMMData->sNeverRetryKFs.count(pKF))
        return false;
    ...
    if (v2Image[0] < 0 || v2Image[1] < 0 ||
        v2Image[0] > pKF->aLevels[0].im.cols - 1 ||
        v2Image[1] > pKF->aLevels[0].im.rows - 1) {
        pMP->pMMData->sNeverRetryKFs.insert(pKF);
        return false;
    }
    ...
    bool bFound =
        Finder.FindPatchCoarse(v2Image, pKF, 4);  // Very tight search radius!
    ...
    if (pKF->mMeasurements.count(pMP)) {
        ...
        assert(
            0 &&
            "You are trying to track a mappoint that has ALREADY been "
            "processed!");  // This should never happen, we checked for this at the start.
    }
...
int MapMaker::ReFindInSingleKeyFrame(KeyFrame::Ptr pKF) {
    ...
    for (unsigned int i = 0; i < vToFind.size(); i++)
        if (ReFind_Common(pKF, vToFind[i]))
            nFoundNow++;
...
void MapMaker::ReFindFromFailureQueue() {
    if (mvFailureQueue.size() == 0)
        return;
    cout << "DEBUG: *************************************** Failure Queue Size "
            ": "
         << mvFailureQueue.size() << endl;
    ...
    for (unsigned int i = 0; i < mvFailureQueue.size(); i++) {
        ...
        if (KF_MP.first.use_count() > 0)
            if (KF_MP.second.use_count() > 0)
                if (ReFind_Common(KF_MP.first, KF_MP.second))
                    nFound++;
    }

    cout << "DEBUG: **************************** Erasing failure to the end! "
         << endl;
...
SE3<> MapMaker::CalcPlaneAligner() {
    ...
    int nRansacs =
        PV3::get<int>("MapMaker.PlaneAlignerRansacs", 100, HIDDEN | SILENT);
    ...
    for (int i = 0; i < nRansacs; i++) {
        ...
        cv::Vec<float, 3> v3BaryCenter =
            0.33333333 * (mMap.vpPoints[A_ind]->v3WorldPos +
                          mMap.vpPoints[B_ind]->v3WorldPos +
                          mMap.vpPoints[C_ind]->v3WorldPos);

        cv::Vec<float, 3> v3AC =
            mMap.vpPoints[C_ind]->v3WorldPos - mMap.vpPoints[A_ind]->v3WorldPos;
        cv::Vec<float, 3> v3AB =
            mMap.vpPoints[B_ind]->v3WorldPos - mMap.vpPoints[A_ind]->v3WorldPos;
        ...
        for (unsigned int i = 0; i < nPoints; i++) {
            ...
            cv::Vec<float, 3> v3Diff =
                mMap.vpPoints[i]->v3WorldPos - v3BaryCenter;
            ...
            if (dNormDist > 0.05)
                dNormDist = 0.05;  // arbitrarilly chosen "robust" cut-off bound
            dSumError += dNormDist;
        }
    ...
    cv::Vec<float, 3> v3Normal(Vt(2, 0), Vt(2, 1), Vt(2, 2));
    ...
    m3Rot(2, 0) = v3Normal[0];
    m3Rot(2, 1) = v3Normal[1];
    m3Rot(2, 2) = v3Normal[2];
    ...
    m3Rot(0, 0) = m3Rot(0, 0) - dot13 * v3Normal[0];
    m3Rot(0, 1) = m3Rot(0, 1) - dot13 * v3Normal[1];
    m3Rot(0, 2) = m3Rot(0, 2) - dot13 * v3Normal[2];
...
void MapMaker::RefreshSceneDepth(KeyFrame::Ptr pKF) {
    ...
    meas_it ipMP_Meas;
    for (ipMP_Meas = pKF->mMeasurements.begin();
         ipMP_Meas != pKF->mMeasurements.end(); ipMP_Meas++) {
    ...
    assert(numMeasurements >
           2);  // If not then something is seriously wrong with this KF!!
    pKF->dSceneDepthMean = dSumDepth / numMeasurements;
    pKF->dSceneDepthSigma =
        sqrt((dSumDepthSquared / numMeasurements) -
...
void MapMaker::GUICommandHandler(
    string sCommand, string sParams)  // Called by the callback func..
{
    if (sCommand == "SaveMap") {
