...
struct InitializerMatch {
    ...
    cv::Matx<float, 2, 2>
        m2PixelProjectionGrad;  // Maybe ready-made derivatives... For the case they are needed...
};

class Initializer {
   public:
    // compute and return relative pose (se3SecondCameraPose)
    virtual bool Compute(std::vector<InitializerMatch> vMatches,
                         double dMaxPixelError, SE3<>& se3SecondCameraPose) = 0;
