...
ATANCamera::ATANCamera(string sName, const cv::Size2i imgsize) {
    ...
    ATANCamera::mvDefaultParams =
        cv::Vec<float, NUMTRACKERCAMPARAMETERS>(0.5, 4.0 / 5.0, 0.5, 0.5, 0.07);
    ...
    PV3::get<cv::Vec<float, NUMTRACKERCAMPARAMETERS> >(
        msName + ".Parameters", ATANCamera::mvDefaultParams,
        Persistence::SILENT);
    PV3.Register(mpvvCameraParams, sName + ".Parameters", mvDefaultParams,
...
void ATANCamera::RefreshParams() {
    ...
    mvFocal[0] =
        mvImageSize.width * (*mpvvCameraParams)[0];  // fx = p[0] * IMG_WIDTH
    mvFocal[1] =
        mvImageSize.height * (*mpvvCameraParams)[1];  // fy = p[1] * IMG_HEIGHT
    mvCenter[0] =
        mvImageSize.width * (*mpvvCameraParams)[2] -
        0.5;  // cx = IMG_WIDTH * p[2]  (NOTE!!! the 0.5 is just an OpenGL related arbitrary offset;
    ...
    mvCenter[1] =
        mvImageSize.height * (*mpvvCameraParams)[3] -
        0.5;  // cy = IMG_HEIGHT * p[3] (again, 0.5 is just an arbitrary offfset...)

    // One over focal length
    mvInvFocal[0] =
        1.0 / mvFocal[0];  // inverse "horizontal: focal length: 1 / fx
    mvInvFocal[1] =
        1.0 / mvFocal[1];  // inverse "vertical" focal length: 1 / fy

    // Some radial distortion parameters..
    mdW = (*mpvvCameraParams)
        [4];  // The Devernay-Faugeras radial distortion FOV model parameter (AKA "angle")
    if (mdW != 0.0) {
    ...
    cv::Vec2f v2(
        std::max((*mpvvCameraParams)[2], 1.0f - (*mpvvCameraParams)[2]) /
            (*mpvvCameraParams)[0],
        std::max((*mpvvCameraParams)[3], 1.0f - (*mpvvCameraParams)[3]) /
            (*mpvvCameraParams)[1]);
    ...
    {
        cv::Vec2f v2Center =
            UnProject(0.5 * mvImageSize.width, 0.5 * mvImageSize.height);
        cv::Vec2f v2RootTwoAway = UnProject(0.5 * mvImageSize.width + 1.0,
                                            0.5 * mvImageSize.height + 1.0);
    ...
    // Work out the linear projection values for the UFB
    {
        ...
        vv2Verts.push_back(UnProject(
            -0.5,
            -0.5));  // Backproject the central pixel (Again, what is implied here is that (0,0) is
            // the left-lower corner, but recall from above that +-0.5 is a necessary offset for display reasons...)
        vv2Verts.push_back(UnProject(
            mvImageSize.width - 0.5,
            -0.5));  // Backproject Right lower - left corner to Euclidean norm. plane
        vv2Verts.push_back(UnProject(
            mvImageSize.width - 0.5,
            mvImageSize.height -
                0.5));  // Backproject Right-Upper corner to norm. Euclidean plabe.
        vv2Verts.push_back(UnProject(
            -0.5,
            mvImageSize.height -
                0.5));  // Backproject Left-Upper corner to norm. Euclidean plane.
...
cv::Matx<float, 4, 4> ATANCamera::MakeUFBLinearFrustumMatrix(float near,
                                                             float far) {
...
cv::Matx<float, 2, 2> ATANCamera::GetProjectionDerivs() {
    ...
    float ru =
        mdLastR *
        mdDistortionEnabled;  // the result here is mdLastR or 0.0, ru = sqrt(xe^2 + ye^2)
    ...
    // otherwise, things get tough .... we need to get the derivative of the fraction in terms of xe and ye inside square->square root -> tangent....
        ...
        dFracBydx =
            (mdWinv * k / (1 + k * k * ru * ru) - mdLastFactor) * x / (ru * ru);

        dFracBydy =
            (mdWinv * k / (1 + k * k * ru * ru) - mdLastFactor) * y / (ru * ru);
    }
...
cv::Matx<float, 2, NUMTRACKERCAMPARAMETERS>
ATANCamera::GetCameraParameterDerivs() {
    ...
    for (int i = 0; i < NUMTRACKERCAMPARAMETERS; i++) {
        ...
        cv::Vec<float, NUMTRACKERCAMPARAMETERS> vNUpdate =
            cv::Vec<float, NUMTRACKERCAMPARAMETERS>::all(0);
        vNUpdate[i] += 0.001;
...
cv::Matx<float, 2, NUMTRACKERCAMPARAMETERS>
ATANCamera::GetCamParamAnalyticalDerivs() {
    // Differentials wrt to the camera parameters
    // Use these to calibrate the camera
    // No need for this to be quick, so do them numerically
...
cv::Vec2f ATANCamera::UFBProject(const cv::Matx<float, 2, 1>& vCam) {
    ...
    mvLastIm[0] =
        (*mpvvCameraParams)[2] + (*mpvvCameraParams)[0] * mvLastDistCam[0];
    mvLastIm[1] =
        (*mpvvCameraParams)[3] + (*mpvvCameraParams)[1] * mvLastDistCam[1];
    return mvLastIm;
}
...
cv::Vec2f ATANCamera::UFBUnProject(const cv::Matx<float, 2, 1>& v2Im) {
    ...
    mvLastDistCam[0] =
        (mvLastIm[0] - (*mpvvCameraParams)[2]) / (*mpvvCameraParams)[0];
    mvLastDistCam[1] =
        (mvLastIm[1] - (*mpvvCameraParams)[3]) / (*mpvvCameraParams)[1];
    mdLastDistR = sqrt(mvLastDistCam[0] * mvLastDistCam[0] +
                       mvLastDistCam[1] * mvLastDistCam[1]);
