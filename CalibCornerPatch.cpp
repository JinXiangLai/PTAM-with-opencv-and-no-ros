...
void CalibCornerPatch::MakeTemplateWithCurrentParams() {
    ...
    int nOffsetCol, nOffsetRow;
    {
        ...
        CvUtils::transform<>(
            mimSharedSourceTemplate, imTwiceToBlur, CvUtils::M2Inverse(m2Warp),
            0.5 * cv::Vec2f(mimSharedSourceTemplate.cols - 1,
                            mimSharedSourceTemplate.rows - 1),
        ...
        cv::GaussianBlur(imToBlur, temp, cv::Size(gkerSize, gkerSize),
                         dBlurSigma);
    ...
    for (r = 0; r < mimGradients.rows; r++) {
        ...
        for (c = 0; c < mimGradients.cols; c++) {
            gRowPtr[c][0] = 0.5 * (bRowPtr[c + nOffsetCol + 1] -
                                   bRowPtr[c + nOffsetCol - 1]);
            gRowPtr[c][1] =
                0.5 * (bRowPtr1[c + nOffsetCol] - bRowPtr_1[c + nOffsetCol]);
        }
    }
...
bool CalibCornerPatch::IterateOnImageWithDrawing(
    CalibCornerPatch::Params& params, cv::Mat_<uchar>& im) {
...
bool CalibCornerPatch::IterateOnImage(CalibCornerPatch::Params& params,
    ...
    for (int i = 0; i < 20; i++) {
        ...
        dLastUpdate = Iterate(
            im);  // store the norm of the last position update in "dLastUpdate"
...
float CalibCornerPatch::Iterate(cv::Mat_<uchar>& im) {
    ...
    cv::Vec2d v2TL = cv::Vec2d(mParams.v2Pos[0] - (mimTemplate.cols - 1) / 2.0,
                               mParams.v2Pos[1] - (mimTemplate.rows - 1) / 2.0);
    if (!(v2TL[0] >= 0.0 && v2TL[1] >= 0.0))
        return -1.0;

    // And the Bottom-Right (BR) corner of the patch in the image
    cv::Vec2d v2BR =
        v2TL + cv::Vec2d(mimTemplate.cols - 1, mimTemplate.rows - 1);
    if ((v2BR[0] >= (im.cols - 1.0)) || (v2BR[1] >= (im.rows - 1.0)))
        return -1.0;

    // create an image interpolation object
    CvUtils::image_interpolate<CvUtils::Interpolate::Bilinear, uchar> imInterp(
        im);

    // normal equation matrices. Prepare for Least Squares!
    cv::Matx<double, 6, 6> m6JTJ = cv::Matx<double, 6, 6>::
        zeros();  // this is the J'*J (can be though of as information matrix)
    ...
    for (r = 0; r < mimTemplate.rows; r++) {
        ...
        for (c = 0; c < mimTemplate.cols; c++) {
            ...
            cv::Vec2d v2Pos_Template(c - (mimTemplate.cols - 1) / 2.0,
                                     r - (mimTemplate.rows - 1) / 2.0);
            ...
            double dDiff = imInterp[v2Pos_Image][0] -
                           (mParams.dGain * tRowPtr[c] + mParams.dMean);
