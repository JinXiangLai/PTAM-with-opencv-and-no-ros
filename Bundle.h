...
struct BAPoint {
    ...
    std::vector<OffDiagScriptEntry>
        vOffDiagonalScript;  // A record of all camera-camera pairs observing this point
};
...
struct BAMeasurement {
    ...
    inline bool operator<(const BAMeasurement& rhs) const {
        return (camIndex < rhs.camIndex ||
                (camIndex == rhs.camIndex && pointIndex < rhs.pointIndex));
    }
...
class Bundle {
   public:
    Bundle(
        const ATANCamera&
            TCam);  // We need the camera model because we do full distorting projection in the bundle adjuster. Could probably get away with a linear approximation.
    int AddCamera(
        SE3<> se3CamFromWorld,
        bool
            bFixed);  // Add a viewpoint. bFixed signifies that this one is not to be adjusted.
    int AddPoint(cv::Vec<float, 3> v3Pos);  // Add a map point.
    void AddMeasurement(int nCam, int nPoint, cv::Vec<float, 2> v2Pos,
                        double dSigmaSquared);  // Add a measurement
    int Compute(
        bool*
            pbAbortSignal);  // Perform bundle adjustment. Aborts if *pbAbortSignal gets set to true. Returns number of accepted update iterations, or negative on error.
   ...
   protected:
    inline void ProjectAndFindSquaredError(
        BAMeasurement&
            measurement);  // Project a single point in a single view, compare to measurement
    ...
    std::vector<std::vector<BAMeasurement*> >
        mvMeasLUTs;  //Each camera gets a per-point table of pointers to valid measurements
