...
class PatchFinder {
   ...
   public:
    ...
    // transformation.
    int CalcSearchLevelAndWarpMatrix(MapPoint::Ptr p, SE3<> se3CFromW,
                                     cv::Matx<float, 2, 2>& m2CamDerivs);
    ...
    void MakeTemplateCoarseCont(
        MapPoint::Ptr
            p);  // If the warping matrix has already been pre-calced, use this.
    void MakeTemplateCoarse(
        MapPoint::Ptr p, SE3<> se3CFromW,
        cv::Matx<float, 2, 2>& m2CamDerivs);  // This also calculates the warp.
    void MakeTemplateCoarseNoWarp(
        MapPoint::Ptr
            p);  // Identity warp: just copies pixels from the source KF.
    void MakeTemplateCoarseNoWarp(
        std::shared_ptr<KeyFrame> k, int nLevel,
        cv::Point2i
            irLevelPos);  // Identity warp if no MapPoint struct exists yet.
    ...
    bool FindPatchCoarse(cv::Vec<float, 2> v2Pos, std::shared_ptr<KeyFrame> pKF,
                         unsigned int nRange);
    int ZMSSDAtPoint(
        const cv::Mat_<uchar>& im,
        const cv::Point2i& ir);  // This evaluates the score at one location
    // Results from step 3:
    // All positions are in the scale of level 0.
    inline cv::Point2i GetCoarsePos() {
        return cv::Point2i((int)mv2CoarsePos[0], (int)mv2CoarsePos[1]);
    }
    inline cv::Vec2f GetCoarsePosAsVector() { return mv2CoarsePos; }

    // Step 4
    void
    prepSubPixGNStep();  // This is the former "MakeSubPixTemplate". It precomputes the
    ...
    double IterateSubPix(
        std::shared_ptr<KeyFrame>
            kf);  // Single iteration of IC. Returns sum-squared pixel update dist, or negative if out of imag
    inline cv::Vec2f GetSubPixPos() { return mv2SubPixPos; }  // Get result
    void SetSubPixPos(cv::Vec2f v2) {
        mv2SubPixPos = v2;
    }  // Set starting point
   ...
   protected:
    ...
    cv::Mat_<cv::Vec<float, 3> >
        mimJacs;  // Inverse composition jacobians; stored as floats to save a bit of space.
    ...
    cv::Vec<float, 2>
        mv2PredictedPos;  // Search center location of FindPatchCoarse in L0
    ...
    cv::Vec2f
        mv2CoarsePos;  // In the scale of level 0; hence the use of vector rather than cv::Point2i
    cv::Point2i
        mirCenter;  // Quantized location of the center pixel of the NxN pixel template
    ...
    MapPoint::Ptr
        mpLastTemplateMapPoint;  // Which was the last map point this PatchFinder used?
    cv::Matx<float, 2, 2>
        mm2LastWarpMatrix;  // What was the last 2x2 warp matrix this PatchFinder used?
};
