...
Tracker::Tracker(cv::Size2i irVideoSize, const ATANCamera& c, Map& m,
                 MapMaker& mm)
    : mMap(m),
      mMapMaker(mm),
      mCamera(c),
...
void Tracker::Reset() {
    ...
    mnFrame = 0;
    mv6CameraVelocity = cv::Vec<float, 6>(
        0, 0, 0, 0, 0, 0);  // could use all() here, but its just 6 zeros....
...
void Tracker::TrackFrame(cv::Mat_<uchar>& imFrame, bool bDraw,
                         cv::Mat& rgbFrame) {
    mbDraw = bDraw;
    ...
    static pvar3<double> gvdSBIBlur("Tracker.RotationEstimatorBlur", 0.75,
                                    SILENT);
    static pvar3<int> gvnUseSBI("Tracker.UseRotationEstimator", 1, SILENT);
    mbUseSBIInit =
        *gvnUseSBI;  // this flag indicates whether to use an initial estimate
    ...
    if (mbDraw) {
        ...
        if (PV3::get<int>("Tracker.DrawFASTCorners", 1, SILENT)) {
            ...
            glPointSize(1);
            glBegin(GL_POINTS);
            for (unsigned int i = 0; i < pCurrentKF->aLevels[0].vCorners.size();
                 i++)
    ...
    if (mMap.IsGood()) {
        ...
        if (mnLostFrames < 3) {
            ...
            // 因为是跟踪地图，意味着地图点的位置有了，那么自然可以计算仿射矩阵
...
bool Tracker::AttemptRecovery() {
    ...
    mv6CameraVelocity = cv::Vec<float, 6>(
        0, 0, 0, 0, 0, 0);  // avoiding the ::all(0) static method
...
void Tracker::RenderGrid() {
    ...
    for (int i = 0; i < nTot; i++)
        for (int j = 0; j < nTot; j++) {
            ...
            cv::Vec<float, 3> v3((i - nHalfCells) * 0.1, (j - nHalfCells) * 0.1,
                                 0.0);
...
void Tracker::TrackForInitialMap() {
    ...
    if (mnInitialStage == TRAIL_TRACKING_NOT_STARTED) {
        ...
        if (mbUserPressedSpacebar) {
            ...
            TrailTracking_Start();
            mnInitialStage =
                TRAIL_TRACKING_STARTED;  // switch state to TRAIL_TRACKING_STARTED
            // (which implies that we are expecting the second KF anytime soon...)
        } else
            mMessageForUser << "Point camera at planar scene and press "
                               "spacebar to start tracking for initial map."
                            << endl;
        return;
    ...
    if (mnInitialStage == TRAIL_TRACKING_STARTED) {
        ...
        if (nGoodTrails < 10) {
            cout << " Less than 10 correspondences found. Resetting... "
                 << endl;
            Reset();
            return;
        }
        ...
        if (mbUserPressedSpacebar) {
            ...
            vector<pair<cv::Point2i, cv::Point2i> >
                vMatches;  // This is the format the mapmaker wants for the stereo pairs
            for (list<Trail>::iterator i = mlTrails.begin();
                 i != mlTrails.end(); i++)
                vMatches.push_back(pair<cv::Point2i, cv::Point2i>(
                    i->irInitialPos, i->irCurrentPos));
            ...
            //
            mMapMaker.InitFromStereo<EssentialInit>(pFirstKF, pCurrentKF,
                                                    vMatches, mse3CamFromWorld);
            mnInitialStage = TRAIL_TRACKING_COMPLETE;
            //cout <<"completed trail tracking and initialized from stereo! " <<endl;
        } else
            mMessageForUser << "Translate the camera slowly sideways, and "
                               "press spacebar again to perform stereo init."
                            << endl;
    }
}

// Need to provide a comparison function for sort because there is no comparison operator for cv::Point2i
bool cornerCompare(const std::pair<double, cv::Point2i>& left,
                   const std::pair<double, cv::Point2i>& right) {
...
void Tracker::TrailTracking_Start() {
    ...
    for (unsigned int i = 0; i < pCurrentKF->aLevels[0].vCandidates.size();
         i++) {
        ...
        if (!CvUtils::in_image_with_border(
                c.irLevelPos.y, c.irLevelPos.x, pCurrentKF->aLevels[0].im,
                MiniPatch::mnHalfPatchSize, MiniPatch::mnHalfPatchSize))
            continue;

        vCornersAndSTScores.push_back(pair<double, cv::Point2i>(
            -c.dSTScore,
    ...
    sort(vCornersAndSTScores.begin(), vCornersAndSTScores.end(),
         cornerCompare);  // Sort according to Shi-Tomasi score
    ...
    for (unsigned int i = 0; i < vCornersAndSTScores.size() && nToAdd > 0;
        ...
        if (!CvUtils::in_image_with_border(
                vCornersAndSTScores[i].second.y,
                vCornersAndSTScores[i].second.x, pCurrentKF->aLevels[0].im,
                MiniPatch::mnHalfPatchSize, MiniPatch::mnHalfPatchSize))
            continue;
    ...
    pPreviousFrameKF =
        pFirstKF;  // Cache the curfrent KF as "mPreviousFrameKF". It will be useful in any case...
}
...
int Tracker::TrailTracking_Advance() {
    ...
    Level& lCurrentFrame =
        pCurrentKF
            ->aLevels[0];  // The eth version has parametrized this 0-level...
    ...
    for (list<Trail>::iterator i = mlTrails.begin(); i != mlTrails.end();) {
        ...
        if (bFound) {
            ...
            bFound =
                BackwardsPatch.FindPatch(irBackWardsFound, lPreviousFrame.im,
                                         10, lPreviousFrame.vCorners);
            // if the match is not the same (more than 2 pixels apart), then set bFound to false.
            if ((irBackWardsFound.x - irStart.x) *
                        (irBackWardsFound.x - irStart.x) +
                    (irBackWardsFound.y - irStart.y) *
                        (irBackWardsFound.y - irStart.y) >
                2)
                bFound = false;
            else {
        ...
        if (mbDraw) {
            ...
            if (!bFound)
                glColor3f(
                    0, 1,
                    1);  // Failed trails are light blue with a bit of purple at the end.
            else
                glColor3f(
                    1, 1,
            ...
            if (bFound)
                glColor3f(
                    1, 0,
                    0);  // so, finish with red if good, otherwise stay purple and die...
...
void Tracker::TrackMap() {
    ...
    for (int i = 0; i < LEVELS; i++)
        avPVS[i].reserve(
            500);  // preallocating - reserve 500 bytes for each trackerdata entry per level
    ...
    if (bTryCoarse &&
        ...
        vIterationSet =
            vNextToSearch;  // Copy over into the to-be-optimised list.
        ...
        if (nFound >= *gvnCoarseMin) {
            ...
            for (int iter = 0; iter < 10; iter++) {
                ...
                if (iter != 0) {
                    ...
                    for (unsigned int trackDataIndex = 0;
                         trackDataIndex < vIterationSet.size();
                         trackDataIndex++)
                        if (vIterationSet[trackDataIndex]->bFound)
                            vIterationSet[trackDataIndex]->ProjectAndDerivs(
                                mse3CamFromWorld, mCamera);
                }

                for (unsigned int trackDataIndex = 0;
                     trackDataIndex < vIterationSet.size(); trackDataIndex++)
                    if (vIterationSet[trackDataIndex]->bFound)
                ...
                cv::Vec<float, 6> v6Update =
                    CalcPoseUpdate(vIterationSet, dOverrideSigma /*, true*/);
    ...
    // so do all of these, with sub-pixel refinement.
    {
        ...
        for (unsigned int TrackerDataIndex = 0;
             TrackerDataIndex < avPVS[levelIndex].size(); TrackerDataIndex++)
            avPVS[levelIndex][TrackerDataIndex]->ProjectAndDerivs(
                mse3CamFromWorld, mCamera);
        ...
        for (unsigned int TrackerDataIndex = 0;
            ...
            vIterationSet.push_back(
                avPVS
                    [levelIndex]
                    [TrackerDataIndex]);  // Again, plonk all searched points onto the (maybe already populate) vIterationSet.
    }
    ...
    static pvar3<int> gvnMaxPatchesPerFrame("Tracker.MaxPatchesPerFrame", 1000,
                                            SILENT);
    ...
    if (mbDidCoarse)
        for (unsigned int TrackerDataIndex = 0;
             TrackerDataIndex < vNextToSearch.size(); TrackerDataIndex++)
            vNextToSearch[TrackerDataIndex]->ProjectAndDerivs(mse3CamFromWorld,
                                                              mCamera);
    ...
    for (int iter = 0; iter < 10; iter++) {
        ...
        if (iter != 0) {
            ...
            if (bNonLinearIteration) {
                ...
                for (unsigned int TrackerDataIndex = 0;
                     TrackerDataIndex < vIterationSet.size();
                     TrackerDataIndex++)
                    if (vIterationSet[TrackerDataIndex]->bFound)
                        vIterationSet[TrackerDataIndex]->ProjectAndDerivs(
                            mse3CamFromWorld, mCamera);
            } else {
        ...
        if (bNonLinearIteration)
            for (unsigned int TrackerDataIndex = 0;
                 TrackerDataIndex < vIterationSet.size(); TrackerDataIndex++)
                if (vIterationSet[TrackerDataIndex]->bFound)
    ...
    if (mbDraw) {
        ...
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        glBegin(GL_POINTS);
        for (vector<TrackerData::Ptr>::reverse_iterator it =
                 vIterationSet.rbegin();
    ...
    for (ipTData = vIterationSet.begin(); ipTData != vIterationSet.end();
         ipTData++) {
    ...
    if (nNum > 20) {
        ...
        pCurrentKF->dSceneDepthSigma =
            sqrt((dSumSq / nNum) -
                 (pCurrentKF->dSceneDepthMean) * (pCurrentKF->dSceneDepthMean));
    }
}

// Find points in the image. Uses the PatchFiner struct stored in TrackerData
int Tracker::SearchForPoints(vector<TrackerData::Ptr>& vTD, int nRange,
                             int nSubPixIts) {
    int nFound = 0;
    for (unsigned int i = 0; i < vTD.size(); i++) {  // for each point..
        ...
        if (nSubPixIts > 0) {
            ...
            bool bSubPixConverges =
                Finder.IterateSubPixToConvergence(pCurrentKF, nSubPixIts);
...
cv::Vec<float, 6> Tracker::CalcPoseUpdate(vector<TrackerData::Ptr> vTD,
    ...
    if (*pvsEstimator == "Tukey")
        nEstimator = 0;
    else if (*pvsEstimator == "Cauchy")
        nEstimator = 1;
    else if (*pvsEstimator == "Huber")
        nEstimator = 2;
    else {
    ...
    for (unsigned int TrackerDataIndex = 0; TrackerDataIndex < vTD.size();
         TrackerDataIndex++) {
        ...
        pTD->v2Error_CovScaled =
            pTD->dSqrtInvNoise * (pTD->v2Found - pTD->v2Image);
        vdErrorSquared.push_back(
            pTD->v2Error_CovScaled.dot(pTD->v2Error_CovScaled));
    }
    ...
    if (vdErrorSquared.size() < 6)
        return cv::Vec<float, 6>(0, 0, 0, 0, 0, 0);

    // What is the distribution of errors?
    double dSigmaSquared;
    if (dOverrideSigma > 0)
        dSigmaSquared =
            dOverrideSigma;  // Bit of a waste having stored the vector of square errors in this case!
    else {
    ...
    // nonlinear LS step
    cv::Matx<float, 6, 6> m6Omega =
        100.0 *
        cv::Matx<float, 6, 6>::eye();  // make the prior very confident...
    cv::Vec<float, 6> v6ksi(
        0, 0, 0, 0, 0,
        0);  // zero the information matrix (we solve for the perturbation Dx)
    ...
    for (unsigned int TrackerDataIndex = 0; TrackerDataIndex < vTD.size();
        ...
        double dErrorSq =
            v2[0] * v2[0] +
            v2[1] * v2[1];  // avoid overloads and inline code whenever u can...
        double dWeight;
        ...
        if (dWeight < 10e-3) {
            ...
            if (bMarkOutliers)
                pTD->Point->nMEstimatorOutlierCount++;

            continue;
        } else if (bMarkOutliers)
            pTD->Point->nMEstimatorInlierCount++;
        ...
        m6Omega += dWeight * (pTD->dSqrtInvNoise * pTD->dSqrtInvNoise) *
                   (pTD->m26Jacobian.t() * pTD->m26Jacobian);
...
void Tracker::AssessTrackingQuality() {
    ...
    if (nTotalFound == 0 || nTotalAttempted == 0)
        ...
        static pvar3<double> gvdQualityGood("Tracker.TrackingQualityGood", 0.3,
                                            SILENT);
        static pvar3<double> gvdQualityLost("Tracker.TrackingQualityLost", 0.13,
                                            SILENT);
    ...
    if (mTrackingQuality == DODGY) {
        ...
        if (mMapMaker.IsDistanceToNearestKeyFrameExcessive(pCurrentKF))
            mTrackingQuality = BAD;
    }
