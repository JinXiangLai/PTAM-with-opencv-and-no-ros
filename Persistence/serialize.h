...
template <class T>
std::string to_string(const T& val, bool precise) {
...
// and the opposite...
template <class T>
struct FromStream<std::vector<std::vector<T> > > {
...
std::string to_string(const cv::Vec<P, N>& m, bool precise) {
...
template <typename P>
std::string to_string(const cv::Mat_<P>& m, bool precise) {
    ...
    for (int i = 0; i < m.rows; i++) {
        if (i != 0)
            o << "; ";  // separating lines with ';'

        for (int j = 0; j < m.cols; j++) {
            if (j != 0)
                o << " ";  // separating elements in row with space
            o << m[i][j];
        }
    }
    o << "]";
...
struct FromStream<cv::Mat_<P> > {
    ...
    static cv::Mat_<P> from(std::istream& i) {
        ...
        if (i.fail() || i.bad()) {
            ...
            return DefaultValue<CvMatrixWrapper<P, -1, 0> >::
                val();  // need a 1D default matrix ...
        }
...
template <class T>
T from_stream(std::istream& i) {
    return FromStream<T>::from(i);
}

template <class T>
int from_string(const std::string& s, T& t) {
