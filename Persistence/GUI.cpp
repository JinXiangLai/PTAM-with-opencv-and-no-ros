...
int GUI::parseArguments(const int argc, char* argv[], int start,
                        const std::string prefix,
                        const std::string execKeyword) {
...
void GUI_impl::RegisterCommand(string sCommandName, GUICallbackProc callback,
                               void* thisptr) {
    // check if command name is already registerd in the list (builtins)
    if (builtins.count(sCommandName)) {
        cerr << "!!GUI_impl::RegisterCommand: Tried to register reserved "
                "keyword "
             << sCommandName << "." << endl;
        return;
    }
...
void GUI_impl::ParseLine(string s, bool bSilentFailure) {
    ...
    {  // Brace expansion wrapper follows:
        ...
        string::size_type nCloseBracePos =
            FindCloseBrace(s, nOpenBracePos, '{', '}');
        if ((nOpenBracePos != s.npos) && (nCloseBracePos != s.npos) &&
            (nCloseBracePos > nOpenBracePos)) {  // Brace Pair found!!
            ...
            string::size_type nLength =
                sVarName
                    .size();  // Check if it's a double brace: {{foo}} in which case remove one pair, but don't expand.
            ...
            if (!bIsDoubleQuoted) {
                ...
                vector<string> vsExpandedList =
                    ChopAndUnquoteString(PV3::get_var(sVarName));
                ...
                while ((dollarpos = sEnd.find("$", dollarpos + 1)) != s.npos) {
                    ...
                    chopped.push_back(
                        sEnd.substr(lastpos, dollarpos - lastpos));
                    lastpos = dollarpos + 1;
                }
...
void builtin_shell(void* /* ptr */, string /* sCommand */, string sParams) {
    ...
    if (v.size() == 0) {
        cerr << "? GUI_impl internal shell command: No prog/args given."
             << endl;
        return;
    }
...
void builtin_if(void* ptr, string /*sCommand*/, string sParams) {
    ...
    vector<string> v = ChopAndUnquoteString(sParams);
    if (v.size() < 2) {
        cerr << "? GUI_impl internal if command: not enough params (syntax: if "
                "gvar command)"
             << endl;
        return;
    }
...
void builtin_ifnot(void* ptr, string /*sCommand*/, string sParams) {
    ...
    vector<string> v = ChopAndUnquoteString(sParams);
    if (v.size() < 2) {
        cerr << "? GUI_impl internal ifnot command: not enough params (syntax: "
                "if gvar command)"
             << endl;
        return;
    }
...
void builtin_ifeq(void* ptr, string /*sCommand*/, string sParams) {
    ...
    vector<string> v = ChopAndUnquoteString(sParams);
    if (v.size() < 3) {
        cerr << "? GUI_impl internal ifeq command: not enough params (syntax: "
                "if gvar command)"
             << endl;
        return;
    }
...
void builtin_toggle(void* /*ptr*/, string /*sCommand*/, string sParams) {
    vector<string> v = ChopAndUnquoteString(sParams);
    if (v.size() != 1) {
        cout << "? GUI_impl internal toggle command: invalid num of params "
                "(syntax: toggle gvar)"
             << endl;
        return;
    };
...
void builtin_gvarlist(void* /*ptr*/, string sCommand, string sParams) {
    ...
    if (error)
        cout << "? GUI_impl internal " << sCommand << ": syntax is " << sCommand
             << " [-a] [pattern] " << endl;
    else
...
void builtin_commandlist(void* ptr, string /*sCommand*/, string /*sParams*/) {
    ...
    for (set<string>::iterator i = p->builtins.begin(); i != p->builtins.end();
         i++)
        cout << "    " << *i << endl;
...
void builtin_queue(void* ptr, string /*sCommand*/, string sParams) {
    ...
    if (vs.size() < 2) {
        cout << "? GUI_impl Internal queue command syntax: queue queue-name "
                "line-to-enqueue"
             << endl;
        return;
    }
...
void builtin_runqueue(void* ptr, string sCommand, string sParams) {
    ...
    vector<string> vs = ChopAndUnquoteString(sParams);
    if (vs.size() != 1) {
        cout << "? GUI_impl Internal " << sCommand
             << " command syntax: runqueue queue-name " << endl;
        ...
        if (nQueues > 0) {
            ...
            for (map<string, vector<string> >::iterator it =
                     pGUI->mmQueues.begin();
                 it != pGUI->mmQueues.end(); it++)
                cout << ((it == pGUI->mmQueues.begin()) ? "" : ", ")
                     << it->first;
            cout << endl;
        }
        return;
    ...
    if (sCommand == "runqueue")
        vQueue
            .clear();  // do not clear the queue if the command was runqueue_noclear!
}

int GUI_impl::parseArguments(const int argc, char* argv[], int start,
                             const string prefix, const string execKeyword) {
    while (start < argc) {
...
void builtin_save_history(void* /*ptr*/, string /*sCommand*/, string sParams) {
    vector<string> v = ChopAndUnquoteString(sParams);
    if (v.size() != 1)
        cout << "? GUI_impl internal savehistory command: need one param "
                "(filename)."
             << endl;
    else {
        ofstream ofs;
        ofs.open(v[0].c_str());
        if (!ofs.good())
            cout << "? GUI_impl internal savehistory command: cannot open "
                 << v[0] << " for write." << endl;
        else {
            print_history(ofs);
            ofs.close();
