...
bool PV3::set_var(string name, string val, bool silent) {
    if (registered_type_and_trait.count(name)) {
        ...
        if (!silent)
            parse_warning(e, registered_type_and_trait[name].first->name(),
                          name, val);
        return e == 0;
    } else {
...
void PV3::print_var_list(ostream& o, string pattern, bool show_all) {
    ...
    if (show_all)
        o << "//Registered persistent variables:" << endl;
    ...
    for (map<string, std::pair<BaseMap*, int> >::iterator i =
        ...
        if (show_all || !(i->second.second & HIDDEN))
            if (no_pattern ||
                !fnmatch(pattern.c_str(), i->first.c_str(), FNM_CASEFOLD))
                o << i->first << "="
...
void parse_warning(int e, string type, string name, string from) {
    if (e > 0)
        std::cerr << "! PV3:Parse error setting " << type << " " << name
                  << " from " << from << std::endl;
    else if (e < 0)
        std::cerr << "! PV3:Parse warning setting " << type << " " << name
                  << " from " << from << ": "
