...
struct DefaultValue {
    ...
    typedef C
        Type;  // return the type 'cause we need to pass it to Serialization functions
...
struct IsAwkward<cv::Vec<P, Sz> > {
    static const int is = 0;
};
// but size -1, 0, 1 is...
template <typename P>
struct IsAwkward<cv::Vec<P, -1> > {
...
struct CvMatrixWrapper;

// use this as template instantiation for Serialize functions};

// by default it is not awkward,
template <typename P, int R, int C>
struct IsAwkward<CvMatrixWrapper<P, R, C> > {
    static const int is = 0;
};
// but other confgurations are...
template <typename P, int R>
struct IsAwkward<CvMatrixWrapper<P, R, 0> > {
    static const int is = 1;
};
template <typename P, int R>
struct IsAwkward<CvMatrixWrapper<P, R, -1> > {
    static const int is = 1;
};
template <typename P, int C>
struct IsAwkward<CvMatrixWrapper<P, 0, C> > {
    static const int is = 1;
};
template <typename P, int C>
struct IsAwkward<CvMatrixWrapper<P, -1, C> > {
    static const int is = 1;
};

// The following specialization(s) concerns the case in whic a vector "is awkward"
template <typename P>
struct DefaultValue<cv::Vec<P, 1> > {
    // the default size
...
template <typename P>
struct DefaultValue<cv::Vec<P, 0> > {
    // the default size
    static const int dim = 2;
    // can only return >= 2 long vectors. So a 2-vector it is..
    static inline const cv::Vec<P, dim> val() { return cv::Vec<P, dim>(0, 0); }
...
template <typename P, int Sz>
struct DefaultValue<cv::Vec<P, Sz> > {
    // And the type!
...
template <typename P, int R, int C>
struct DefaultValue<CvMatrixWrapper<P, R, C> > {
    // And the type!
...
template <typename P, int R>
struct DefaultValue<CvMatrixWrapper<P, R, -1> > {
    // default is a 1x1 zero
    static inline const cv::Mat_<P> val() {
        cv::Mat_<P> ret(1, 1);
        ret(0, 0) = 0;
        return ret;
    }
...
template <typename P, int R>
struct DefaultValue<CvMatrixWrapper<P, R, 0> > {
    // default is a 1x1 zero
    static inline const cv::Mat_<P> val() {
        cv::Mat_<P> ret(1, 1);
        ret(0, 0) = 0;
...
template <typename P, int C>
struct DefaultValue<CvMatrixWrapper<P, 0, C> > {
    // default is a 1x1 zero
    static inline const cv::Mat_<P> val() {
        cv::Mat_<P> ret(1, 1);
        ret(0, 0) = 0;
...
template <typename P, int C>
struct DefaultValue<CvMatrixWrapper<P, -1, C> > {
    // default is a 1x1 zero
    static inline const cv::Mat_<P> val() {
        cv::Mat_<P> ret(1, 1);
        ret(0, 0) = 0;
...
template <class C, int PainInTheNeck = IsAwkward<C>::is>
struct ValueHolder {
...
template <class C>
struct ValueHolder<C, 1> {

    typedef typename DefaultValue<C>::Type T;

    std::unique_ptr<T>
        val;  // pointer to the object (instead of a standard variable)
    ...
    void set(const T& c) { val = std::unique_ptr<T>(new T(c)); }

    T* ptr() { return val.get(); }
};
