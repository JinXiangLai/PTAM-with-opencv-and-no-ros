...
class BaseMap {
   public:
    virtual std::string get_as_string(const std::string& name,
                                      bool precise) = 0;
    virtual int set_from_string(const std::string& name,
...
template <class T>
class pvar2  // correspondes to gvar2
{
    ValueHolder<T>* data;
   ...
   public:
    pvar2() {
        data = NULL;
    }  // Debugging comfort feature, makes unregistered vars more obvious.
    ...
    bool IsRegistered() const { return data != NULL; }
};
...
template <class T>
class pvar3 : public pvar2<T> {
    friend class PV3;

   public:
    inline pvar3(const std::string& name, const T& default_val = T(),
                 int flags = 0);
    inline pvar3(const std::string& name, const std::string& default_val,
                 int flags);
    inline pvar3(){};
};

template <>
class pvar3<std::string> : public pvar2<std::string> {
    friend class PV3;
...
class PV3 {
   private:
    ...
    class TypedMap : public BaseMap {
       ...
       private:
        ...
        ValueHolder<C>* create(const std::string& n) {
            return &(
                data.insert(std::make_pair(n, DefaultValue<C>::val()))->second);
        }
        ...
        virtual std::string get_as_string(const std::string& name,
            ...
            if (i == data.end())
                i = data.insert(std::make_pair(name, DefaultValue<C>::val()))
                        .first;

            return Serialize::to_string(i->second.get(), precise);
        }

        virtual std::string name() { return type_name<T>(); }
        ...
        typedef
            typename std::map<std::string, ValueHolder<C>>::iterator DataIter;
    };
    ...
    template <class T>
    static ValueHolder<T>* attempt_get(const std::string& name) {
        ...
        if (!d)  //Data not present in map of the correct type
        {
            ...
            if (registered_type_and_trait.count(
                    name)) {  //Yes: programmer error.

                std::string err = type_name<T>() + " " + name +
                                  ": already registered "
                                  "as type " +
                                  registered_type_and_trait[name].first->name();

                std::cerr << "PV3:Error: type mismatch while getting " << err
                          << ". Fix your code.\n";
    ...
    template <class T>
    static ValueHolder<T>* get_by_val(const std::string& name,
    ...
    template <class T>
    static ValueHolder<T>* get_by_str(const std::string& name,
    ...
    template <class T>
    static ValueHolder<T>* register_new_pvar(const std::string& name,
                                             const T& default_val, int flags) {
        ...
        if (i == unmatched_tags.end()) {
            ...
            if (flags & FATAL_IF_NOT_DEFINED) {
                std::cerr << "!!PV3::Register: " << type_name<T>() << " "
                          << name << " must be defined. Exception. "
                          << std::endl;
    ...
    static ValueHolder<cv::Vec<T, Sz>>* register_new_pvar(
        const std::string& name, const cv::Vec<T, Sz>& default_val, int flags) {

        std::map<std::string, std::string>::iterator i;

        i = unmatched_tags.find(name);

        ValueHolder<cv::Vec<T, Sz>>*
            d;  // incredibly, this should be valid (but in the case of matrices we need to use the wrapper...)

        registered_type_and_trait[name] = std::pair<BaseMap*, int>(
            &TypedMap<cv::Vec<T, Sz>>::instance(), flags);

        //Look to see if "name" has not already been set
        if (i == unmatched_tags.end()) {

            if (flags & FATAL_IF_NOT_DEFINED) {
                std::cerr << "!!PV3::Register: " << type_name<cv::Vec<T, Sz>>()
                          << " " << name << " must be defined. Exception. "
                          << std::endl;
                throw pvar_was_not_defined(name);
            };
    ...
    static ValueHolder<CvMatrixWrapper<T, R, C>>* register_new_pvar(
        const std::string& name, const cv::Mat_<T>& default_val, int flags) {

        std::map<std::string, std::string>::iterator i;

        i = unmatched_tags.find(name);

        ValueHolder<CvMatrixWrapper<T, R, C>>*
            d;  // I am hoping my CvMatrixWrapper alchemy won't get stuck in some dark corner of the template side of the force....

        registered_type_and_trait[name] = std::pair<BaseMap*, int>(
            &TypedMap<CvMatrixWrapper<T, R, C>>::instance(), flags);

        //Look to see if "name" has not already been set
        if (i == unmatched_tags.end()) {

            if (flags & FATAL_IF_NOT_DEFINED) {
                std::cerr << "!!PV3::Register: "
                          << type_name<CvMatrixWrapper<T, R, C>>() << " "
                          << name << " must be defined. Exception. "
                          << std::endl;
                throw pvar_was_not_defined(name);
            };
            ...
            parse_warning(e, type_name<CvMatrixWrapper<T, R, C>>(), name,
                          i->second);
            if (e > 0 && flags & FATAL_IF_NOT_DEFINED) {
                std::cerr << "!!PV3::Register: "
                          << type_name<CvMatrixWrapper<T, R, C>>() << " "
   ...
   public:
    ...
    static T& get(const std::string& name,
                  const T& default_val = Persistence::DefaultValue<T>::val(),
                  int flags = 0) {
    ...
    template <class T>
    static T& get(const std::string& name, const std::string& default_val,
                  int flags = 0) {
    ...
    static void Register(
        pvar2<T>& pv, const std::string& name,
        const T& default_val = Persistence::DefaultValue<T>::val(),
        int flags = 0) {
    ...
    template <class T>
    static void Register(pvar2<T>& pv, const std::string& name,
                         const std::string& default_val, int flags = 0) {

        pv.data = get_by_str<T>(name, default_val, flags);
    }

    static inline void Register(pvar2<std::string>& pv, const std::string& name,
                                const std::string& default_val, int flags = 0) {
    ...
    static void print_var_list(std::ostream& o, std::string pattern = "",
                               bool show_all = true);
...
template <class T>
pvar3<T>::pvar3(const std::string& name, const T& default_val, int flags) {
    PV3::Register(*this, name, default_val, flags);
}

template <class T>
pvar3<T>::pvar3(const std::string& name, const std::string& default_val,
                int flags) {
    PV3::Register(*this, name, default_val, flags);
}
pvar3<std::string>::pvar3(const std::string& name,
...
template <class T>
inline void robust_set_var(std::string& name, const T& value) {
