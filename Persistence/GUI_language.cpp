...
class ThreadLocal {
   private:
    ...
    static void deleter(void* v) { delete static_cast<C*>(v); }
   ...
   public:
    ...
    C& operator()() { return *static_cast<C*>(pthread_getspecific(key)); }
};
...
class MutexMap {
   ...
   public:
    MutexMap() { pthread_mutex_init(&mutex, 0); }

    ~MutexMap() { pthread_mutex_destroy(&mutex); }
...
class GUI_language {
   ...
   private:
    ...
    static GUI_language& C(void* v) { return *static_cast<GUI_language*>(v); }

#define CallBack(X) \
    static void X##CB(void* t, string a, string b) { C(t).X(a, b); }

    CallBack(collect_line);
    void collect_line(string, string l) { collection().push_back(l); }
    ...
    CallBack(endfunction) void endfunction(string name, string args) {
        if (current_function() == "") {
    ...
    CallBack(runfunc) void runfunc(string name, string /*args*/) {
        vector<string> v = functions.get(name);
        for (unsigned int i = 0; i < v.size(); i++)
            GUI.ParseLine(v[i]);
    }

    CallBack(gui_if_equal) void gui_if_equal(string name, string args) {
        vector<string> vs = ChopAndUnquoteString(args);
        if (vs.size() != 2) {
            cerr << "Error: " << name << " takes 2 arguments: " << name
                 << " gvar string\n";
            return;
        }
    ...
    CallBack(gui_if_else) void gui_if_else(string /*name*/, string /*args*/) {
        ifbit() = collection();
        if (ifbit().empty())
            ifbit().push_back("");
        collection().clear();
    ...
    CallBack(gui_endif) void gui_endif(string /*name*/, string /*args*/) {
        if (ifbit().empty())
            ifbit() = collection();
        else
            elsebit() = collection();
