...
GLWindowMenu::GLWindowMenu(string sName, string sTitle) {
    ...
    PV3.Register(mgvnMenuItemWidth, msName + ".MenuItemWidth", 90,
                 HIDDEN | SILENT);
    PV3.Register(mgvnMenuTextOffset, msName + ".MenuTextOffset", 20,
                 HIDDEN | SILENT);
...
void GLWindowMenu::GUICommandCallBack(void* ptr, string sCommand,
                                      string sParams) {
...
void GLWindowMenu::GUICommandHandler(string sCommand, string sParams) {
    ...
    if (sCommand == msName + ".AddMenuButton") {
        if (vs.size() < 3) {
            cout << "? GLWindowMenu.AddMenuButton: Need 3/4 params: Target "
                    "Menu, Name, Command , NextMenu=\"\""
                 << endl;
            return;
        };
        MenuItem m;
    ...
    if (sCommand == msName + ".AddMenuToggle") {
        if (vs.size() < 3) {
            cout << "? GLWindowMenu.AddMenuToggle: Need 3/4 params: Target "
                    "Menu, Name, gvar_int name , NextMenu=\"\""
                 << endl;
            return;
        };
        MenuItem m;
    ...
    if (sCommand == msName + ".AddMenuMonitor") {
        if (vs.size() < 3) {
            cout << "? GLWindowMenu.AddMenuMonitor: Need 3/4 params: Target "
                    "Menu, Name, gvar name , NextMenu=\"\""
                 << endl;
            return;
        };
        MenuItem m;
    ...
    if (sCommand == msName + ".AddMenuSlider") {
        if (vs.size() < 5) {
            cout << "? GLWindowMenu.AddMenuSlider: Need 5/6 params: Target "
                    "Menu, Name, gvar_int name, min, max, NextMenu=\"\""
                 << endl;
            return;
        };
        MenuItem m;
...
void GLWindowMenu::Render(int nTop, int nHeight, int nWidth, GLWindow2& glw) {
        ...
        switch (i->type) {
            case Button:
                ...
                LineBox(nBase, nBase + *mgvnMenuItemWidth + 1, mnMenuTop,
                        mnMenuTop + mnMenuHeight);
                glw.PrintString(
                    cv::Point2i(nBase + 3, mnMenuTop + *mgvnMenuTextOffset),
                    i->sName);
                break;
            ...
            case Slider: {
                ...
                double dFrac =
                    (double)(*(i->gvnIntValue) - i->min) / (i->max - i->min);
                if (dFrac < 0.0)
                    dFrac = 0.0;
                if (dFrac > 1.0)
                    dFrac = 1.0;
    ...
    LineBox(mnWidth - *mgvnMenuItemWidth, mnWidth - 1, mnMenuTop,
            mnMenuTop + mnMenuHeight);
    cv::Point2i ir(mnWidth - *mgvnMenuItemWidth + 5,
        ...
        case Slider: {
            if (nMouseButton == GLWindow::BUTTON_WHEEL_UP) {
                ...
                *(SelectedItem.gvnIntValue) -= 1;
                ...
                int nPos =
                    *mgvnMenuItemWidth - ((mnWidth - x) % *mgvnMenuItemWidth);
