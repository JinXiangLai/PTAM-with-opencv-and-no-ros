...
void EyeGame::DrawStuff(cv::Vec3f v3CameraPos) {
    ...
    for (int i = 0; i < 4; i++) {
        if (mnFrameCounter < 100) {
            cv::Vec<float, 3> v3Look(
                (i < 2 ? -1 : 1) * (mnFrameCounter < 50 ? -1 : 1) * -0.4, -0.1,
                1);
...
void EyeGame::LookAt(int nEye, cv::Vec3f v3, double dRotLimit) {
    ...
    cv::Matx<float, 1, 3> r0t =
        CvUtils::normalize(m3Rot.row(0) - (dot13 * m3Rot.row(2)));
    // assign the result
    m3Rot(0, 0) = r0t(0, 0);
    m3Rot(0, 1) = r0t(0, 1);
    m3Rot(0, 2) = r0t(0, 2);
...
void EyeGame::MakeShadowTex() {
    ...
    GLXInterface::glTexSubImage2DGRAY(
        imShadow);  // n.b. the GLXInterface function uses GL_LUMINANCE instead ogf GL_ALPHA; we'll see...
