...
template <typename P>
inline bool EulerAnglesZYX(const cv::Matx<P, 3, 3>& R, P& thetax, P& thetay,
                           P& thetaz) {
    if (fabs(cv::determinant(R) - 1) < 0.00001)
        return false;

    P R11 = R(0, 0), R12 = R(0, 1), R13 = R(0, 2), R21 = R(1, 0), R22 = R(1, 1),
      R23 = R(1, 2), R31 = R(2, 0), R32 = R(2, 1), R33 = R(2, 2);
...
void halfSample(const cv::Mat_<T>& src, cv::Mat_<T>& dest) {
    ...
    for (r = 0; r < rows; r++) {
        ...
        for (c = 0; c < cols; c++)
            dRowPtr[c] = (sRowPtr00[c * 2] + sRowPtr10[c * 2] +
                          sRowPtr00[c * 2 + 1] + sRowPtr10[c * 2 + 1]) /
                         4;
    }
}
...
inline bool in_image_with_border(cv::Point2i pos, cv::Mat im,
                                 cv::Size2i border) {
    return (pos.x - border.width >= 0) && (pos.y - border.height >= 0) &&
           (pos.x + border.width < im.cols) &&
           (pos.y + border.height < im.rows);
};

inline bool in_image_with_border(int row, int col, cv::Mat im, int bwidth,
                                 int bheight) {
    return (col - bwidth >= 0) && (row - bheight >= 0) &&
           (col + bwidth < im.cols) && (row + bheight < im.rows);
};
...
inline void M3Symm_UT_Inverse(const T (&V)[9], T (&Vinv)[9]) {
    ...
    T a = V[0], b = V[1], c = V[2], d = V[1], e = V[4], f = V[5], g = V[2],
      h = V[5], i = V[8];
    ...
    Vinv[0] = (e * i - f * h) * invDet;
    Vinv[1] = (h * c - i * b) * invDet;
    Vinv[2] = (b * f - c * e) * invDet;
    Vinv[3] = Vinv[1];
    Vinv[4] = (a * i - g * c) * invDet;
    Vinv[5] = (d * c - a * f) * invDet;
    Vinv[6] = Vinv[2];
    Vinv[7] = Vinv[5];
    Vinv[8] = (a * e - d * b) * invDet;
}
...
inline void M3Symm_LT_Inverse(const T (&V)[9], T (&Vinv)[9]) {
    // 1. Get the elements of the matrix
    T a = V[0], b = V[3], c = V[6], d = V[3], e = V[4], f = V[7], g = V[6],
      h = V[7], i = V[8];
...
inline bool M3PSD_LT_Cholesky(const T (&M)[9], T (&L)[9]) {
    ...
    if (M[8] - L32 * L32 - L31 * L31 < 0)
        return false;  // leave the 0-case in to tell us if the matrix is singular
    ...
    const Tout defaultValue = Tout())  // default value for boundary pixels
{
    ...
    const cv::Vec<P, 2> p1(p0[0] + w * across[0], p0[1] + w * across[1]);
    const cv::Vec<P, 2> p2(p0[0] + h * down[0], p0[1] + h * down[1]);
...
inline void sample(
    ...
    assert(im.channels() == 1 &&
           "No more channels in sample() for the time being... Thank you!");
...
inline T M3Symm_LT_Det(const T (&V)[9]) {
    ...
    // 2. Determinant
    return a * e * i + b * f * g + c * d * h - g * e * c - h * f * a -
           i * d * b;
}
