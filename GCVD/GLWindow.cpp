...
GLXInterface::Exceptions::GLWindow::CreationError::CreationError(
    std::string w) {
...
void GLXInterface::GLWindow::init(const cv::Size2i& size, int bpp,
    ...
    XVisualInfo* visualInfo =
        glXChooseVisual(display, DefaultScreen(display), visualAttributes);
    ...
    attributes.colormap =
        XCreateColormap(display, rootWindow, visualInfo->visual, AllocNone);
    attributes.event_mask = KeyPressMask | KeyReleaseMask | ButtonPressMask |
                            ButtonReleaseMask | PointerMotionMask |
                            StructureNotifyMask | ExposureMask;

    Window window =
        XCreateWindow(display, rootWindow, 0, 0, size.width, size.height, 0,
    ...
    glRasterPos2f(-1, 1);
    glOrtho(
        -0.375, size.width - 0.375, size.height - 0.375, -0.375, -1,
        1);  //offsets to make (0,0) the top left pixel (rather than off the display)
    glPixelZoom(1, -1);

    XColor black = {0, 0, 0, 0, 0, 0};
    XFontStruct* fixed =
        XLoadQueryFont(display, "-misc-fixed-medium-r-*-*-12-*-*-*-*-*-*-1");
...
inline int convertButton(unsigned int button) {
    switch (button) {
        case Button1:
            return GLXInterface::GLWindow::BUTTON_LEFT;
        case Button2:
            return GLXInterface::GLWindow::BUTTON_MIDDLE;
        case Button3:
            return GLXInterface::GLWindow::BUTTON_RIGHT;
        case Button4:
            return GLXInterface::GLWindow::BUTTON_WHEEL_UP;
        case Button5:
            return GLXInterface::GLWindow::BUTTON_WHEEL_DOWN;
    }
    return 0;
}
...
inline int convertButtonState(unsigned int state) {
    ...
    if (state & Button1Mask)
        ret |= GLXInterface::GLWindow::BUTTON_LEFT;
    if (state & Button2Mask)
        ret |= GLXInterface::GLWindow::BUTTON_MIDDLE;
    if (state & Button3Mask)
        ret |= GLXInterface::GLWindow::BUTTON_RIGHT;
    if (state & ControlMask)
        ret |= GLXInterface::GLWindow::BUTTON_MOD_CTRL;
    if (state & ShiftMask)
        ret |= GLXInterface::GLWindow::BUTTON_MOD_SHIFT;
    return ret;
}
        ...
        switch (event.type) {
            case ButtonPress:
                handler.on_mouse_down(
                    *this, cv::Point2i(event.xbutton.x, event.xbutton.y),
            ...
            case MotionNotify:
                handler.on_mouse_move(
                    *this, cv::Point2i(event.xmotion.x, event.xmotion.y),
                    convertButtonState(event.xbutton.state));
                break;
            case KeyPress: {
            ...
            case ConfigureNotify:
                if (event.xconfigure.width != state->size.width ||
                    event.xconfigure.height != state->size.height) {
                    activate();
                    state->size = cv::Size2i(event.xconfigure.width,
...
class SaveEvents : public GLXInterface::GLWindow::EventHandler {
   ...
   public:
    SaveEvents(std::vector<GLXInterface::GLWindow::Event>& events_)
        : events(events_) {}
    ...
    void on_mouse_down(GLXInterface::GLWindow&, cv::Point2i where, int state,
                       int button) {
        GLXInterface::GLWindow::Event e;
    ...
    void on_mouse_up(GLXInterface::GLWindow&, cv::Point2i where, int state,
                     int button) {
        GLXInterface::GLWindow::Event e;
...
bool GLXInterface::GLWindow::EventSummary::should_quit() const {
    return key_down.count(XK_Escape) ||
           events.count(GLXInterface::GLWindow::EVENT_CLOSE);
}
...
class MakeSummary : public GLXInterface::GLWindow::EventHandler {
   ...
   public:
    MakeSummary(GLXInterface::GLWindow::EventSummary& summary_)
        : summary(summary_) {}
    ...
    void on_mouse_move(GLXInterface::GLWindow&, cv::Point2i where, int) {
