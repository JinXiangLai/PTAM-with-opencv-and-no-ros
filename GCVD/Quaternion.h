...
class Quaternion {
   ...
   private:
    ...
    struct QMulType {
        typedef decltype(_gettype<L>() * _gettype<R>()) type;
    };
   ...
   protected:
    P s, v1, v2,
        v3;  // Adopting a generalized complex number notation: q = s + [v1 v2 v3] .* [i j k]
   ...
   public:
    ...
    template <typename Ps, typename Pv1, typename Pv2, typename Pv3>
    inline Quaternion(Ps s_, Pv1 v1_, Pv2 v2_, Pv3 v3_)
        : s(s_), v1(v1_), v2(v2_), v3(v3_) {}
    ...
    inline Quaternion(const Pr* R) {
        *this = Rotation2Quaternion(R);
    }
    ...
    inline void RotationMatrix(Pr* R) {
        ...
        R[0] = s * s + v1 * v1 - v2 * v2 - v3 * v3;
        R[1] = 2 * (v1 * v2 - s * v3);
        R[2] = 2 * (v1 * v3 + s * v2);
        R[3] = 2 * (v1 * v2 + s * v3);
        R[4] = s * s - v1 * v1 + v2 * v2 - v3 * v3;
        R[5] = 2 * (v2 * v3 - s * v1);
        R[6] = 2 * (v1 * v3 - s * v2);
        R[7] = 2 * (v2 * v3 + s * v1);
        R[8] = s * s - v1 * v1 - v2 * v2 + v3 * v3;
    }

    /// Quaternion multiplication
    template <typename Pr>
    inline Quaternion<typename QMulType<P, Pr>::type> Mul(
        const Quaternion<Pr>& rhs) const {

        Pr r = rhs.get_s(), z1 = rhs.get_v1(), z2 = rhs.get_v2(),
           z3 = rhs.get_v3();
    ...
    inline Quaternion<typename QMulType<P, Ps>::type> Muls(Ps lscalar) const {
        ...
        return Quaternion<retType>(lscalar * get_s(), lscalar * get_v1(),
                                   lscalar * get_v2(), lscalar * get_v3());
    }
    ...
    P& operator[](int index) {
        ...
        assert(index > -1 && index < 4 &&
               "Invalid indexing to quaternion component");
    ...
    inline typename QMulType<P, Pw>::type Dot(const Quaternion<Pw>& w) const {
        ...
        Pw r = w.get_s(), z1 = w.get_v1(), z2 = w.get_v2(), z3 = w.get_v3();
    ...
    template <typename Pu>
    inline typename QMulType<P, Pu>::type dot(const Pu u1, const Pu u2,
                                              const Pu u3, const Pu u4) {
    ...
    template <typename Pu>
    inline static Quaternion exp(const Pu* u);
    ...
    template <typename Ppsi>
    inline void MRPCoordinates(Ppsi* psi) const;
    template <typename Ppsi1, typename Ppsi2, typename Ppsi3>
    ...
    template <typename Pg>
    inline void GibbsVector(Pg* g) const;
    template <typename Pg1, typename Pg2, typename Pg3>
    ...
    P RotationJacWRTquat(int row, int col, int i) {
        ...
        assert(i > -1 && i < 5 && row > -1 && row < 4 && col > -1 && col < 4 &&
               "Quaternion or Rotation matrix index out-of-bounds");
        ...
        switch (i) {
            case 0: /* dRdqi(0, 0) = 2 * q0;    dRdqi(0, 1) = 2 * (-q3);  dRdqi(0, 2) = 2 * q2;
		      ...
		      */
                ...
                if (row == col)
                    ...
                    return (col == 1) ? -2 * v3 : 2 * v3;
                else if (row + col == 2)
                    return (col == 0) ? -2 * v2 : 2 * v2;
                else if (row + col == 3)
                    return (col == 2) ? -2 * v1 : 2 * v1;
            ...
            case 1:
		     ...
		     dRdqi(2, 0) = 2 * q3;    dRdqi(2, 1) = 2 * q0;     dRdqi(2, 2) = 2 * (-q1); */
                ...
                if (row == col)
                    return (col == 0) ? 2 * v1 : -2 * v1;
                else if (row + col == 1)
                    return 2 * v2;
                else if (row + col == 2)
                    return 2 * v3;
                else if (row + col == 3)
                    return (col == 2) ? -2 * s : 2 * s;
            ...
            case 3: /* dRdqi(0, 0) = 2 * (-q3); dRdqi(0, 1) = 2 * (-q0);  dRdqi(0, 2) = 2 * q1;
		     ...
		     dRdqi(2, 0) = 2 * q1;    dRdqi(2, 1) = 2 * q2;     dRdqi(2, 2) = 2 * q3; */
                ...
                if (row == col)
                    return (col == 2) ? 2 * v3 : -2 * v3;
                else if (row + col == 1)
                    return (col == 1) ? -2 * s : 2 * s;
                else if (row + col == 2)
                    return 2 * v1;
                else if (row + col == 3)
                    return 2 * v2;
    ...
    inline void QuaternionVectorJacWRTAA(Pd* J) {
        ...
        if (theta < 10e-5) {
            ...
            // Column #1
            J[0] = inv_theta3 * (theta * theta * sin(theta_h) +
                                 0.5 * u1 * u1 * theta * cos(theta_h) -
                                 u1 * u1 * sin(theta_h));
            J[3] = inv_theta3 * u1 * u2 *
                   (0.5 * theta * cos(theta_h) - sin(theta_h));
            J[6] = inv_theta3 * u1 * u3 *
                   (0.5 * theta * cos(theta_h) - sin(theta_h));

            // Column #2
            J[1] = inv_theta3 * u1 * u2 *
                   (0.5 * theta * cos(theta_h) - sin(theta_h));
            J[4] = inv_theta3 * (theta * theta * sin(theta_h) +
                                 0.5 * u2 * u2 * theta * cos(theta_h) -
                                 u2 * u2 * sin(theta_h));
            J[7] = inv_theta3 * u2 * u3 *
                   (0.5 * theta * cos(theta_h) - sin(theta_h));

            // Column #3
            J[2] = inv_theta3 * u1 * u3 *
                   (0.5 * theta * cos(theta_h) - sin(theta_h));
            J[5] = inv_theta3 * u2 * u3 *
                   (0.5 * theta * cos(theta_h) - sin(theta_h));
            J[8] = inv_theta3 * (theta * theta * sin(theta_h) +
                                 0.5 * u3 * u3 * theta * cos(theta_h) -
                                 u3 * u3 * sin(theta_h));
        }
    }
    ...
    inline void RotatedVectorJacWRTMRPs(const Pu* u, Pd* D) const {
        ...
        D[0] =
            2 * v1 * (-v1 * v1 + v2 * v2 + v3 * v3 + 1 - s * s) * u[0] +
            2 * (v2 * (-2 * v1 * v1 + s + 1) + v1 * v3 * (2 * s + 1)) * u[1] +
            2 * (v3 * (-2 * v1 * v1 + s + 1) - v1 * v2 * (2 * s + 1)) * u[2];

        D[3] =
            2 * (v2 * (-2 * v1 * v1 + s + 1) - v1 * v3 * (2 * s + 1)) * u[0] +
            2 * v1 * (-v2 * v2 + v1 * v1 + v3 * v3 - (s + 1) * (s + 1)) * u[1] +
            2 * (-s * (-2 * v1 * v1 + s + 1) + v1 * (v1 - 2 * v2 * v3)) * u[2];

        D[6] =
            2 * (v3 * (-2 * v1 * v1 + s + 1) + v1 * v2 * (2 * s + 1)) * u[0] +
            2 * (s * (-2 * v1 * v1 + s + 1) - v1 * (v1 + 2 * v2 * v3)) * u[1] +
            2 * v1 * (-v3 * v3 + v1 * v1 + v2 * v2 - (1 + s) * (1 + s)) * u[2];
        ...
        //  ( DRDq0 * Dq0Dpsi1 + DRDq1 * Dq1Dpsi1 + DRDq2 * Dq2Dpsi1 + DRDq3 * Dq3Dpsi1 ) * u
        D[1] =
            2 * v2 * (-v1 * v1 + v2 * v2 + v3 * v3 - (1 + s) * (1 + s)) * u[0] +
            2 * (v1 * (-2 * v2 * v2 + s + 1) + v2 * v3 * (2 * s + 1)) * u[1] +
            2 * (s * (-2 * v2 * v2 + s + 1) - v2 * (v2 + 2 * v1 * v3)) * u[2];

        D[4] =
            2 * (v1 * (-2 * v2 * v2 + s + 1) - v2 * v3 * (2 * s + 1)) * u[0] +
            2 * v2 * (-v2 * v2 + v1 * v1 + v3 * v3 + 1 - s * s) * u[1] +
            2 * (v3 * (-2 * v2 * v2 + s + 1) + v1 * v2 * (2 * s + 1)) * u[2];

        D[7] =
            2 * (-s * (-2 * v2 * v2 + s + 1) + v2 * (v2 - 2 * v1 * v3)) * u[0] +
            2 * (v3 * (-2 * v2 * v2 + s + 1) - v1 * v2 * (2 * s + 1)) * u[1] +
            2 * v2 * (-v3 * v3 + v1 * v1 + v2 * v2 - (s + 1) * (s + 1)) * u[2];
        ...
        D[2] =
            2 * v3 * (-v1 * v1 + v2 * v2 + v3 * v3 - (1 + s) * (1 + s)) * u[0] +
            2 * (-s * (-2 * v3 * v3 + s + 1) + v3 * (v3 - 2 * v1 * v2)) * u[1] +
            2 * (v1 * (-2 * v3 * v3 + s + 1) - v2 * v3 * (2 * s + 1)) * u[2];

        D[5] =
            2 * (s * (-2 * v3 * v3 + s + 1) - v3 * (v3 + 2 * v1 * v2)) * u[0] +
            2 * v3 * (-v2 * v2 + v1 * v1 + v3 * v3 - (1 + s) * (1 + s)) * u[1] +
            2 * (v2 * (-2 * v3 * v3 + s + 1) + v1 * v3 * (2 * s + 1)) * u[2];

        D[8] =
            2 * (v1 * (-2 * v3 * v3 + s + 1) + v2 * v3 * (2 * s + 1)) * u[0] +
            2 * (v2 * (-2 * v3 * v3 + s + 1) - v1 * v3 * (2 * s + 1)) * u[1] +
            2 * (v3 * (-v3 * v3 + v1 * v1 + v2 * v2 + 1 - s * s)) * u[2];
    }
    ...
    inline void RotationJacWRTMRPs(unsigned int i, Pd* D) const {
        ...
        assert(i < 3 &&
               "Invalid stereographic coordinate index (must be 0, 1, 2)");
        ...
        switch (i) {
            case 0:  // [dR / dpsi0]
                ...
                D[1] =
                    2 * (v2 * (-2 * v1 * v1 + s + 1) + v1 * v3 * (2 * s + 1));
                D[2] =
                    2 * (v3 * (-2 * v1 * v1 + s + 1) - v1 * v2 * (2 * s + 1));

                // Row #2
                D[3] =
                    2 * (v2 * (-2 * v1 * v1 + s + 1) - v1 * v3 * (2 * s + 1));
                D[4] =
                    2 * v1 * (-v2 * v2 + v1 * v1 + v3 * v3 - (s + 1) * (s + 1));
                D[5] =
                    2 * (-s * (-2 * v1 * v1 + s + 1) + v1 * (v1 - 2 * v2 * v3));
                ...
                D[7] =
                    2 * (s * (-2 * v1 * v1 + s + 1) - v1 * (v1 + 2 * v2 * v3));
                D[8] =
                    2 * v1 * (-v3 * v3 + v1 * v1 + v2 * v2 - (1 + s) * (1 + s));
            ...
            case 1:  // [dR / dpsi1]
                ...
                // Row #1
                D[0] =
                    2 * v2 * (-v1 * v1 + v2 * v2 + v3 * v3 - (1 + s) * (1 + s));
                D[1] =
                    2 * (v1 * (-2 * v2 * v2 + s + 1) + v2 * v3 * (2 * s + 1));
                D[2] =
                    2 * (s * (-2 * v2 * v2 + s + 1) - v2 * (v2 + 2 * v1 * v3));
                ...
                D[7] =
                    2 * (v3 * (-2 * v2 * v2 + s + 1) - v1 * v2 * (2 * s + 1));
                D[8] =
                    2 * v2 * (-v3 * v3 + v1 * v1 + v2 * v2 - (s + 1) * (s + 1));
            ...
            case 2:  // [dR / dpsi2]
                ...
                // Row #1
                D[0] =
                    2 * v3 * (-v1 * v1 + v2 * v2 + v3 * v3 - (1 + s) * (1 + s));
                D[1] =
                    2 * (-s * (-2 * v3 * v3 + s + 1) + v3 * (v3 - 2 * v1 * v2));
                D[2] =
                    2 * (v1 * (-2 * v3 * v3 + s + 1) - v2 * v3 * (2 * s + 1));

                // Row #2
                D[3] =
                    2 * (s * (-2 * v3 * v3 + s + 1) - v3 * (v3 + 2 * v1 * v2));
                D[4] =
                    2 * v3 * (-v2 * v2 + v1 * v1 + v3 * v3 - (1 + s) * (1 + s));
                D[5] =
                    2 * (v2 * (-2 * v3 * v3 + s + 1) + v1 * v3 * (2 * s + 1));
    ...
    template <typename Pd>
    inline void RotationElementJacWRTMRPs(unsigned int r, unsigned int c,
                                          Pd* dr) {
        ...
        switch (r) {
            case 0:
                ...
                switch (c) {
                    case 0:  // [dr11 / dψ]
                        dr[0] =
                            2 * v1 * (-v1 * v1 + v2 * v2 + v3 * v3 + 1 - s * s);
                        dr[1] =
                            2 * v2 *
                            (-v1 * v1 + v2 * v2 + v3 * v3 - (1 + s) * (1 + s));
                        dr[2] =
                            2 * v3 *
                            (-v1 * v1 + v2 * v2 + v3 * v3 - (1 + s) * (1 + s));
                        break;
                    ...
                    case 1:  // [dr12 / dψ]
                        dr[0] = 2 * (v2 * (-2 * v1 * v1 + s + 1) +
                                     v1 * v3 * (2 * s + 1));
                        dr[1] = 2 * (v1 * (-2 * v2 * v2 + s + 1) +
                                     v2 * v3 * (2 * s + 1));
                        dr[2] = 2 * (-s * (-2 * v3 * v3 + s + 1) +
                                     v3 * (v3 - 2 * v1 * v2));
                        break;

                    case 2:  // [dr13 / dψ]
                        dr[0] = 2 * (v3 * (-2 * v1 * v1 + s + 1) -
                                     v1 * v2 * (2 * s + 1));
                        dr[1] = 2 * (s * (-2 * v2 * v2 + s + 1) -
                                     v2 * (v2 + 2 * v1 * v3));
                        dr[2] = 2 * (v1 * (-2 * v3 * v3 + s + 1) -
                                     v2 * v3 * (2 * s + 1));
                        break;
                }
                break;
            ...
            case 1:
                ...
                switch (c) {
                    ...
                    case 1:  // [dr22 / dψ]
                        dr[0] =
                            2 * v1 *
                            (-v2 * v2 + v1 * v1 + v3 * v3 - (1 + s) * (1 + s));
                        dr[1] =
                            2 * v2 * (-v2 * v2 + v1 * v1 + v3 * v3 + 1 - s * s);
                        dr[2] =
                            2 * v3 *
                            (-v2 * v2 + v1 * v1 + v3 * v3 - (1 + s) * (1 + s));
                        break;
                    ...
                    case 2:  // [dr23 / dψ]
                        dr[0] = 2 * (-s * (-2 * v1 * v1 + s + 1) +
                                     v1 * (v1 - 2 * v2 * v3));
                        dr[1] = 2 * (v3 * (-2 * v2 * v2 + s + 1) +
                                     v1 * v2 * (2 * s + 1));
                        dr[2] = 2 * (v2 * (-2 * v3 * v3 + s + 1) +
                                     v1 * v3 * (2 * s + 1));
                        break;
                }
                break;
            ...
            case 2:
                ...
                switch (c) {
                    ...
                    case 2:  // [dr33 / dψ]
                        dr[0] =
                            2 * v1 *
                            (-v3 * v3 + v1 * v1 + v2 * v2 - (1 + s) * (1 + s));
                        dr[1] =
                            2 * v2 *
                            (-v3 * v3 + v1 * v1 + v2 * v2 - (1 + s) * (1 + s));
                        dr[2] =
                            2 * v3 * (-v3 * v3 + v1 * v1 + v2 * v2 + 1 - s * s);
                        break;
                }
                break;
        }
    ...
    /// Use this function to avoid unnecessary conversions to parameters
    template <typename Pdelta1, typename Pdelta2, typename Pdelta3>
    inline Quaternion<P> UpdateFromDeltaMRPs(Pdelta1 delta1, Pdelta2 delta2,
                                             Pdelta3 delta3) {
    ...
    inline static Quaternion<
        typename QMulType<P0, typename QMulType<P1, Pt>::type>::type>
    SLERP(const Quaternion<P0>& q0, const Quaternion<P1>& q1, Pt t) {
        // Get a better name for the return type
        typedef typename QMulType<P0, typename QMulType<P1, Pt>::type>::type
            RetType;
        ...
        if (cosPhi > -1 + epsilon) {
            ...
            return q0.Muls(sin((1 - t) * Phi) * invSinPhi) +
                   q1.Muls(sin(t * Phi) * invSinPhi);
        }
...
template <typename Precision>
template <typename Ppsi>
inline Quaternion<Precision> Quaternion<Precision>::CreateFromMRPs(
    const Ppsi* psi) {

    const Precision normsq =
        psi[0] * psi[0] + psi[1] * psi[1] + psi[2] * psi[2];
...
template <typename Precision>
template <typename Pg>
inline Quaternion<Precision> Quaternion<Precision>::CreateFromGibbs(
    const Pg* g) {
    ...
    const Precision s_ =
        1.0 / sqrt(1 + g[0] * g[0] + g[1] * g[1] + g[2] * g[2]);
    Precision v1_ = g[0] * s_, v2_ = g[1] * s_, v3_ = g[2] * s_;
...
inline Quaternion<Precision> Quaternion<Precision>::exp(const Pu* u) {
    ...
    if (theta < 0.0001) {
        return Quaternion<Precision>(cos(0.5 * theta), coeff * u[0],
                                     coeff * u[1], coeff * u[2]);
    }
...
template <typename Precision>
template <typename Pu1, typename Pu2, typename Pu3>
inline Quaternion<Precision> Quaternion<Precision>::exp(const Pu1 u1,
                                                        const Pu2 u2,
                                                        const Pu3 u3) {
    // obtaining the norm of u (i.e., the angle of the rotation)
    ...
    if (theta < 0.0001) {
        return Quaternion<Precision>(cos(0.5 * theta), coeff * u1, coeff * u2,
                                     coeff * u3);
    }

    return Quaternion<Precision>(
        cos(0.5 * theta), sin(0.5 * theta) * inv_theta * u1,
        sin(0.5 * theta) * inv_theta * u2, sin(0.5 * theta) * inv_theta * u3);
}
...
template <typename Precision>
template <typename Ppsi1, typename Ppsi2, typename Ppsi3>
inline void Quaternion<Precision>::MRPCoordinates(Ppsi1& psi1, Ppsi2& psi2,
                                                  Ppsi3& psi3) const {
...
template <typename Precision>
template <typename Pg1, typename Pg2, typename Pg3>
inline void Quaternion<Precision>::GibbsVector(Pg1& g1, Pg2& g2,
                                               Pg3& g3) const {
...
inline void Quaternion<Precision>::ln(Pu1& u1, Pu2& u2, Pu3& u3) const {
    ...
    if (fabs(theta) > 10e-5) {
        ...
        const Precision inv_apr_sin_div_theta =
            1.0 / (0.5 - theta * theta / 48.0);
...
inline Quaternion<typename Quaternion<>::QAddType<Pl, Pr>::type> operator+(
    const Quaternion<Pl>& ql, const Quaternion<Pr>& qr) {

    typedef typename Quaternion<>::QAddType<Pl, Pr>::type retType;

    return Quaternion<retType>(
        ql.get_s() + qr.get_s(), ql.get_v1() + qr.get_v1(),
        ql.get_v2() + qr.get_v2(), ql.get_v3() + qr.get_v3());
}
...
template <typename Pl, typename Pr>
inline Quaternion<typename Quaternion<>::QSubType<Pl, Pr>::type> operator-(
    const Quaternion<Pl>& lhs, const Quaternion<Pr>& rhs) {

    typedef typename Quaternion<>::QSubType<Pl, Pr>::type retType;

    return Quaternion<retType>(
        lhs.get_s() - rhs.get_s(), lhs.get_v1() - rhs.get_v1(),
        lhs.get_v2() - rhs.get_v2(), lhs.get_v3() - rhs.get_v3());
}

/// Write a quaternion to a stream
template <typename Precision>
inline std::ostream& operator<<(std::ostream& os,
                                const Quaternion<Precision>& q) {

    return os << "[" << q.get_s() << ", " << q.get_v1() << ", " << q.get_v2()
              << ", " << q.get_v3() << "]";
}
