...
template <typename Precision>
inline std::istream& operator>>(std::istream&, SO2<Precision>&);
template <typename Precision>
inline std::istream& operator>>(std::istream&, SE2<Precision>&);
...
class SO2 {
   ...
   private:
    ...
    cv::Matx<Precision, 2, 2>
        mat_;  // the 2x2 matrix containing the transformation.
   ...
   public:
    ...
    void coerce() {
        ...
        assignRowAt(
            1, CvUtils::normalize(mat_.row(1) - (mat_.row(1).dot(mat_.row(0))) *
                                                    mat_.row(0)));
    }
    ...
    cv::Matx<Precision, 2, 2>& get_matrix() {
        return mat_;
    }  // IMPORTANT OVERLOAD!!!!!!!!
...
inline std::ostream& operator<<(std::ostream& os,
                                const RigidTransforms::SO2<Precision>& right) {
...
inline std::istream& operator>>(std::istream& is,
                                RigidTransforms::SO2<Precision>& right) {
    is >> right.mat_;
...
inline RigidTransforms::SO2<
    typename MyOperatorOverloads::MultiplyType<Pl, Pr>::type>
operator*(const RigidTransforms::SO2<Pl>& left,
...
operator*(const RigidTransforms::SO2<P>& so2, const cv::Vec<PV, 2>& v) {
...
inline cv::Vec<typename MyOperatorOverloads::MultiplyType<PV, P>::type, 2>
operator*(const cv::Vec<PV, 2>& v, const RigidTransforms::SO2<P>& so2) {
...
operator*(const RigidTransforms::SO2<P>& so2, const cv::Mat_<PM>& M) {
...
inline cv::Matx<typename MyOperatorOverloads::MultiplyType<P, PM>::type, 2, C>
operator*(const RigidTransforms::SO2<P>& so2, const cv::Matx<PM, 2, C>& M) {
...
operator*(const cv::Mat_<PM> M, const RigidTransforms::SO2<P>& so2) {
...
inline cv::Matx<typename MyOperatorOverloads::MultiplyType<PM, P>::type, R, 2>
operator*(const cv::Matx<PM, R, 2> M, const RigidTransforms::SO2<P>& so2) {
