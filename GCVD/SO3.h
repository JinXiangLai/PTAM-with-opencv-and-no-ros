...
inline std::istream& operator>>(std::istream&, SO3<Precision>&);
template <class Precision>
inline std::istream& operator>>(std::istream&, SE3<Precision>&);
...
class SO3 {
   ...
   public:
    friend std::istream& operator>>
        <Precision>(std::istream& is, SO3<Precision>& rhs);
    friend std::istream& operator>>
        <Precision>(std::istream& is, SE3<Precision>& rhs);
    ...
    template <typename P>
    SO3(const cv::Vec<P, 3>& v) {
        *this = exp(v);
    }

    /// Construct from a rotation matrix.
    template <typename P>
    SO3(const cv::Matx<P, 3, 3>& r) : mat_(r) {
        coerce();
        //orthoProcrustes();
    }
    ...
    SO3(const cv::Vec<P1, 3>& a, const cv::Vec<P2, 3>& b) {
        ...
        if (cv::norm(n) == 0) {
            ...
            assert(a * b >= 0 &&
                   "Attempted to construct an SO3 from two collinear oposite "
                   "vectors!");
        ...
        R1(0, 0) = a_normalized[0];
        R1(1, 0) = a_normalized[1];
        R1(2, 0) = a_normalized[2];
        ...
        mat_(0, 0) = b_normalized[0];
        mat_(1, 0) = b_normalized[1];
        mat_(2, 0) = b_normalized[2];
    ...
    void coerce() {
        ...
        assignRowAt(0, CvUtils::normalize(mat_.row(
                           0)));  // use CvUtils::normalize for Max vectors

        // 2. Project the second on the first and normalize the difference of the second minus the projection
        assignRowAt(
            1, CvUtils::normalize(mat_.row(1) - (mat_.row(0).dot(mat_.row(1))) *
                                                    mat_.row(0)));

        // 3. Project the 3d row to the 1st and the 2nd and then take the normalized difference from the sum of the projections
        assignRowAt(
            2, CvUtils::normalize(
                   mat_.row(2) - (mat_.row(1).dot(mat_.row(2))) * mat_.row(1) -
                   (mat_.row(0).dot(mat_.row(2))) * mat_.row(0)));
        ...
        assert((mat_.row(0) ^ mat_.row(1)).dot(mat_.row(2)) > 0 &&
               "Coercing did not produce a rotation matrix, or you gave me "
               "low-rank materials!");
    }
    ...
    inline static SO3 exp(const cv::Vec<VP, 3>&);
    ...
    template <typename P>
    SO3<typename MyOperatorOverloads::MultiplyType<Precision, P>::type>
    operator*(const SO3<P>& right) const {
    ...
    inline static cv::Vec<Precision, 3> generator_field(
        int i, const cv::Vec<Precision, 3>& pos) {
    ...
    inline SO3(const SO3<PA>& a, const SO3<PB>& b)
        : mat_(a.get_matrix() * b.get_matrix()) {}
...
inline std::ostream& operator<<(std::ostream& os,
                                const RigidTransforms::SO3<Precision>& so3) {
...
inline std::istream& operator>>(std::istream& is,
                                RigidTransforms::SO3<Precision>& so3) {
...
inline static void rodrigues_so3_exp(const cv::Vec<PV, 3>& w, const Ps A,
                                     const Ps B, cv::Matx<P, 3, 3>& R) {
...
template <typename Precision>
template <typename VP>
inline RigidTransforms::SO3<Precision> RigidTransforms::SO3<Precision>::exp(
    const cv::Vec<VP, 3>& w) {
    using std::cos;
    using std::sin;
    using std::sqrt;
...
inline cv::Vec<Precision, 3> RigidTransforms::SO3<Precision>::ln() const {
    ...
    const Precision cos_angle =
        (mat_(0, 0) + mat_(1, 1) + mat_(2, 2) - 1.0) * 0.5;
...
operator*(const RigidTransforms::SO3<P>& so3, const cv::Vec<PV, 3>& v) {
...
operator*(const cv::Vec<PV, 3>& v, const RigidTransforms::SO3<P>& so3) {
...
operator*(const RigidTransforms::SO3<P>& so3, const cv::Mat_<PM>& M) {
...
operator*(const cv::Mat_<PM>& M, const RigidTransforms::SO3<P>& so3) {
...
inline cv::Matx<typename MyOperatorOverloads::MultiplyType<P, PM>::type, 3,
                Cols>
operator*(const RigidTransforms::SO3<P>& so3, const cv::Matx<PM, 3, Cols>& M) {
...
inline cv::Matx<typename MyOperatorOverloads::MultiplyType<PM, P>::type, Rows,
                3>
operator*(const cv::Matx<PM, Rows, 3>& M, const RigidTransforms::SO3<P>& so3) {
