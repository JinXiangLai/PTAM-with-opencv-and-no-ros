...
struct Add {
    ...
    struct Return {
        typedef typename AddType<P1, P2>::type Type;
    };
};
...
struct Subtract {
    ...
    template <class P1, class P2>
    struct Return {
        typedef typename SubtractType<P1, P2>::type Type;
    };
};
...
struct Multiply {
    ...
    template <class P1, class P2>
    struct Return {
        typedef typename MultiplyType<P1, P2>::type Type;
    };
};
...
struct Divide {
    ...
    template <class P1, class P2>
    struct Return {
        typedef typename DivideType<P1, P2>::type Type;
    };
};
...
template <typename Op, typename P1, int S, typename P2>
struct VElementwise;
...
template <typename Op, typename P1, int S,
          typename P2>  // Here Op is the operation (i.e., Add, Multiply, etc..)
struct Operator<VElementwise<Op, P1, S, P2> > {
    const cv::Vec<P1, S>& left;
    const cv::Vec<P2, S>& right;

    Operator(const cv::Vec<P1, S>& left_in, const cv::Vec<P2, S>& right_in)
...
template <typename P1, int S,
          typename P2>  // Here Op is specialized to DotProduct
...
struct Operator<VElementwise<DotProduct, P, S, P> > {
    ...
    Operator(const cv::Vec<P, S>& left_in, const cv::Vec<P, S>& right_in)
        : left(left_in), right(right_in) {}

    P compute() const { return left.dot(right); }
};
...
cv::Vec<typename MyOperatorOverloads::AddType<P1, P2>::type, S> operator+(
    const cv::Vec<P1, S>& v1, const cv::Vec<P2, S>& v2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::VElementwise<
        MyOperatorOverloads::Add, P1, S, P2> >(v1, v2)
        .compute();
}

// 2. Subtraction Vector - Vector operator
template <int S, typename P1, typename P2>
cv::Vec<typename MyOperatorOverloads::SubtractType<P1, P2>::type, S> operator-(
    const cv::Vec<P1, S>& v1, const cv::Vec<P2, S>& v2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::VElementwise<
        MyOperatorOverloads::Subtract, P1, S, P2> >(v1, v2)
        .compute();
}
...
template <typename P1, int S, typename P2>
cv::Vec<typename MyOperatorOverloads::MultiplyType<P1, P2>::type, S> diagmult(
    const cv::Vec<P1, S>& v1, const cv::Vec<P2, S>& v2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::VElementwise<
        MyOperatorOverloads::Multiply, P1, S, P2> >(v1, v2)
        .compute();
}
...
cv::Vec<typename MyOperatorOverloads::MultiplyType<P1, P2>::type, 3> operator^(
    const cv::Matx<P1, 3, 1>& v1, const cv::Matx<P2, 3, 1>& v2) {
...
template <typename P1, typename P2>
cv::Matx<typename MyOperatorOverloads::MultiplyType<P1, P2>::type, 1, 3>
operator^(const cv::Matx<P1, 1, 3>& v1, const cv::Matx<P2, 1, 3>& v2) {
...
template <typename P1, typename P2>
cv::Mat_<typename MyOperatorOverloads::MultiplyType<P1, P2>::type> operator^(
    const cv::Mat_<P1>& mv1, const cv::Mat_<P2>& mv2) {
...
struct Operator<MElementwise<Op, P1, P2> > {
    ...
    Operator(const cv::Mat_<P1>& left_in, const cv::Mat_<P2>& right_in)
        : left(left_in), right(right_in) {}
    ...
    cv::Mat_<P0> compute() const {
        ...
        for (int r = 0; r < rrows; ++r) {
            for (int c = 0; c < rcols; ++c) {
                res(r, c) =
                    Op::template op<P0, P1, P2>(left(r, c), right(r, c));
            }
        }
        return res;
    }
...
struct Operator<MxElementwise<Op, P1, R, C, P2> > {
    ...
    Operator(const cv::Matx<P1, R, C>& left_in,
             const cv::Matx<P2, R, C>& right_in)
        : left(left_in), right(right_in) {
        assert(right_in.rows == left_in.rows && right_in.cols == left_in.cols &&
               "array size mismatch");
    }
...
template <typename P>
struct Operator<MElementwise<Add, P, P> > {

    const cv::Mat_<P>& left;
    const cv::Mat_<P>& right;

    Operator(const cv::Mat_<P>& left_in, const cv::Mat_<P>& right_in)
        : left(left_in), right(right_in) {}

    cv::Mat_<P> compute() const { return cv::operator+(left, right); }
};
...
struct Operator<MxElementwise<Add, P, R, C, P> > {
    ...
    Operator(const cv::Matx<P, R, C>& left_in,
             const cv::Matx<P, R, C>& right_in)
        : left(left_in), right(right_in) {}

    cv::Matx<P, R, C> compute() const {
        return cv::operator+(
            left, right);  // size mismatches should be taken care by OpenCV...
    }
};

// Subtraction Operator struct specialization for cv::Mat of same-type
template <typename P>
struct Operator<MElementwise<Subtract, P, P> > {
...
struct Operator<MxElementwise<Subtract, P, R, C, P> > {
    ...
    cv::Matx<P, R, C> compute() const {
        return cv::operator-(
            left, right);  // assertions tshould be taken care by OpenCV...
    }
};
...
template <typename P1, typename P2>
cv::Mat_<typename MyOperatorOverloads::AddType<P1, P2>::type> operator+(
    const cv::Mat_<P1>& m1, const cv::Mat_<P2>& m2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::MElementwise<
        MyOperatorOverloads::Add, P1, P2> >(m1, m2)
        .compute();
}

// Matrix Addition  operator '+' overload for cv::Matx objects
template <typename P1, int R, int C, typename P2>
cv::Matx<typename MyOperatorOverloads::AddType<P1, P2>::type, R, C> operator+(
    const cv::Matx<P1, R, C>& m1, const cv::Matx<P2, R, C>& m2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::MxElementwise<
        MyOperatorOverloads::Add, P1, R, C, P2> >(m1, m2)
        .compute();
}

// Matrix subtraction operator '-' overload for cv::Mat objects
template <typename P1, typename P2>
cv::Mat_<typename MyOperatorOverloads::SubtractType<P1, P2>::type> operator-(
    const cv::Mat_<P1>& m1, const cv::Mat_<P2>& m2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::MElementwise<
        MyOperatorOverloads::Subtract, P1, P2> >(m1, m2)
        .compute();
}

// Matrix subtraction operator '-' overload for cv::Matx objects
template <typename P1, int R, int C, typename P2>
cv::Matx<typename MyOperatorOverloads::SubtractType<P1, P2>::type, R, C>
operator-(const cv::Matx<P1, R, C>& m1, const cv::Matx<P2, R, C>& m2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::MxElementwise<
        MyOperatorOverloads::Subtract, P1, R, C, P2> >(m1, m2)
        .compute();
}

// cv::Mat .* cv::Mat - Elementwise multiplication of cv::Mat matrices
template <typename P1, typename P2>
cv::Mat_<typename MyOperatorOverloads::MultiplyType<P1, P2>::type> mmult(
    const cv::Mat_<P1>& m1, const cv::Mat_<P2>& m2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::MElementwise<
        MyOperatorOverloads::Multiply, P1, P2> >(m1, m2)
        .compute();
}

// cv::Matx .* cv::Matx - Elementwise multiplication of cv::Matx matrices
template <typename P1, int R, int C, typename P2>
cv::Matx<typename MyOperatorOverloads::MultiplyType<P1, P2>::type, R, C> mmult(
    const cv::Matx<P1, R, C>& m1, const cv::Matx<P2, R, C>& m2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::MxElementwise<
        MyOperatorOverloads::Multiply, P1, R, C, P2> >(m1, m2)
        .compute();
}
...
template <typename P1, typename P2>
struct Operator<MatrixMultiply<P1, P2> > {
    const cv::Mat_<P1>& left;
    const cv::Mat_<P2>& right;
...
template <typename P>
struct Operator<MatrixMultiply<P, P> > {
    const cv::Mat_<P>& left;
    const cv::Mat_<P>& right;
...
template <typename P1, int R1, int C1, typename P2, int R2, int C2>
struct Operator<MatxMultiply<P1, R1, C1, P2, R2, C2> > {
    const cv::Matx<P1, R1, C1>& left;
    const cv::Matx<P2, R2, C2>& right;

    Operator(const cv::Matx<P1, R1, C1>& left_in,
             const cv::Matx<P2, R2, C2>& right_in)
        : left(left_in), right(right_in) {
...
template <typename P, int R1, int C1, int R2, int C2>
struct Operator<MatxMultiply<P, R1, C1, P, R2, C2> > {
    const cv::Matx<P, R1, C1>& left;
    const cv::Matx<P, R2, C2>& right;

    Operator(const cv::Matx<P, R1, C1>& left_in,
             const cv::Matx<P, R2, C2>& right_in)
        : left(left_in), right(right_in) {}
...
cv::Mat_<typename MyOperatorOverloads::MultiplyType<P1, P2>::type> operator*(
    ...
    return MyOperatorOverloads::Operator<
               MyOperatorOverloads::MatrixMultiply<P1, P2> >(M1, M2)
        .compute();
}

// Standard Matrix multiplication Matrix * Matrix for cv::Matx objects: The actual operator
template <typename P1, int R1, int C1, typename P2, int R2, int C2>
cv::Matx<typename MyOperatorOverloads::MultiplyType<P1, P2>::type, R1, C2>
operator*(const cv::Matx<P1, R1, C1>& M1, const cv::Matx<P2, R2, C2>& M2) {

    return MyOperatorOverloads::Operator<
               MyOperatorOverloads::MatxMultiply<P1, R1, C1, P2, R2, C2> >(M1,
                                                                           M2)
        .compute();
}
...
struct Operator<MatrixVectorMultiply<PM, PV, Sz> > {
    ...
    Operator(const cv::Mat_<PM>& M_in, const cv::Vec<PV, Sz>& v_in)
        : M(M_in), v(v_in) {
    ...
    cv::Mat_<P0> compute() const {
        ...
        cv::Mat_<P0> res(M.rows,
                         1);  // we construct a rowsx1 matrix (it should suffice
...
struct Operator<MatxVectorMultiply<PM, R, C, PV, Sz> > {
    ...
    Operator(const cv::Matx<PM, R, C>& M_in, const cv::Vec<PV, Sz>& v_in)
        : M(M_in), v(v_in) {
        assert(
            M_in.cols == v.rows &&
            "Size mismatch");  // need to do this here otherwise noone will shout...
    }
...
struct Operator<MatxVectorMultiply<P, R, C, P, Sz> > {
    ...
    Operator(const cv::Matx<P, R, C>& M_in, const cv::Vec<P, Sz>& v_in)
        : M(M_in), v(v_in) {}

    cv::Vec<P, R> compute() const {

        return cv::operator*(
            M, v);  // size mismatches should be taken care of by OpenCV
    }
};
...
template <typename PM, typename PV, int Sz>
cv::Mat_<typename MyOperatorOverloads::MultiplyType<PM, PV>::type> operator*(
    const cv::Mat_<PM>& m, const cv::Vec<PV, Sz>& v) {
    return MyOperatorOverloads::Operator<
               MyOperatorOverloads::MatrixVectorMultiply<PM, PV, Sz> >(m, v)
        .compute();
}

// cv::Matx * cv::Vec multiplication operator actual overload.
template <typename PM, int R, int C, typename PV, int Sz>
cv::Vec<typename MyOperatorOverloads::MultiplyType<PM, PV>::type, R> operator*(
    const cv::Matx<PM, R, C>& m, const cv::Vec<PV, Sz>& v) {
    return MyOperatorOverloads::Operator<
               MyOperatorOverloads::MatxVectorMultiply<PM, R, C, PV, Sz> >(m, v)
        .compute();
}

// Now, it is easy to get the cv::Vec * cv::Mat operator...
template <typename PV, int Sz, typename PM>
cv::Mat_<typename MyOperatorOverloads::MultiplyType<PV, PM>::type> operator*(
    const cv::Vec<PV, Sz>& v, const cv::Mat_<PM>& M) {
