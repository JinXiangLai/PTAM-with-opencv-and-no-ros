...
class SimpleTimer {
   public:
    ...
    SimpleTimer(const std::string& description, const int& cycles_to_time = 1,
                bool output = true, std::ostream& out = std::cout)
        : output_info(output),
          max(0),
          min(0),
          average(0),
    ...
    ~SimpleTimer() { delete internal_Timer; }
    ...
    void print() {
        if (timings > 0) {
            if ((int)time_buffer.size() == 1)
                sout << text << " takes: " << get_average() << "s over 1 timing"
                     << std::endl;
            else
                sout << text << " takes : av " << get_average() << "s , max "
                     << get_max() << "s, min " << get_min() << "s, over "
                     << time_buffer.size() << " timings" << std::endl;
        } else
            sout << text
                 << " section : error. No timed cycles. Use click() to start "
                    "and stop timing."
                 << std::endl;
    }
