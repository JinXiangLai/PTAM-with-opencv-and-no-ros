...
std::pair<double, double> glDrawText(const std::string& text,
...
std::pair<double, double> glGetExtends(const std::string& text,
                                       double spacing = 1.5,
                                       double kerning = 0.1);
...
template <class P>
inline void glMultMatrix4x4(const cv::Matx<P, 4, 4>& m) {
    GLdouble glm[16];
    glm[0] = m(0, 0);
    glm[1] = m(1, 0);
    glm[2] = m(2, 0);
...
/// @param m the transformation matrix
template <class P>
inline void glMultMatrix3x3(const cv::Matx<P, 3, 3>& m) {
    GLdouble glm[16];
    glm[0] = m(0, 0);
    glm[1] = m(1, 0);
    glm[2] = m(2, 0);
...
/// @param m the transformation matrix
template <class P>
inline void glMultMatrix2x2(const cv::Matx<P, 3, 3>& m) {
    GLdouble glm[16];
    glm[0] = m(0, 0);
    glm[1] = m(1, 0);
    glm[2] = 0;
...
inline void glOrtho(const cv::Size2i& size, const double nearPlane = -1.0,
                    const double farPlane = 1.0) {
    ::glOrtho(-0.375, size.width - 0.375, size.height - 0.375, -0.375,
              nearPlane, farPlane);
}
...
template <typename P>
inline void glFrustum(const cv::Vec<P, 4>& params, double width, double height,
                      double nearPlane = 0.1, double farPlane = 100) {
...
inline void glLine(const P1& x1, const P2& x2) {
    glBegin(GL_LINES);
    glVertex(x1);
    glVertex(x2);
    glEnd();
...
template <typename P>
inline void glVertex(const std::vector<P>& list) {
...
inline void glReadPixelsBGR(cv::Mat& im,
                            cv::Point2i origin = cv::Point2i(0, 0)) {
    ::glReadPixels(origin.x, origin.y, im.cols, im.rows, GL_BGR,
...
/// @param origin The window co-ordinate of the first pixel to be read from the frame buffer
inline cv::Mat glReadPixelsBGR(cv::Size2i size,
                               cv::Point2i origin = cv::Point2i(0, 0)) {
...
inline void glReadPixelsGRAY(cv::Mat& im,
                             cv::Point2i origin = cv::Point2i(0, 0)) {
    glReadPixels(origin.x, origin.y, im.cols, im.rows, GL_LUMINANCE,
                 GL_UNSIGNED_BYTE, im.data);
}
...
/// @param size   The size of the area to read.
/// @param origin The window co-ordinate of the first pixel to be read from the frame buffer
inline cv::Mat glReadPixelsGRAY(cv::Size2i size,
                                cv::Point2i origin = cv::Point2i(0, 0)) {
...
inline void glTexSubImage2DBGR(const cv::Mat& img, GLint xoffset = 0,
                               GLint yoffset = 0, GLenum target = GL_TEXTURE_2D,
                               GLint level = 0) {
    ...
    ::glTexSubImage2D(target, level, xoffset, yoffset, img.cols, img.rows,
                      GL_BGR, GL_UNSIGNED_BYTE, img.data);
    ::glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
}

inline void glTexSubImage2DGRAY(const cv::Mat_<uchar>& img, GLint xoffset = 0,
                                GLint yoffset = 0,
...
inline void glTexImage2DBGR(const cv::Mat& img, GLint border = 0,
                            GLenum target = GL_TEXTURE_2D, GLint level = 0) {
    int alpha = std::min(cvdalignof(img.ptr<void>(0, 0)),
                         cvdalignof(img.ptr<void>(1, 0)));
    ::glPixelStorei(GL_UNPACK_ALIGNMENT, alpha);
    ::glPixelStorei(GL_UNPACK_ROW_LENGTH, img.step / img.elemSize());
    ::glTexImage2D(target, level, GL_BGR, img.cols, img.rows, border, GL_BGR,
