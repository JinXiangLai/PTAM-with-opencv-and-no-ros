...
class SE2 {
   ...
   public:
    ...
    SE2(const SO2<Precision>& R, const cv::Vec<Precision, 2>& T)
        : R_(R), t_(T) {}
    ...
    cv::Matx<Precision, 3, 3> get_matrix() {
        ...
        mat(2, 0) = 0 = mat(2, 1) = 0;
        mat(2, 2) = 1;
        mat(0, 2) = get_translation()[0];
        mat(0, 2) = get_translation()[1];
    ...
    template <typename P>
    SE2<typename MyOperatorOverloads::MultiplyType<Precision, P>::type>
    operator*(const SE2<P>& right) const {

        typedef
            typename MyOperatorOverloads::MultiplyType<Precision, P>::type P0;
    ...
    cv::Vec<Precision, 3> adjoint(const cv::Matx<Precision, 3, 1>& v) const {
        ...
        result[0] =
            R_.get_matrix()(0, 0) * v(0, 0) + R_.get_matrix()(0, 1) * v(1, 0);
        result[1] =
            R_.get_matrix()(1, 0) * v(0, 0) + R_.get_matrix()(1, 1) * v(1, 0);
    ...
    cv::Matx<Precision, 3, 3> adjoint(
        const cv::Matx<Precision, 3, 3>& M) const {
        cv::Matx<Precision, 3, 3> result;
        cv::Vec<Precision, 3> adj;
        for (int i = 0; i < 3; ++i) {
            //result.T()[i] = adjoint(M.T()[i]);
            adj = adjoint(cv::Vec<Precision, 3>(M(0, i), M(1, i), M(2, i)));
            result(0, i) = adj[0];
            result(1, i) = adj[1];
            result(2, i) = adj[2];
        }
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            adj = adjoint(cv::Vec<Precision, 3>(result(i, 0), result(i, 1),
                                                result(i, 2)));
            result(i, 0) = adj[0];
            result(i, 1) = adj[1];
            result(i, 2) = adj[2];
...
inline std::ostream& operator<<(std::ostream& os,
                                const RigidTransforms::SE2<Precision>& se2) {
...
inline std::istream& operator>>(std::istream& is,
                                RigidTransforms::SE2<Precision>& se2) {
    //for(int i=0; i<2; i++)
...
struct Operator<MyOperatorOverloads::SE2VMult<P, PV> > {
    ...
    Operator(const RigidTransforms::SE2<P>& se2_in, const cv::Vec<PV, 3>& v_in)
        : se2(se2_in), v(v_in) {}
    ...
    cv::Vec<P0, 3> compute() const {
        ...
        res[0] = se2.get_rotation().get_matrix()(0, 0) * v[0] +
                 se2.get_rotation().get_matrix()(0, 1) * v[1];
        res[1] = se2.get_rotation().get_matrix()(1, 0) * v[0] +
                 se2.get_rotation().get_matrix()(1, 1) * v[1];
...
struct Operator<MyOperatorOverloads::VSE2Mult<PV, P> > {
    ...
    Operator(const cv::Vec<PV, 3>& v_in, const RigidTransforms::SE2<P>& se2_in)
        : se2(se2_in), v(v_in) {}
...
operator*(const RigidTransforms::SE2<P>& se2, const cv::Vec<PV, 3>& v) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::SE2VMult<P, PV> >(
               se2, v)
        .compute();
}
...
template <typename P, typename PV>
inline cv::Vec<typename MyOperatorOverloads::MultiplyType<P, PV>::type, 2>
operator*(const RigidTransforms::SE2<P>& se2, const cv::Vec<PV, 2>& v) {
    ...
    return cv::Vec<P0, 2>(result[0], result[1]);
}
...
template <typename PV, typename P>
inline cv::Vec<typename MyOperatorOverloads::MultiplyType<PV, P>::type, 3>
operator*(const cv::Vec<PV, 3>& v, const RigidTransforms::SE2<P>& se2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::VSE2Mult<PV, P> >(
               v, se2)
        .compute();
}
...
struct Operator<SE2MMult<P, PM> > {
    ...
    Operator(const RigidTransforms::SE2<P>& se2_in, const cv::Mat_<PM>& M_in)
        : se2(se2_in), M(M_in) {}

    typedef typename MyOperatorOverloads::MultiplyType<P, PM>::type P0;

    cv::Mat_<P0> compute() const { return se2.get_matrix() * M; }
};
...
operator*(const RigidTransforms::SE2<P>& se2, const cv::Mat_<PM>& M) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::SE2MMult<P, PM> >(
               se2, M)
        .compute();
}
...
struct Operator<MSE2Mult<PM, P> > {
    ...
    Operator(const cv::Mat_<PM>& M_in, const RigidTransforms::SE2<P>& se2_in)
        : M(M_in), se2(se2_in) {}

    typedef typename MyOperatorOverloads::MultiplyType<PM, P>::type P0;

    cv::Mat_<P0> compute() const { return M * se2.get_matrix(); }
};
}  // namespace MyOperatorOverloads
...
operator*(const cv::Mat_<PM>& M, const RigidTransforms::SE2<P>& se2) {

    return MyOperatorOverloads::Operator<MyOperatorOverloads::MSE2Mult<PM, P> >(
        M, se2);
}
...
inline RigidTransforms::SE2<Precision> RigidTransforms::SE2<Precision>::exp(
    const cv::Vec<PV, 3>& mu) {
    ...
    const Precision theta =
        mu[2];  // angle is stored in the last corindate of the 2D pose vector
    ...
    if (theta_sq < 1e-8) {
        ...
        result.get_translation() = cv::Vec<Precision, 2>(
            mu[0] + 0.5 * cross[0], mu[1] + 0.5 * cross[1]);
        ...
        result.get_translation() = cv::Vec<Precision, 2>(
            A * mu[0] + B * cross[0], A * mu[1] + B * cross[1]);
    }
    return result;
}
...
template <typename Precision>
inline cv::Vec<Precision, 3> RigidTransforms::SE2<Precision>::ln(
    const RigidTransforms::SE2<Precision>& se2) {
    ...
    result[0] = (R(0, 0) * t[0] + R(0, 1) * t[1]) / (2 * shtot);
    result[1] = (R(1, 0) * t[0] + R(1, 1) * t[1]) / (2 * shtot);
...
template <typename Precision>
inline RigidTransforms::SE2<Precision> operator*(
    const RigidTransforms::SO2<Precision>& so2,
    const RigidTransforms::SE2<Precision>& se2) {

    return RigidTransforms::SE2<Precision>(
        so2 * se2.get_rotation(), so2.get_matrix() * se2.get_translation());
}
