...
class WLS {
   ...
   private:
    ...
    unsigned int
        DecompositionType;  // will be just passed to the inversion method
    int Dim;

   public:
    /// Default constructor or construct with the number of dimensions for the Dynamic case
    WLS(int dim = 1, unsigned int decomposition = cv::DECOMP_CHOLESKY)
        : Omega_(cv::Mat_<Precision>(dim, dim)),
    ...
    void add_prior_diagonal(
        const cv::Mat_<Precision>& v) {  // here v is a Dim x 1 matrix
    ...
    void add_prior(const cv::Mat_<Precision>& m) { Omega_ += m; }
    ...
    inline void add_mJ(Precision y, const cv::Mat_<Precision>& J,
                       Precision weight = 1) {
    ...
    inline void add_mJ(
        const cv::Mat_<Precision>& y,  // N x 1 matrix
        const cv::Mat_<Precision>& J,  // Jacobian (N x Dim)
        const cv::Mat_<Precision>&
            Qinv  // The inverse covariance matrix of the measurement likelihood
    ) {

        const cv::Mat_<Precision> temp =
            J.t() * Qinv;  // this is a (1xDim)*(Dim x Dim) = 1 x Dim matrix
        Omega_ += temp * J;
        ksi_ += temp * y;
    }
    ...
    inline void add_sparse_mJ(
        const Precision y,              // N x 1 measurement vector
    ...
    inline void add_sparse_mJ_rows(
        const cv::Mat_<Precision>& y,  // N x 1 matrix
        const cv::Mat_<Precision>& J,  // N x M matrix
        const int
            index,  // Starting index of the first dependent variable in the state vector
        const cv::Mat_<Precision>& Qinv) {
        const cv::Mat_<Precision> temp1 =
            J.t() * Qinv;  // this is a (N x M) * (N x N ) = M x N matrix
        const int M = J.cols;
        // so now updating block-wise....
        cv::Mat_<Precision> OmegaBlockMxM =
            Omega_(cv::Range(index, index + M), cv::Range(index, index + M));
        ...
        cv::Mat_<Precision> ksiBlockMx1 =
            ksi_(cv::Range(index, index + M), cv::Range::all());
    ...
    inline void add_sparse_mJ_rows(const cv::Mat_<Precision>& y,
        ...
        const cv::Mat_<Precision> temp1 =
            J1.t() * Qinv;  // An (M1 x N) * (N x N) = M1 x N matrix
        const cv::Mat_<Precision> temp2 =
            J2.t() * Qinv;  // An (M2 x N) * (N x N) = M2 x N matrix
        const cv::Mat_<Precision> mixed =
            temp1 * J2;  // A (M1 x N) * (N x M2) = M1 x M2 block
        const int M1 = J1.cols;
        const int M2 = J2.cols;
        //my_C_inv.slice(index1, index1, size1, size1) += temp1 * J1;
        cv::Mat_<Precision> OmegaBlockM1xM1 = Omega_(
            cv::Range(index1, index1 + M1), cv::Range(index1, index1 + M1));
        ...
        cv::Mat_<Precision> OmegaBlockM2xM2 = Omega_(
            cv::Range(index2, index2 + M2), cv::Range(index2, index2 + M2));
        ...
        cv::Mat_<Precision> OmegaBlockM1xM2 = Omega_(
            cv::Range(index1, index1 + M1), cv::Range(index2, index2 + M2));
        ...
        cv::Mat_<Precision> OmegaBlockM2xM1 = Omega_(
            cv::Range(index2, index2 + M2), cv::Range(index1, index1 + M2));
        ...
        //my_vector.slice(index1, size1) += temp1 * m;
        cv::Mat_<Precision> ksiBlockM1 =
            ksi_(cv::Range(index1, index1 + M1), cv::Range::all());
        ...
        cv::Mat_<Precision> ksiBlockM2 =
            ksi_(cv::Range(index2, index2 + M2), cv::Range(0, 1));
    ...
    cv::Mat_<Precision>& get_Omega() {
        return Omega_;
    }  /// Returns the information matrix
    const cv::Mat_<Precision>& get_Omega() const {
        return Omega_;
    }  /// Returns information matrix (const overload)
    cv::Mat_<Precision>& get_mu() {
        return mu_;
    }  ///Returns the update. With no prior, this is the last computation inv(Omega_) * ksi_.
    const cv::Mat_<Precision>& get_mu() const {
        return mu_;
    }  ///<Returns the update. With no prior, this is the result of \f$J^\dagger e\f$.
    cv::Mat_<Precision>& get_ksi() {
        return ksi_;
    }  ///Returns the  information vector
    const cv::Mat_<Precision>& get_ksi() const {
        return ksi_;
    }  ///Returns the  information vector (constant overload)
    unsigned int get_decomposition() {
        return DecompositionType;
    }  /// Return the decomposition type (just an integer)
};
