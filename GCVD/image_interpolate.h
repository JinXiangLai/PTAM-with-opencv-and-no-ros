...
class image_interpolate<Interpolate::NearestNeighbour, T> {
   ...
   public:
    image_interpolate(const cv::Mat_<T>& i)
        : im(&i), numChannels(i.channels()) {}

    template <typename P>
    bool in_image(const cv::Vec<P, 2>& pos) const {
        return (pos[0] >= 0) && (pos[1] >= 0) && (pos[0] < im->cols) &&
               (pos[1] < im->rows);
    }
    ...
    cv::Vec4d operator[](const cv::Vec<P, 2>& pos) const {
        ...
        for (int i = 0; i < numChannels; i++) {
            T* pt =
                (T*)im->ptr((int)std::round(pos[1]), (int)std::round(pos[0]));
            ret[i] = (double)pt[i];
        }
   ...
   public:
    ...
    bool in_image(const cv::Vec<P, 2>& pos) const {
        ...
        return (std::floor(pos[0]) >= 0) && (std::floor(pos[1]) >= 0) &&
               (std::floor(pos[0]) < im->cols) &&
               (std::floor(pos[1]) < im->rows) && (std::ceil(pos[0]) >= 0) &&
               (std::ceil(pos[1]) >= 0) && (std::ceil(pos[0]) < im->cols) &&
               (std::ceil(pos[1]) < im->rows);
    }
    ...
    cv::Vec4d operator[](const cv::Vec<P, 2>& pos) const {
        ...
        cv::Point2i p =
            cv::Point2i((int)std::floor(pos[0]), (int)std::floor(pos[1]));
...
class image_interpolate<Interpolate::Bicubic, T> {
   private:
    ...
    template <typename P>
    P r(P x) const {
        return (pow(p(x + 2), 3) - 4 * pow(p(x + 1), 3) + 6 * pow(p(x), 3) -
                4 * pow(p(x - 1), 3)) /
               6;
    }
   ...
   public:
    ...
    template <typename P>
    bool in_image(const cv::Vec<P, 2>& pos) const {
        return pos[0] >= 1 && pos[1] >= 1 && pos[0] < im->cols - 2 &&
               pos[1] < im->rows - 2;
    }
