...
class EssentialInit : Initializer {
   ...
   public:
    ...
    static constexpr double RANSAC_DEFAULT_THRESHOLD_BOUND =
        1 - cos(2.0 * M_PI / 180);

    bool Compute(std::vector<InitializerMatch> vMatches,
                 double dMaxAngleCosError, SE3<>& se3SecondCameraPose);
    EssentialInit(ATANCamera* pcam) : Initializer(pcam){};

   protected:
    cv::Matx<float, 3, 3> EssentialFromMatchesLLS(
        std::vector<InitializerMatch> vMatches);
    void BestEssentialFromMatches_MLESAC();

    EssentialRelativePose PickCorrectRelativePose(
        cv::Matx<float, 3, 3>& m3E, vector<InitializerMatch>& vMatches);

    void ComputeEssentialJac(
    ...
    double ComputeMisalignementCostAndJac(
        Quaternion<>& q,  // Quaternion is only references
    ...
    bool IsEssentialInlier(const EssentialRelativePose& rpose,
                           const InitializerMatch& match);
    double MLESACScore(const EssentialRelativePose& rpose,
                       const InitializerMatch& match);

    double
        mdMaxAngleCosError;  // This is angular error between epipolar planes!
