...
inline int MiniPatch::SSDAtPoint(cv::Mat_<uchar>& im, const cv::Point2i& ir) {
    if (!CvUtils::in_image_with_border(ir.y, ir.x, im, mnHalfPatchSize,
                                       mnHalfPatchSize))
        return mnMaxSSD + 1;
...
// FAST corners
bool MiniPatch::FindPatch(cv::Point2i& irPos, cv::Mat_<uchar>& im, int nRange,
    ...
    int nBestSSD =
        mnMaxSSD +
        1;  // se the best square difference to something not acceptable (abovw the maximum allowable difference)
    ...
    // point indexed by the row of the left top corner pf the search region
        ...
        if (nTopRow < 0)
            nTopRow = 0;  // if below 0 then start from the first (0) row.
...
void MiniPatch::SampleFromImage(cv::Point2i irPos, cv::Mat_<uchar>& im) {
    assert(CvUtils::in_image_with_border(irPos.y, irPos.x, im, mnHalfPatchSize,
                                         mnHalfPatchSize) &&
           "irPos out of bounds when minipatch sampling!!! That should have "
    ...
    cv::Point2i LRCorner(TLCorner.x + irPatchSize.width,
                         TLCorner.y + irPatchSize.height);
