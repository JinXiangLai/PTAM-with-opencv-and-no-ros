...
struct
    Trail  // This struct is used for initial correspondences of the first stereo pair.
{
    MiniPatch mPatch;
...
class Tracker {
   ...
   protected:
    ...
    void
    TrackForInitialMap();  // This is called by TrackFrame if there is not a map yet.
    enum {
    ...
    void
    TrailTracking_Start();  // First frame of initial trail tracking. Called by TrackForInitialMap.
    int
    TrailTracking_Advance();  // Steady-state of initial trail tracking. Called by TrackForInitialMap.
    std::list<Trail> mlTrails;  // Used by trail tracking
    KeyFrame::Ptr pFirstKF;     // First of the stereo pair
    KeyFrame::Ptr
        pPreviousFrameKF;  // Used by trail tracking to check married matches

    // a helper function for corner comparisons (passed to sort() )
    bool pairCompare(const std::pair<double, cv::Point2i>& left,
    ...
    void
    ApplyMotionModel();  // Decaying velocity motion model applied prior to TrackMap
    ...
    cv::Vec<float, 6> CalcPoseUpdate(
        std::vector<std::shared_ptr<TrackerData> > vTD,
        double dOverrideSigma = 0.0,
        bool bMarkOutliers = false);  // Updates pose from found points.
    SE3<>
        mse3CamFromWorld;  // Camera pose: this is what the tracker updates every frame.
    ...
    double
        mdMSDScaledVelocityMagnitude;  // Velocity magnitude scaled by relative scene depth.
    ...
    void
    AddNewKeyFrame();  // Gives the current frame to the mapmaker to use as a keyframe
