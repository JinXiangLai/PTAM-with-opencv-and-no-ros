...
inline bool EssentialInit::IsEssentialInlier(const EssentialRelativePose& rpose,
                                             const InitializerMatch& match) {

    // 1. Obtain the normalized Euclidean projections of the correspondences
    cv::Vec<float, 3> v3EucPlaneFirst =
        CvUtils::backproject(match.v2EucPlaneFirst);
    cv::Vec<float, 3> v3EucPlaneSecond =
        CvUtils::backproject(match.v2EucPlaneSecond);
    ...
    double AngleCosError =
        1 - CvUtils::normalize(p1).dot(CvUtils::normalize(p2));

    // dMaxPixelErrorSquared is the sequared robust cutoff error by Torr and Zisserman
    return (AngleCosError < mdMaxAngleCosError);
}

inline double EssentialInit::MLESACScore(const EssentialRelativePose& rpose,
                                         const InitializerMatch& match) {
    ...
    return AngleCosError > mdMaxAngleCosError ? mdMaxAngleCosError
                                              : AngleCosError;
}

bool EssentialInit::Compute(vector<InitializerMatch> vMatches,
                            double dMaxAngleCosError,
    ...
    for (unsigned int i = 0; i < mvMatches.size(); i++) {
        // checking if inlier
        if (!IsEssentialInlier(mrpBestPose, mvMatches[i]))
            continue;
        // checking if it projects correctly
        // projection to the second view

        double Z1 = TwoViewGeometry::recontructDepth2Views(
            vMatches[i].v2EucPlaneFirst[0], vMatches[i].v2EucPlaneFirst[1],
            vMatches[i].v2EucPlaneSecond[0], vMatches[i].v2EucPlaneSecond[1], R,
            b);
        // obtaining the predicted coordinates in the second view frame
        cv::Vec<float, 3> M2 =
            R.t() *
            (Z1 * CvUtils::backproject(vMatches[i].v2EucPlaneFirst) - b);
        ...
        if (p2[1] > pCamera->GetImageSize().height - 1 || p2[1] < 0 ||
            p2[0] > pCamera->GetImageSize().width - 1 || p2[0] < 0) {
    ...
    mrpBestPose.se3.get_rotation().get_matrix() =
        mrpBestPose.se3.get_rotation().get_matrix().t();
    mrpBestPose.se3.get_translation() =
        -mrpBestPose.se3.get_rotation().get_matrix() *
        mrpBestPose.se3.get_translation();
...
cv::Matx<float, 3, 3> EssentialInit::EssentialFromMatchesLLS(
    vector<InitializerMatch> vMatches) {
...
void EssentialInit::RefineEssentialWithInliers(int maxIteration) {
    ...
    cv::Vec<float, 3> sop;
    q.MRPCoordinates(sop.val);
    cv::Matx<float, 3, 3> R;
    q.RotationMatrix(R.val);
    cv::Matx<float, 3, 3> E = R.t() * Bx;
    cv::Vec<float, 9> v9e(E(0, 0), E(0, 1), E(0, 2), E(1, 0), E(1, 1), E(1, 2),
                          E(2, 0), E(2, 1), E(2, 2));
    ...
    cv::Vec<float, 5> mu(sop[0], sop[1], sop[2], b[0] / (1 + b[2]),
                         b[1] / (1 + b[2]));
    ...
    while (!stop) {
        ...
        Quaternion<> qtmp = Quaternion<>::CreateFromMRPs(
            mutemp
                .val);  // the method will only access the first 3 SOP parameters
        cv::Vec<float, 3> btmp(
            2 * mutemp[3], 2 * mutemp[4],
            1 - mutemp[3] * mutemp[3] - mutemp[4] * mutemp[4]);
        btmp = (1.0 / (1.0 + mutemp[3] * mutemp[3] + mutemp[4] * mutemp[4])) *
               btmp;
        cv::Matx<float, 3, 3> Bxtmp = TwoViewGeometry::CrossPoductMatrix(btmp);
        cv::Matx<float, 3, 3> Rtmp;
        qtmp.RotationMatrix(Rtmp.val);
        cv::Matx<float, 3, 3> Etmp = Rtmp.t() * Bxtmp;
        cv::Vec<float, 9> etmp(Etmp(0, 0), Etmp(0, 1), Etmp(0, 2), Etmp(1, 0),
                               Etmp(1, 1), Etmp(1, 2), Etmp(2, 0), Etmp(2, 1),
                               Etmp(2, 2));
        ...
        if (errorsq < tol || step > maxIteration ||
            fabs(prevErrorsq - errorsq) < 10e-12)
            stop = true;
        else {
...
void EssentialInit::RefineEssentialWithInliers1(int maxIteration) {
    ...
    double prev_cost = -99999999999, cur_cost = -9999999;
    ...
    do {
        ...
        cout << "DEBUG: Current cost (misalignement cosine) : " << cur_cost
             << endl;

        if (cur_cost < cost_tol || fabs(cur_cost - prev_cost) < dcost_tol)
            break;
        prev_cost = cur_cost;

        mu += gamma * J;
        // updating quaternion, rotation and baseline
        q = Quaternion<>::CreateFromMRPs<>(
            mu.val);  // will access ONLY the first 3 elements anyway...
        q.RotationMatrix(R.val);
        float n2 = mu[3] * mu[3] + mu[4] * mu[4];
        float inv_one_plus_n2 = 1.0 / (1 + n2);
        b[0] = 2 * mu[3] * inv_one_plus_n2;
        b[1] = 2 * mu[4] * inv_one_plus_n2;
        b[2] = (1 - n2) * inv_one_plus_n2;
...
double EssentialInit::ComputeMisalignementCostAndJac(
    Quaternion<>&
        q,  // only referenced, because the rotation matrix method normalizes it
    const cv::Vec<float, 3>& v3b, cv::Vec<float, 5>& m15J) {
    ...
    cv::Matx<float, 3, 3> dRdpsi0;
    q.RotationJacWRTMRPs(0, dRdpsi0.val);  // rotation derivative wrt psi0
    cv::Matx<float, 3, 3> dRdpsi1;
    q.RotationJacWRTMRPs(1, dRdpsi1.val);  // rotation derivative wrt psi1
    cv::Matx<float, 3, 3> dRdpsi2;
    q.RotationJacWRTMRPs(2, dRdpsi2.val);  // rotation derivative wrt psi2

    // 2. Obtain the derivative of the baseline
    // a. derivative wrt 1st seterographic coordinate (k)
    cv::Vec<float, 3> dbdk(-v3b[0] * v3b[0] + 1 + v3b[2], -v3b[0] * v3b[1],
                           -v3b[0] * (1 + v3b[2]));

    // b. derivative of baseline wrt 2nd seterographic coordinate (l)
    cv::Vec<float, 3> dbdl(-v3b[1] * v3b[0], -v3b[1] * v3b[1] + 1 + v3b[2],
                           -v3b[1] * (1 + v3b[2]));
    // 2. Derivative of the projector wrt to the baseline SOP parameters k and l
    cv::Matx<float, 3, 3> dPdk;
    dPdk(0, 0) = -(dbdk[0] * v3b[0] + v3b[0] * dbdk[0]);
    dPdk(0, 1) = -(dbdk[0] * v3b[1] + v3b[0] * dbdk[1]);
    dPdk(0, 2) = -(dbdk[0] * v3b[2] + v3b[0] * dbdk[2]);
    dPdk(1, 0) = -(dbdk[1] * v3b[0] + v3b[1] * dbdk[0]);
    dPdk(1, 1) = -(dbdk[1] * v3b[1] + v3b[1] * dbdk[1]);
    dPdk(1, 2) = -(dbdk[1] * v3b[2] + v3b[1] * dbdk[2]);
    dPdk(2, 0) = -(dbdk[2] * v3b[0] + v3b[2] * dbdk[0]);
    dPdk(2, 1) = -(dbdk[2] * v3b[1] + v3b[2] * dbdk[1]);
    dPdk(2, 2) = -(dbdk[2] * v3b[2] + v3b[2] * dbdk[2]);

    cv::Matx<float, 3, 3> dPdl;
    dPdl(0, 0) = -(dbdl[0] * v3b[0] + v3b[0] * dbdl[0]);
    dPdl(0, 1) = -(dbdl[0] * v3b[1] + v3b[0] * dbdl[1]);
    dPdl(0, 2) = -(dbdl[0] * v3b[2] + v3b[0] * dbdl[2]);
    dPdl(1, 0) = -(dbdl[1] * v3b[0] + v3b[1] * dbdl[0]);
    dPdl(1, 1) = -(dbdl[1] * v3b[1] + v3b[1] * dbdl[1]);
    dPdl(1, 2) = -(dbdl[1] * v3b[2] + v3b[1] * dbdl[2]);
    dPdl(2, 0) = -(dbdl[2] * v3b[0] + v3b[2] * dbdl[0]);
    dPdl(2, 1) = -(dbdl[2] * v3b[1] + v3b[2] * dbdl[1]);
    dPdl(2, 2) = -(dbdl[2] * v3b[2] + v3b[2] * dbdl[2]);
    ...
    for (unsigned int i = 0; i < mvEssentialInliers.size(); i++) {
        ...
        double numerator =
            (x2r * P(0, 0) + y2r * P(1, 0) + z2r * P(2, 0)) * x1 +
        ...
        double x2_psi0 =
                   dRdpsi0(0, 0) * x2 + dRdpsi0(0, 1) * y2 + dRdpsi0(0, 2) * z2,
               y2_psi0 =
                   dRdpsi0(1, 0) * x2 + dRdpsi0(1, 1) * y2 + dRdpsi0(1, 2) * z2,
               z2_psi0 =
                   dRdpsi0(2, 0) * x2 + dRdpsi0(2, 1) * y2 + dRdpsi0(2, 2) * z2;
        double dfdpsi0 =
            ((x2_psi0 * P(0, 0) + y2_psi0 * P(1, 0) + z2_psi0 * P(2, 0)) * x1 +
             (x2_psi0 * P(0, 1) + y2_psi0 * P(1, 1) + z2_psi0 * P(2, 1)) * y1 +
             (x2_psi0 * P(0, 2) + y2_psi0 * P(1, 2) + z2_psi0 * P(2, 2)) * z1) *
            inv_denominator;
        double x2_psi1 =
                   dRdpsi1(0, 0) * x2 + dRdpsi1(0, 1) * y2 + dRdpsi1(0, 2) * z2,
               y2_psi1 =
                   dRdpsi1(1, 0) * x2 + dRdpsi1(1, 1) * y2 + dRdpsi1(1, 2) * z2,
               z2_psi1 =
                   dRdpsi1(2, 0) * x2 + dRdpsi1(2, 1) * y2 + dRdpsi1(2, 2) * z2;

        double dfdpsi1 =
            ((x2_psi1 * P(0, 0) + y2_psi1 * P(1, 0) + z2_psi1 * P(2, 0)) * x1 +
             (x2_psi1 * P(0, 1) + y2_psi1 * P(1, 1) + z2_psi1 * P(2, 1)) * y1 +
             (x2_psi1 * P(0, 2) + y2_psi1 * P(1, 2) + z2_psi1 * P(2, 2)) * z1) *
            inv_denominator;

        double x2_psi2 =
                   dRdpsi2(0, 0) * x2 + dRdpsi2(0, 1) * y2 + dRdpsi2(0, 2) * z2,
               y2_psi2 =
                   dRdpsi2(1, 0) * x2 + dRdpsi2(1, 1) * y2 + dRdpsi2(1, 2) * z2,
               z2_psi2 =
                   dRdpsi2(2, 0) * x2 + dRdpsi2(2, 1) * y2 + dRdpsi2(2, 2) * z2;

        double dfdpsi2 =
            ((x2_psi2 * P(0, 0) + y2_psi2 * P(1, 0) + z2_psi2 * P(2, 0)) * x1 +
             (x2_psi2 * P(0, 1) + y2_psi2 * P(1, 1) + z2_psi2 * P(2, 1)) * y1 +
             (x2_psi2 * P(0, 2) + y2_psi2 * P(1, 2) + z2_psi2 * P(2, 2)) * z1) *
            inv_denominator;

        // 2. Derivatives wrt stereographic parameters of the baseline
        // The dot product of
        double dfdk =
            ((x2r * dPdk(0, 0) + y2r * dPdk(1, 0) + z2r * dPdk(2, 0)) * x1 +
             (x2r * dPdk(0, 1) + y2r * dPdk(1, 1) + z2r * dPdk(2, 1)) * y1 +
             (x2r * dPdk(0, 2) + y2r * dPdk(1, 2) + z2r * dPdk(2, 2)) * x1) *
                inv_denominator +
            numerator * dot_u1_b *
                (dbdk[0] * x1 + dbdk[1] * y1 + dbdk[2] * z1) * inv_denominator *
                inv_denominator * inv_denominator;

        double dfdl =
            ((x2r * dPdl(0, 0) + y2r * dPdl(1, 0) + z2r * dPdl(2, 0)) * x1 +
             (x2r * dPdl(0, 1) + y2r * dPdl(1, 1) + z2r * dPdl(2, 1)) * y1 +
             (x2r * dPdl(0, 2) + y2r * dPdl(1, 2) + z2r * dPdl(2, 2)) * x1) *
                inv_denominator +
            numerator * dot_u1_b *
                (dbdl[0] * x1 + dbdl[1] * y1 + dbdl[2] * z1) * inv_denominator *
                inv_denominator * inv_denominator;
...
void EssentialInit::ComputeEssentialJac(
    const cv::Matx<float, 9, 9>& m9D,  // the 9x9 data matrix
    Quaternion<>&
        q,  // relative orientation (must be referenced because the quaternion
    ...
    dBxdk(0, 0) = 0;
    dBxdk(0, 1) = v3b[0] * (1 + v3b[2]);
    dBxdk(0, 2) = -v3b[0] * v3b[1];
    dBxdk(1, 0) = -dBxdk(0, 1);
    dBxdk(1, 1) = 0;
    dBxdk(1, 2) = v3b[0] * v3b[0] - 1 - v3b[2];
    dBxdk(2, 0) = -dBxdk(0, 2);
    dBxdk(2, 1) = -dBxdk(1, 2);
    dBxdk(2, 2) = 0;
    ...
    dBxdl(0, 0) = 0;
    dBxdl(0, 1) = v3b[1] * (1 + v3b[2]);
    dBxdl(0, 2) = -v3b[1] * v3b[1] + v3b[2] + 1;
    dBxdl(1, 0) = -dBxdl(0, 1);
    dBxdl(1, 1) = 0;
    dBxdl(1, 2) = v3b[0] * v3b[1];
    dBxdl(2, 0) = -dBxdl(0, 2);
    dBxdl(2, 1) = -dBxdl(1, 2);
    dBxdl(2, 2) = 0;
    ...
    m95J(0, 0) = dEdpsi0(0, 0);
    m95J(0, 1) = dEdpsi0(0, 1);
    m95J(0, 2) = dEdpsi0(0, 2);
    m95J(0, 3) = dEdpsi0(1, 0);
    m95J(0, 4) = dEdpsi0(1, 1);
    m95J(0, 5) = dEdpsi0(1, 2);
    m95J(0, 6) = dEdpsi0(2, 0);
    m95J(0, 7) = dEdpsi0(2, 1);
    m95J(0, 8) = dEdpsi0(2, 2);

    m95J(1, 0) = dEdpsi1(0, 0);
    m95J(1, 1) = dEdpsi1(0, 1);
    m95J(1, 2) = dEdpsi1(0, 2);
    m95J(1, 3) = dEdpsi1(1, 0);
    m95J(1, 4) = dEdpsi1(1, 1);
    m95J(1, 5) = dEdpsi1(1, 2);
    m95J(1, 6) = dEdpsi1(2, 0);
    m95J(1, 7) = dEdpsi1(2, 1);
    m95J(1, 8) = dEdpsi1(2, 2);

    m95J(2, 0) = dEdpsi2(0, 0);
    m95J(2, 1) = dEdpsi2(0, 1);
    m95J(2, 2) = dEdpsi2(0, 2);
    m95J(2, 3) = dEdpsi2(1, 0);
    m95J(2, 4) = dEdpsi2(1, 1);
    m95J(2, 5) = dEdpsi2(1, 2);
    m95J(2, 6) = dEdpsi2(2, 0);
    m95J(2, 7) = dEdpsi2(2, 1);
    m95J(2, 8) = dEdpsi2(2, 2);
...
EssentialRelativePose EssentialInit::PickCorrectRelativePose(
    cv::Matx<float, 3, 3>& m3E, vector<InitializerMatch>& vMatches) {
    ...
    for (unsigned int i = 0; i < 4; i++) {
        ...
        for (unsigned int j = 0; j < vMatches.size(); j++) {
            ...
            C[0] = (R(0, 0) - R(0, 2) * x2) * (R(0, 0) - R(0, 2) * x2) +
                   (R(0, 1) - R(0, 2) * y2) * (R(0, 1) - R(0, 2) * y2);

            C[1] = (R(0, 0) - R(0, 2) * x2) * (R(1, 0) - R(1, 2) * x2) +
                   (R(0, 1) - R(0, 2) * y2) * (R(1, 1) - R(1, 2) * y2);

            C[2] = (R(0, 0) - R(0, 2) * x2) * (R(2, 0) - R(2, 2) * x2) +
                   (R(0, 1) - R(0, 2) * y2) * (R(2, 1) - R(2, 2) * y2);

            C[3] = (R(1, 0) - R(1, 2) * x2) * (R(1, 0) - R(1, 2) * x2) +
                   (R(1, 1) - R(1, 2) * y2) * (R(1, 1) - R(1, 2) * y2);

            C[4] = (R(1, 0) - R(1, 2) * x2) * (R(2, 0) - R(2, 2) * x2) +
                   (R(1, 1) - R(1, 2) * y2) * (R(2, 1) - R(2, 2) * y2);

            C[5] = (R(2, 0) - R(2, 2) * x2) * (R(2, 0) - R(2, 2) * x2) +
                   (R(2, 1) - R(2, 2) * y2) * (R(2, 1) - R(2, 2) * y2);
    ...
    bestRelPose.ProjectFromView1(0, 0) = 1 - b[0] * b[0];
    bestRelPose.ProjectFromView1(0, 1) = -b[0] * b[1];
    bestRelPose.ProjectFromView1(0, 2) = -b[0] * b[2];
    bestRelPose.ProjectFromView1(1, 0) = -b[1] * b[0];
    bestRelPose.ProjectFromView1(1, 1) = 1 - b[1] * b[1];
    bestRelPose.ProjectFromView1(1, 2) = -b[1] * b[2];
    bestRelPose.ProjectFromView1(2, 0) = -b[2] * b[0];
    bestRelPose.ProjectFromView1(2, 1) = -b[2] * b[1];
    bestRelPose.ProjectFromView1(2, 2) = 1 - b[2] * b[2];

    bestRelPose.ProjectFromView2 =
        bestRelPose.ProjectFromView1 *
        poses[bestScoreIndex].get_rotation().get_matrix();

    bestRelPose.se3 = poses[bestScoreIndex];
    bestRelPose.nScore = Scores
        [bestScoreIndex];  // this will NOT have to be negated in the process.

    return bestRelPose;
}

// Need this for sorting candidate transformations...
bool operator<(const EssentialRelativePose lhs,
               const EssentialRelativePose rhs) {

    return lhs.nScore < rhs.nScore;
}

static double SampsonusError(cv::Vec<float, 2>& v2Dash,
                             const cv::Matx<float, 3, 3>& m3Essential,
                             cv::Vec<float, 2>& v2) {
    ...
    // fv3 = E * v3
    cv::Vec3f fv3(m3Essential(0, 0) * v3[0] + m3Essential(0, 1) * v3[1] +
                      m3Essential(0, 2) * v3[2],
                  m3Essential(1, 0) * v3[0] + m3Essential(1, 1) * v3[1] +
                      m3Essential(1, 2) * v3[2],
                  m3Essential(2, 0) * v3[0] + m3Essential(2, 1) * v3[1] +
                      m3Essential(2, 2) * v3[2]);

    //fTv3Dash = E' * v3Dash;
    cv::Vec3f fTv3Dash(
        m3Essential(0, 0) * v3Dash[0] + m3Essential(1, 0) * v3Dash[1] +
            m3Essential(2, 0) * v3Dash[2],
        m3Essential(0, 1) * v3Dash[0] + m3Essential(1, 1) * v3Dash[1] +
            m3Essential(2, 1) * v3Dash[2],
        m3Essential(0, 2) * v3Dash[0] + m3Essential(1, 2) * v3Dash[1] +
            m3Essential(2, 2) * v3Dash[2]);
    //Vector<2> fv3Slice = fv3.slice<0,2>();
    cv::Vec2f fv3Slice(fv3[0], fv3[1]);

    //Vector<2> fTv3DashSlice = fTv3Dash.slice<0,2>();
    cv::Vec2f fTv3DashSlice(fTv3Dash[0], fTv3Dash[1]);

    return dError * dError /
           (fv3Slice[0] * fv3Slice[0] + fv3Slice[1] * fv3Slice[1] +
            fTv3DashSlice[0] * fTv3DashSlice[0] +
            fTv3DashSlice[1] * fTv3DashSlice[1]);
}
