...
int main() {
    cout << "  Welcome to CameraCalibrator for Visual Tracking and Mapping"
         << endl;
    cout << "  "
            "----------------------------------------------------------------- "
         << endl;

    cout << "  A deep code adaptation of the original 'Parallel tracking and "
            "Mapping' by Klein and Murray"
         << endl;
    cout << "  This code is purely OpenCV based and does not require libCVD, "
            "TooN and GVars (although you will need openGL)"
         << endl;
    cout << " *" << endl;
    cout << " For interoperability with OpenCV methods and structures," << endl;
    cout << " Partial functionality of TooN, libCVD and GVars was retained "
            "based "
         << endl
         << "on the latest code by Rosten, Klein and others." << endl
         << endl;
    ...
    GUI.LoadFile("../src/calibrator_settings.cfg");
...
CameraCalibrator::CameraCalibrator(int camera_index)
    : mVideoSource(camera_index),
      mGLWindow(mVideoSource.getSize(), "Camera Calibrator"),
      mCamera("Camera", mVideoSource.getSize()) {

    mbDone = false;

    // 通过按钮注册的功能名字
    // 需要输入如下对应的指令才能调用 GUICommandCallBack 函数对应的逻辑功能
    ...
    PV3::Register(mpvnDisableDistortion, "CameraCalibrator.NoDistortion", 0,
                  SILENT);

    GUI.ParseLine("GLWindow.AddMenu CalibMenu");
    GUI.ParseLine(
        "CalibMenu.AddMenuButton Live GrabFrame "
        "CameraCalibrator.GrabNextFrame");
    GUI.ParseLine("CalibMenu.AddMenuButton Live Reset CameraCalibrator.Reset");
    GUI.ParseLine(
        "CalibMenu.AddMenuButton Live Optimize "
        "\"CameraCalibrator.Optimize=1\"");
    GUI.ParseLine(
    ...
    GUI.ParseLine(
        "CalibMenu.AddMenuButton Opti Save CameraCalibrator.SaveCalib");
    Reset();

    cout << " Initial camera parameters : " << *mCamera.mpvvCameraParams
         << endl;
    cout << " Default camera parameters : " << ATANCamera::mvDefaultParams
         << endl;
    cout << "Image size as provided by the VideoSource object: "
         << mCamera.GetImageSize().width << " , "
         << mCamera.GetImageSize().height << endl;
}
...
void CameraCalibrator::Run() {
    while (!mbDone) {
        ...
        if (mvCalibImgs.size() < 1)
            *mpvnOptimizing =
                0;  // if no calibration images exist, then set the optimization flag to false;
        ...
        ostringstream ost;
        ost << "Camera Calibration: Grabbed " << mvCalibImgs.size()
            << " images." << endl;
        if (!*mpvnOptimizing) {
            ost << "Take snapshots of the calib grid with the \"GrabFrame\" "
                   "button,"
                << endl;
            ...
            ost << "RMS should go below 0.5, typically below 0.3 for a wide "
                   "lens."
                << endl;
            ost << "Press \"save\" to save calibration to camera.cfg file and "
                   "exit."
                << endl;
        }
...
void CameraCalibrator::Reset() {
    ...
    PV3::get<cv::Vec<float, NUMTRACKERCAMPARAMETERS> >(
        "Camera.Parameters", ATANCamera::mvDefaultParams, SILENT);

    if (*mpvnDisableDistortion)
...
void CameraCalibrator::GUICommandCallBack(void* ptr, string sCommand,
                                          string sParams) {
    ...
    string sCommand, string sParams)  // Called by the callback func..
{
    ...
    if (sCommand == "CameraCalibrator.SaveCalib") {
        cout << "  Camera calib is " << PV3::get_var("Camera.Parameters")
             << endl;
        ...
        if (ofs.good()) {
            ...
            cout << "  Copy-paste above line to settings.cfg or camera.cfg! "
                 << endl;
        }
        mbDone = true;
    }
...
void CameraCalibrator::OptimizeOneStep() {
    ...
    // information vector
    cv::Mat_<double> vJTe = cv::Mat_<double>::zeros(
        nDim, 1);  // a matrix vector... Smells like Least Squares....
    ...
    cv::Matx<double, 2, 1>
        mv2Error;  // temporary storage for v2error as a 2x1 matrix (to accommodate multiplications)
    ...
    for (int n = 0; n < nViews; n++) {
        ...
        if (vEAJ.size() == 0) {
            cout << "All point projections are invalid with current "
                    "parameters. Leaving image out of the optimization..."
                 << endl;
        ...
        for (unsigned int i = 0; i < vEAJ.size(); i++) {
            ...
            for (r_ = 0; r_ < 6; r_++) {
                mJTJ(nMotionBase + r_, nMotionBase + r_) +=
                    EAJ.m26PoseJac(0, r_) * EAJ.m26PoseJac(0, r_) +
                    EAJ.m26PoseJac(1, r_) * EAJ.m26PoseJac(1, r_);
                for (c_ = 0; c_ < r_; c_++)
                    mJTJ(r_ + nMotionBase, c_ + nMotionBase) =
                        (mJTJ(c_ + nMotionBase, r_ + nMotionBase) +=
                         EAJ.m26PoseJac(0, r_) * EAJ.m26PoseJac(0, c_) +
            ...
            for (r_ = 0; r_ < NUMTRACKERCAMPARAMETERS; r_++) {
                ...
                mJTJ(nCamParamBase + r_, nCamParamBase + r_) +=
                    EAJ.m2NCameraJac(0, r_) * EAJ.m2NCameraJac(0, r_) +
                    EAJ.m2NCameraJac(1, r_) * EAJ.m2NCameraJac(1, r_);

                for (c_ = 0; c_ < r_; c_++)
                    mJTJ(r_ + nCamParamBase, c_ + nCamParamBase) =
                        (mJTJ(c_ + nCamParamBase, r_ + nCamParamBase) +=
	  ...
	  tempBlock6xn.copyTo(mJTJBlock6xn);*/
            ...
            for (r_ = 0; r_ < 6; r_++)
                for (c_ = 0; c_ < NUMTRACKERCAMPARAMETERS; c_++)
                    mJTJ(r_ + nMotionBase, c_ + nCamParamBase) =
                        (mJTJ(c_ + nCamParamBase, r_ + nMotionBase) +=
	  ...
	  tempv6.copyTo(vJTe6); */
            for (r_ = 0; r_ < 6; r_++)
                vJTe(r_ + nMotionBase) +=
                    EAJ.m26PoseJac(0, r_) * EAJ.v2Error[0] +
	  ...
	  tempvn.copyTo(vJTen);*/
            for (c_ = 0; c_ < NUMTRACKERCAMPARAMETERS; c_++)
                vJTe(c_ + nCamParamBase, 0) +=
                    EAJ.m2NCameraJac(0, c_) * EAJ.v2Error[0] +
                    EAJ.m2NCameraJac(1, c_) * EAJ.v2Error[1];

            //dSumSquaredError += EAJ.v2Error * EAJ.v2Error;
            dSumSquaredError += EAJ.v2Error[0] * EAJ.v2Error[0] +
                                EAJ.v2Error[1] * EAJ.v2Error[1];
    ...
    if (nTotalMeas == 0) {
        cout << "Did not manage to include a single grid corner in the "
                "optimization ! Skipping updates !"
             << endl;
        return;
    }
    ...
    for (int n = 0; n < nViews; n++) {
        ...
        SE3<> Dse3 = SE3<>::exp(
            cv::Vec<float, 6>(vUpdate(n * 6 + 0, 0), vUpdate(n * 6 + 1, 0),
                              vUpdate(n * 6 + 2, 0), vUpdate(n * 6 + 3, 0),
                              vUpdate(n * 6 + 4, 0), vUpdate(n * 6 + 5, 0)));
        // perturb the camera pose
        mvCalibImgs[n].mse3CamFromWorld =
            Dse3 * mvCalibImgs[n].mse3CamFromWorld;
    }
    // Updating camera parameters
    cv::Vec<float, NUMTRACKERCAMPARAMETERS> Dparams;
    for (int k = 0; k < NUMTRACKERCAMPARAMETERS; k++)
        Dparams[k] = vUpdate(nCamParamBase + k, 0);
