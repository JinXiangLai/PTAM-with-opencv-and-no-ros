...
/**Perform nonmaximal suppression on a set of features, in a 3 by 3 window.
	...
	*/
void nonmax_suppression_strict(const std::vector<cv::Point2i>& corners,
                               const std::vector<int>& scores,
                               std::vector<cv::Point2i>& nmax_corners);
/**Perform nonmaximal suppression on a set of features, in a 3 by 3 window.
	...
	@param max_corners The locally maximal corners.
	*/
void nonmax_suppression(const std::vector<cv::Point2i>& corners,
                        const std::vector<int>& scores,
                        std::vector<cv::Point2i>& nmax_corners);
...
void nonmax_suppression_with_scores(
    const std::vector<cv::Point2i>& corners, const std::vector<int>& scores,
    std::vector<std::pair<cv::Point2i, int> >& max_corners);
