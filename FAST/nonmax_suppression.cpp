...
inline void nonmax_suppression_t(const vector<cv::Point2i>& corners,
    ...
    for (int i = 0; i < sz; i++) {
        ...
        //Check left
        if (i > 0)
            if (corners[i - 1] == cv::Point2i(pos.x - 1, pos.y) &&
...
struct collect_pos {
    static inline cv::Point2i collect(const cv::Point2i& pos, int) {
        return pos;
    }
};
...
struct collect_score {
    ...
    static inline pair<cv::Point2i, int> collect(const cv::Point2i& pos,
                                                 int score) {
...
void nonmax_suppression_strict(const vector<cv::Point2i>& corners,
                               const vector<int>& scores,
                               vector<cv::Point2i>& nonmax_corners) {
    nonmax_suppression_t<int, cv::Point2i, collect_pos, GreaterEqual>(
        corners, scores, nonmax_corners);
}

void nonmax_suppression(const vector<cv::Point2i>& corners,
                        const vector<int>& scores,
                        vector<cv::Point2i>& nonmax_corners) {
    nonmax_suppression_t<int, cv::Point2i, collect_pos, Greater>(
        corners, scores, nonmax_corners);
}

void nonmax_suppression_with_scores(
    const vector<cv::Point2i>& corners, const vector<int>& scores,
    vector<pair<cv::Point2i, int> >& nonmax_corners) {
    nonmax_suppression_t<int, pair<cv::Point2i, int>, collect_score, Greater>(
        corners, scores, nonmax_corners);
}
