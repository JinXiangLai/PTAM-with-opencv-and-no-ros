...
const cv::Point2i fast_pixel_ring[16] = {
    cv::Point2i(0, 3),   cv::Point2i(1, 3),   cv::Point2i(2, 2),
    cv::Point2i(3, 1),   cv::Point2i(3, 0),   cv::Point2i(3, -1),
    cv::Point2i(2, -2),  cv::Point2i(1, -3),  cv::Point2i(0, -3),
    cv::Point2i(-1, -3), cv::Point2i(-2, -2), cv::Point2i(-3, -1),
    cv::Point2i(-3, 0),  cv::Point2i(-3, 1),  cv::Point2i(-2, 2),
    cv::Point2i(-1, 3)};
...
struct CornerNegative {
    inline static int sub(int a, int b) { return b - a; }
};

int old_style_corner_score(const cv::Mat_<uchar>& im, cv::Point2i c,
                           const int* pointer_dir, int barrier) {
...
void compute_fast_score_old(const cv::Mat_<uchar>& im,
                            const vector<cv::Point2i>& corners, int barrier,
                            vector<int>& scores) {
    int pointer_dir[16];
    ...
    for (unsigned int i = 0; i < corners.size(); i++)
        scores[i] =
            old_style_corner_score(im, corners[i], pointer_dir, barrier);
}

void fast_nonmax(const cv::Mat_<uchar>& im, const vector<cv::Point2i>& corners,
                 int barrier, vector<cv::Point2i>& max_corners) {
    vector<int> scores;
    compute_fast_score_old(im, corners, barrier, scores);
    nonmax_suppression(corners, scores, max_corners);
}

void fast_nonmax_with_scores(const cv::Mat_<uchar>& im,
                             const vector<cv::Point2i>& corners, int barrier,
                             vector<pair<cv::Point2i, int> >& max_corners) {
    vector<int> scores;
    compute_fast_score_old(im, corners, barrier, scores);
