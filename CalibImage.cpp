...
inline bool IsCorner(
    cv::Mat_<uchar>& im, int row, int col,
    int nGate) {  // Does a quick check to see if a point in an image could be a grid corner.
    ...
    for (int i = 0; i < 16; i++) {
        abPixels[i] =
            im(row + fast_pixel_ring[i].y, col + fast_pixel_ring[i].x);
        nSum += abPixels[i];
    };
    int nMean = nSum / 16;
...
cv::Vec2f GuessInitialAngles(cv::Mat_<uchar>& im, cv::Point2i irCenter) {
    ...
    for (double dAngle = 0.0; dAngle < M_PI; dAngle += 0.001) {
        ...
        cv::Vec2d v2Dirn(cos(dAngle),
                         sin(dAngle));            // horizontal principal axis
        ...
        for (int k = 0; k < 10; k++) {
            // first criterion
            cv::Vec4d dG4 = imInterp[irCenterv + (k + 1.0) * v2Dirn +
                                     k * sin(M_PI * 10.0 / 180.0) * v2Perp] -
                            imInterp[irCenterv + (k + 1.0) * v2Dirn -
                                     k * sin(M_PI * 10.0 / 180.0) * v2Perp] +
                            imInterp[irCenterv - (k + 1.0) * v2Dirn -
                                     k * sin(M_PI * 10.0 / 180.0) * v2Perp] -
                            imInterp[irCenterv - (k + 1.0) * v2Dirn +
...
bool CalibImage::MakeFromImage(cv::Mat_<uchar>& im, cv::Mat& cim) {
    static Persistence::pvar3<int> gvnCornerPatchSize(
        "CameraCalibrator.CornerPatchPixelSize", 20, Persistence::SILENT);
    mvCorners.clear();
    mvGridCorners.clear();
    ...
    cv::Vec2f baryCenter(
        0,
        0);  // destined to be the barycenter of the detected points (if any that is...)
    {
        cv::Mat_<uchar> imBlurred;

        double dBlurSigma = Persistence::PV3::get<double>(
            "CameraCalibrator.BlurSigma", 2.0, Persistence::SILENT);

        int gkerSize = (int)ceil(
            dBlurSigma *
            3.0);  // where 3.0 is the default "sigmas" parameter in libCVD
        gkerSize += (gkerSize % 2 == 0) ? 1 : 0;

        cv::GaussianBlur(mim, imBlurred, cv::Size(gkerSize, gkerSize),
                         dBlurSigma);
        ...
        int nGate = Persistence::PV3.get<int>(
            "CameraCalibrator.MeanGate", 20,
            Persistence::SILENT);  // 10 is a good value for some cameras,
    ...
    if ((int)mvCorners.size() <
        Persistence::PV3.get<int>("CameraCalibrator.MinCornersForGrabbedImage",
                                  20, Persistence::SILENT))
        return false;

    // normalizing the baryCenter
    baryCenter[0] /= mvCorners.size();
    baryCenter[1] /= mvCorners.size();
    ...
    cFirst.Params =
        Params;  // copy the parameters given to us by "IterateOnImageWithDrawing" in a silver platter
    mvGridCorners.push_back(
        cFirst);  // Oh my! My first GRID corner in the sack!!!! Now going fishing!!!!
    ...
    if (!(ExpandByAngle(0, 0) || ExpandByAngle(0, 2)))
        return false;  // if nothing on the left AND nothing on the right, false
    if (!(ExpandByAngle(0, 1) || ExpandByAngle(0, 3)))
        return false;  // if nothing above AND nothing below, false
    ...
    unsigned int minGridCorners = Persistence::PV3::get<int>(
        "CameraCalibrator.MinimumGridCorners4Pose", 8, Persistence::SILENT);
    cout << " minimum allowable corners per calibration image, "
         << minGridCorners << " and only " << mvGridCorners.size()
         << " found ... " << endl;
    if (mvGridCorners.size() < minGridCorners)
        return false;
...
bool CalibImage::ExpandByAngle(int nSrc, int nDirn) {
    ...
    for (i = 0; i < mvCorners.size(); i++) {
        ...
        cv::Vec2f v2Diff(mvCorners[i].x - gSrc.Params.v2Pos[0],
                         mvCorners[i].y - gSrc.Params.v2Pos[1]);
        ...
        if (v2Diff[0] * v2Diff[0] + v2Diff[1] * v2Diff[1] >
            dBestDist * dBestDist)
            continue;
        ...
        double angularMargin =
            Persistence::PV3::get("CameraCalibrator.CornerSearchAngMargin",
                                  30.0, Persistence::SILENT);
        if (v2Dirn[0] * v2TargetDirn[0] + v2Dirn[1] * v2TargetDirn[1] <
            cos(M_PI * angularMargin / 180.0))
            continue;
    ...
    gTarget.Params.v2Angles =
        cv::Vec2f(gSrc.Params.v2Angles[0], gSrc.Params.v2Angles[1]);
    ...
    gTarget.irGridPos.x = virGridPos[0];
    gTarget.irGridPos.y = virGridPos[1];

    // Update connection states:

    // 1. push the new grid corner in the list
    mvGridCorners.push_back(
        gTarget);  // n.b. This invalidates gSrc! (interesting...)
...
void CalibGridCorner::Draw() {
    ...
    // right vertex
    cv::Vec2d vertex1(Params.v2Pos[0] + Params.m2Warp()(0, 0) * 15 +
                          Params.m2Warp()(0, 1) * 0.0,
                      Params.v2Pos[1] + Params.m2Warp()(1, 0) * 15 +
                          Params.m2Warp()(1, 1) * 0.0);
    // left vertex
    cv::Vec2d vertex2(Params.v2Pos[0] + Params.m2Warp()(0, 0) * (-15) +
                          Params.m2Warp()(0, 1) * 0.0,
                      Params.v2Pos[1] + Params.m2Warp()(1, 0) * (-15) +
                          Params.m2Warp()(1, 1) * 0.0);
    // upper vertex
    cv::Vec2d vertex3(Params.v2Pos[0] + Params.m2Warp()(0, 0) * 0.0 +
                          Params.m2Warp()(0, 1) * 15,
                      Params.v2Pos[1] + Params.m2Warp()(1, 0) * 0.0 +
                          Params.m2Warp()(1, 1) * 15);
    // lower vertex
    cv::Vec2d vertex4(Params.v2Pos[0] + Params.m2Warp()(0, 0) * 0.0 +
                          Params.m2Warp()(0, 1) * (-15),
                      Params.v2Pos[1] + Params.m2Warp()(1, 0) * 0.0 +
                          Params.m2Warp()(1, 1) * (-15));
...
double CalibGridCorner::ExpansionPotential() {
    ...
    assert(
        0 &&
        " Yes, we got t a point that the code is not supposed to get!!!!! Got "
        "FOUR UNTRIED neighboring states!");  // should never get here. Oh yes you get...
...
cv::Mat_<float> CalibGridCorner::GetSteps(vector<CalibGridCorner>& vgc) {
    ...
    for (int dirn = 0; dirn < 2; dirn++) {
        ...
        if (aNeighborStates[dirn].val >= 0) {
            ...
            v2Dirn +=
                vgc[aNeighborStates[dirn].val].Params.v2Pos - Params.v2Pos;
            nFound++;
        }
        ...
        if (aNeighborStates[dirn + 2].val >= 0) {
            v2Dirn -=
                vgc[aNeighborStates[dirn + 2].val].Params.v2Pos - Params.v2Pos;
            nFound++;
        }
...
void CalibImage::ExpandByStep(int n) {
    static Persistence::pvar3<double> gvdMaxStepDistFraction(
        "CameraCalibrator.ExpandByStepMaxDistFrac", 0.4, Persistence::SILENT);
    static Persistence::pvar3<int> gvnCornerPatchSize(
        "CameraCalibrator.CornerPatchPixelSize", 20, Persistence::SILENT);
    ...
    assert((nDirn != -10) &&
           "Couldn't find a direction to expand when we were actually supposed "
           "to (by virtue of the results of 'ExpansionPotential()' ) !!!");
    ...
    double dBestDist = 99999;
    for (unsigned int i = 0; i < mvCorners.size(); i++) {
        cv::Vec2f v2Diff(mvCorners[i].x - v2SearchPos[0],
                         mvCorners[i].y - v2SearchPos[1]);
    ...
    gTarget.irGridPos = cv::Point2i(gSrc.irGridPos.x + irGridStep.x,
                                    gSrc.irGridPos.y + irGridStep.y);
    ...
    for (int dirn = 0; dirn < 4; dirn++) {
        ...
        cv::Point2i irSearch(
            gTarget.irGridPos.x + irFromDirn.x,
            gTarget.irGridPos.y +
                irFromDirn.y);  // not using overload just for peace of mind...
        ...
        for (unsigned int i = 0; i < mvGridCorners.size(); i++)
            if (mvGridCorners[i].irGridPos == irSearch) {
                ...
                mvGridCorners[i].aNeighborStates[(dirn + 2) % 4].val =
                    nTargetNum;  // recall that nTargetNum is the index of the new corner to be
            }
    }
    ...
    for (int i = 0; i < (int)mvGridCorners.size(); i++) {
        for (int dirn = 0; dirn < 4; dirn++)
            if (mvGridCorners[i].aNeighborStates[dirn].val > i) {
                glVertex2f(mvGridCorners[i].Params.v2Pos[0],
                           mvGridCorners[i].Params.v2Pos[1]);

                glVertex2f(
                    mvGridCorners[mvGridCorners[i].aNeighborStates[dirn].val]
                        .Params.v2Pos[0],
                    mvGridCorners[mvGridCorners[i].aNeighborStates[dirn].val]
                        .Params.v2Pos[1]);
            }
...
void CalibImage::Draw3DGrid(ATANCamera& Camera, bool bDrawErrors) {
        ...
        for (int dirn = 0; dirn < 4; dirn++)
            if (mvGridCorners[i].aNeighborStates[dirn].val > i) {
                ...
                cv::Vec3f v3(mvGridCorners[i].irGridPos.x,
                             mvGridCorners[i].irGridPos.y, 0.0);
                // Now, transform these coordinates into the 3D world
                cv::Vec3f cvec =
                    mse3CamFromWorld *
                    v3;  // Hopefully this overload works well (havent yet tested SE3, SO3, SO2, and SE2 for bugs)...
                // Acquiring the 2D Euclidean projection of cvec into cvec_proj
                cv::Vec2f cvec_proj =
                    cv::Vec2f(cvec[0] / cvec[2], cvec[1] / cvec[2]);
                ;
                ...
                v3[1] =
                    mvGridCorners[mvGridCorners[i].aNeighborStates[dirn].val]
                        .irGridPos.y;
                // World location
                cvec = mse3CamFromWorld *
                       v3;  // Hope the overload is done without mistakes...
    ...
    if (bDrawErrors) {
        ...
        for (int i = 0; i < (int)mvGridCorners.size(); i++) {
            ...
            cv::Vec3f cvec =
                mse3CamFromWorld *
                v3;  // Once again, SE3, SO3, SE2, SO2 classes and operators have not been fully tested for bugs.
            ...
            cv::Vec2f v2Error =
                mvGridCorners[i].Params.v2Pos - v2pixBackProjection;
            ...
            glVertex2f(v2pixBackProjection[0] + 10.0 * v2Error[0],
                       v2pixBackProjection[1] + 10.0 * v2Error[1]);
        }
...
void CalibImage::GuessInitialPose(ATANCamera& Camera) {
    ...
    for (int n = 0; n < nPoints; n++) {
        ...
        double x1 =
            mvGridCorners[n]
                .irGridPos
                .x;  // corner x-location in the grid! (assuming unit length in the grid!)
        double y1 =
            mvGridCorners[n].irGridPos.y;  // corner y-locatin in the grid!
        ...
        m9D(5, 7) += -y1 * y2;
        m9D(5, 8) += -y2;
        m9D(6, 6) += x1 * x1 * (x2 * x2 + y2 * y2);
        m9D(6, 7) += x1 * y1 * (x2 * x2 + y2 * y2);
        m9D(6, 8) += x1 * (x2 * x2 + y2 * y2);
        m9D(7, 7) += y1 * y1 * (x2 * x2 + y2 * y2);
        m9D(7, 8) += y1 * (x2 * x2 + y2 * y2);
        m9D(8, 8) += x2 * x2 + y2 * y2;
    }
    ...
    m9D(2, 0) = m9D(5, 3) = m9D(3, 5) = m9D(0, 2);
    m9D(2, 1) = m9D(5, 4) = m9D(4, 5) = m9D(1, 2);
    ...
    m3Homography(0, 0) = Vt(8, 0);
    m3Homography(0, 1) = Vt(8, 1);
    m3Homography(0, 2) = Vt(8, 2);
    m3Homography(1, 0) = Vt(8, 3);
    m3Homography(1, 1) = Vt(8, 4);
    m3Homography(1, 2) = Vt(8, 5);
    m3Homography(2, 0) = Vt(8, 6);
    m3Homography(2, 1) = Vt(8, 7);
    m3Homography(2, 2) = Vt(8, 8);
    ...
    double dot12 = m3Homography(0, 0) * m3Homography(0, 1) +
                   m3Homography(1, 0) * m3Homography(1, 1) +
                   m3Homography(2, 0) * m3Homography(2, 1);
    ...
    double norm2 = sqrt(mRotation(0, 1) * mRotation(0, 1) +
                        mRotation(1, 1) * mRotation(1, 1) +
                        mRotation(2, 1) * mRotation(2, 1));
    mRotation(0, 1) /= norm2;
    mRotation(1, 1) /= norm2;
    mRotation(2, 1) /= norm2;
    ...
    mRotation(0, 2) =
        -mRotation(2, 0) * mRotation(1, 1) + mRotation(1, 0) * mRotation(2, 1);
    mRotation(1, 2) =
        mRotation(2, 0) * mRotation(0, 1) - mRotation(0, 0) * mRotation(2, 1);
    mRotation(2, 2) =
        -mRotation(1, 0) * mRotation(0, 1) + mRotation(0, 0) * mRotation(1, 1);
    ...
    int negative_depth_counter = 0, positive_depth_counter = 0;
    for (int n = 0; n < nPoints; n++) {
...
vector<CalibImage::ErrorAndJacobians> CalibImage::Project(ATANCamera& Camera) {
    ...
    for (unsigned int n = 0; n < mvGridCorners.size(); n++) {
        ...
        cv::Vec3f v3World(mvGridCorners[n].irGridPos.x,
                          mvGridCorners[n].irGridPos.y, 0.0);
        ...
        for (int dof = 0; dof < 6; dof++) {
            ...
            const cv::Vec4f v4Motion =
                SE3<>::generator_field(dof, CvUtils::backproject(v3Cam));

            cv::Vec2f v2CamFrameMotion(
                (v4Motion[0] - X * v4Motion[2] * invZ) * invZ,
                (v4Motion[1] - Y * v4Motion[2] * invZ) * invZ);

            //EAJ.m26PoseJac.T()[dof] = m2CamDerivs * v2CamFrameMotion;
            EAJ.m26PoseJac(0, dof) = m2CamDerivs(0, 0) * v2CamFrameMotion[0] +
                                     m2CamDerivs(0, 1) * v2CamFrameMotion[1];
            EAJ.m26PoseJac(1, dof) = m2CamDerivs(1, 0) * v2CamFrameMotion[0] +
