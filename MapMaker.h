...
struct MapMakerData {
   public:  // think itys public by default...
    ...
    std::set<KeyFrame::Ptr>
        sMeasurementKFs;  // Which keyframes has this map point got measurements in?
    std::set<KeyFrame::Ptr>
        sNeverRetryKFs;  // Which keyframes have measurements failed enough so I should never retry?
...
class MapMaker {
   ...
   public:
    ...
    bool InitFromStereo(
        KeyFrame::Ptr kFirst, KeyFrame::Ptr kSecond,
        std::vector<std::pair<cv::Point2i, cv::Point2i> >& vMatches,
        SE3<>& se3CameraPos);

    bool InitFromStereo_OLD(
        KeyFrame::Ptr kFirst, KeyFrame::Ptr kSecond,  // EXPERIMENTAL HACK
        std::vector<std::pair<cv::Point2i, cv::Point2i> >& vMatches,
        SE3<>& se3CameraPos);

    void AddKeyFrame(
        KeyFrame::Ptr k);  // Add a key-frame to the map. Called by the tracker.
    ...
    bool NeedNewKeyFrame(
        KeyFrame::Ptr
            kCurrent);  // Is it a good camera pose to add another KeyFrame?
    bool IsDistanceToNearestKeyFrameExcessive(
        KeyFrame::Ptr
            kCurrent);  // Is the camera far away from the nearest KeyFrame (i.e. maybe lost?)
   ...
   protected:
    ...
    bool AddPointEpipolar(KeyFrame::Ptr kSrc, KeyFrame::Ptr kTarget, int nLevel,
                          int nCandidate);
    // Returns point in ref frame B
    cv::Vec<float, 3> ReconstructPoint2Views(SE3<> se3AfromB,
                                             const cv::Vec2f& v2A,
                                             const cv::Vec2f& v2B);

    // Bundle adjustment functions:
    void BundleAdjust(std::set<KeyFrame::Ptr>, std::set<KeyFrame::Ptr>,
                      std::set<std::shared_ptr<MapPoint> >, bool);
    ...
    std::vector<KeyFrame::Ptr> NClosestKeyFrames(KeyFrame::Ptr k,
                                                 unsigned int N);
    ...
    std::vector<KeyFrame::Ptr>
        mvpKeyFrameQueue;  // Queue of keyframes from the tracker waiting to be processed
    std::vector<std::pair<KeyFrame::Ptr, std::shared_ptr<MapPoint> > >
        mvFailureQueue;  // Queue of failed observations to re-find
    std::queue<std::shared_ptr<MapPoint> >
        mqNewQueue;  // Queue of newly-made map points to re-find in other KeyFrames

    double
        mdWiggleScale;  // Metric distance between the first two KeyFrames (copied from GVar)
...
bool MapMaker::InitFromStereo(
    KeyFrame::Ptr pkF,  // First KF
    KeyFrame::Ptr pkS,  // Second KF
    vector<pair<cv::Point2i, cv::Point2i> >&
        vTrailMatches,     // image correspondences
    ...
    for (unsigned int i = 0; i < vTrailMatches.size(); i++) {
        ...
        m.v2EucPlaneFirst = mCamera.UnProject(vTrailMatches[i].first.x,
                                              vTrailMatches[i].first.y);
        m.v2EucPlaneSecond = mCamera.UnProject(vTrailMatches[i].second.x,
                                               vTrailMatches[i].second.y);
    ...
    bGood = initializer.Compute(
        vMatches, SLAMInitializer::RANSAC_DEFAULT_THRESHOLD_BOUND, se3);
    if (!bGood) {
    ...
    if (dTransMagn == 0) {
        ...
        cout << "  Estimated zero baseline from stereo pair, try again." << se3
             << endl;
        return false;
    }
    ...
    for (unsigned int i = 0; i < vMatches.size(); i++) {
        ...
        p->v3Center_NEC = CvUtils::backproject(
            mCamera.UnProject(p->irCenter.x, p->irCenter.y));

        // The Euclidean normalized projection of the pixel eactly below the feature.
        p->v3OneDownFromCenter_NEC = CvUtils::backproject(
            mCamera.UnProject(p->irCenter.x, p->irCenter.y + 1.0));

        // The Euclidean normalized projection of the pixel exactly to the right of the feature.
        p->v3OneRightFromCenter_NEC = CvUtils::backproject(
            mCamera.UnProject(p->irCenter.x + 1.0, p->irCenter.y));

        // Now turn all Eudlidean projections into unit (Direction) vectors in 3D:
        p->v3Center_NEC = CvUtils::normalize(p->v3Center_NEC);
        p->v3OneDownFromCenter_NEC =
            CvUtils::normalize(p->v3OneDownFromCenter_NEC);
        p->v3OneRightFromCenter_NEC =
            CvUtils::normalize(p->v3OneRightFromCenter_NEC);
        ...
        finder.SetSubPixPos(
            cv::Vec2f(vTrailMatches[i].second.x, vTrailMatches[i].second.y));
        ...
        p->v3WorldPos = ReconstructPoint2Views(se3, vMatches[i].v2EucPlaneFirst,
                                               mCamera.UnProject(v2SecondPos));
        ...
        mFirst.v2RootPos =
            cv::Vec2f(vTrailMatches[i].first.x, vTrailMatches[i].first.y);
        ...
        mSecond.v2RootPos =
            finder
                .GetSubPixPos();  // recall that the last subpixel iteration was on the second KF
