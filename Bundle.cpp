...
inline void BundleTriangle_UpdateM6U_LL(
    cv::Matx<float, 6, 6>& m6U,   // 6x6 matrix
...
inline void BundleTriangle_UpdateM3V_LL(
    cv::Matx<float, 3, 3>& m3V,   // 3x3 matrix
...
Bundle::Bundle(const ATANCamera& TCam) : mCamera(TCam) {
    ...
    PV3::Register(mgvdUpdateConvergenceLimit,
...
void Bundle::AddMeasurement(int nCam, int nPoint, cv::Vec<float, 2> v2Pos,
...
void Bundle::ClearAccumulators() {
    for (size_t pointIndex = 0; pointIndex < mvPoints.size(); pointIndex++) {
        ...
        mvPoints[pointIndex].v3EpsilonB[0] =
            mvPoints[pointIndex].v3EpsilonB[1] =
                mvPoints[pointIndex].v3EpsilonB[2] = 0;
    }
    for (size_t camIndex = 0; camIndex < mvCameras.size(); camIndex++) {

        mvCameras[camIndex].m6U = cv::Matx<float, 6, 6>::zeros();

        mvCameras[camIndex].v6EpsilonA[0] = mvCameras[camIndex].v6EpsilonA[1] =
            mvCameras[camIndex].v6EpsilonA[2] = 0;
        mvCameras[camIndex].v6EpsilonA[3] = mvCameras[camIndex].v6EpsilonA[4] =
            mvCameras[camIndex].v6EpsilonA[5] = 0;
    }
}
...
inline void Bundle::ProjectAndFindSquaredError(BAMeasurement& measurement) {
    ...
    measurement.v2Epsilon =
        measurement.dSqrtInvNoise * (measurement.v2Found - v2Image);
    measurement.dErrorSquared =
        measurement.v2Epsilon.dot(measurement.v2Epsilon);
}
...
bool Bundle::Do_LM_Step(bool* pbAbortSignal) {
    ...
    for (iMeasurement = mMeasList.begin(); iMeasurement != mMeasList.end();
         iMeasurement++) {

        ProjectAndFindSquaredError(*iMeasurement);

        if (!iMeasurement->bBad)
            vdErrorSquared.push_back(iMeasurement->dErrorSquared);
    }
    ...
    for (iMeasurement = mMeasList.begin(); iMeasurement != mMeasList.end();
        ...
        double dWeight = MEstimator::SquareRootWeight(
            iMeasurement->dErrorSquared, mdSigmaSquared);
        // Re-weight error:
        ...
        dCurrentError += MEstimator::ObjectiveScore(iMeasurement->dErrorSquared,
                                                    mdSigmaSquared);
        ...
        const cv::Vec<float, 4> v4Cam = CvUtils::backproject(
            iMeasurement->v3Cam);  // to get the Lie generators...

        // *****************  Calculate A: (the projection derivativess WRT the camera pose) *********************
        if (cam.bFixed)
            iMeasurement->m26A = cv::Matx<float, 2, 6>::zeros();
        else {

            for (int m = 0; m < 6; m++) {
                // derivative of the point wrt the translation and the axis angle vector
                const cv::Vec<float, 4> v4Motion =
                    SE3<>::generator_field(m, v4Cam);

                cv::Vec<float, 2> v2CamFrameMotion(
                    (v4Motion[0] - v4Cam[0] * v4Motion[2] * invDepth) *
                        invDepth,
                    (v4Motion[1] - v4Cam[1] * v4Motion[2] * invDepth) *
                        invDepth);
                //meas.m26A.T()[m] = meas.dSqrtInvNoise * m2CamDerivs * v2CamFrameMotion;
                iMeasurement->m26A(0, m) =
                    iMeasurement->dSqrtInvNoise *
                    (m2CamDerivs(0, 0) * v2CamFrameMotion[0] +
                     m2CamDerivs(0, 1) * v2CamFrameMotion[1]);
                iMeasurement->m26A(1, m) =
                    iMeasurement->dSqrtInvNoise *
                    (m2CamDerivs(1, 0) * v2CamFrameMotion[0] +
        ...
        for (int m = 0; m < 3; m++) {
            ...
            cv::Matx<float, 3, 1> v3Motion =
                cam.se3CfW.get_rotation().get_matrix().col(m);

            cv::Vec<float, 2> v2CamFrameMotion(
                (v3Motion(0, 0) - v4Cam[0] * v3Motion(2, 0) * invDepth) *
                    invDepth,
                (v3Motion(1, 0) - v4Cam[1] * v3Motion(2, 0) * invDepth) *
                    invDepth);
            //meas.m23B.T()[m] = meas.dSqrtInvNoise * m2CamDerivs * v2CamFrameMotion;
            iMeasurement->m23B(0, m) =
                iMeasurement->dSqrtInvNoise *
                (m2CamDerivs(0, 0) * v2CamFrameMotion[0] +
                 m2CamDerivs(0, 1) * v2CamFrameMotion[1]);
            iMeasurement->m23B(1, m) =
                iMeasurement->dSqrtInvNoise *
                (m2CamDerivs(1, 0) * v2CamFrameMotion[0] +
                 m2CamDerivs(1, 1) * v2CamFrameMotion[1]);
        }
        ...
        if (cam.bFixed)
            iMeasurement->m63W = cv::Matx<float, 6, 3>::zeros();
        else
    ...
    while (dNewError > dCurrentError && !mbConverged && !mbHitMaxIterations &&
        ...
        for (iBAPoint = mvPoints.begin(); iBAPoint != mvPoints.end();
             iBAPoint++) {

            cv::Matx<float, 3, 3> m3VStar = iBAPoint->m3V;
            // augmenting the diagonal now
            m3VStar(0, 0) *= (1.0 + mdLambda);
            m3VStar(1, 1) *= (1.0 + mdLambda);
            m3VStar(2, 2) *= (1.0 + mdLambda);

            // CASE #1 : if V* is NOT Positive Semi-Definite, we zero the inverse.
            float dummy[9];
            if (!CvUtils::M3PSD_LT_Cholesky<>(
                    m3VStar.val,
                    dummy))  // checking if B is a PSD matrix (it should be if
            ...
            //if (m3VStar(0, 0) * m3VStar(1, 1) == 0 )  // This is an alternative (weaker) condition for non PSD matrix but it should work as well here.
            {
                ...
                if (fabs(CvUtils::M3Symm_LT_Det(m3VStar.val)) < 10e-5) {
                    ...
                    iBAPoint->m3VStarInv = cv::Matx<float, 3, 3>::zeros();
                    cout << "Points have vanishing gradients (accumulated in "
                            "V*) !!!"
                         << endl;
                } else  // CASE #3: Use the standard (adjoint) inversion formula for 3x3 symmetric matrices
                {
                    CvUtils::M3Symm_LT_Inverse(
                        m3VStar.val,
                        iBAPoint->m3VStarInv
                            .val);  // standard inversion using LT only
                }
            }
        }
        ...
        cv::Mat_<float> mS =
            cv::Mat_<float>::zeros(mnCamsToUpdate * 6, mnCamsToUpdate * 6);

        cv::Mat_<float> vE = cv::Mat_<float>::zeros(
            mnCamsToUpdate * 6, 1);  // a vector as a mnCamsToUpdate x 1 matrix
        ...
        for (unsigned int cam1Index = 0; cam1Index < mvCameras.size();
             cam1Index++) {
            ...
            for (unsigned int pointIndex = 0; pointIndex < mvPoints.size();
                 pointIndex++) {
                ...
                if (pMeasurement == NULL)
                    continue;
                if (pMeasurement->bBad)
                    continue;

                m6 -= pMeasurement->m63W *
                      (mvPoints[pointIndex].m3VStarInv *
                       pMeasurement->m63W.t());  // SLOW SLOW should by 6x6sy
                v6 -= pMeasurement->m63W * (mvPoints[pointIndex].m3VStarInv *
                                            mvPoints[pointIndex].v3EpsilonB);
            }
            ...
            for (r_ = 0; r_ < 6; r_++) {
                ...
                mS(nCam1StartRow + r_, nCam1StartRow + r_) =
                    m6(r_, r_);  // diagonal of mS

                for (c_ = 0; c_ < r_; c_++)
                    mS(nCam1StartRow + r_, nCam1StartRow + c_) =
                        mS(nCam1StartRow + c_, nCam1StartRow + r_) = m6(r_, c_);
            }
        }
        ...
        for (unsigned int pointIndex = 0; pointIndex < mvPoints.size();
            ...
            for (iOffDiagLink = point.vOffDiagonalScript.begin();
                 iOffDiagLink != point.vOffDiagonalScript.end();
                 iOffDiagLink++) {
                ...
                pMeasurement_Cam2 =
                    mvMeasLUTs[iOffDiagLink->cam2Index][pointIndex];

                if (pMeasurement_Cam2 == NULL)
                    continue;
                if (pMeasurement_Cam2->bBad)
                    continue;

                if (iOffDiagLink->cam1Index != nCurrent1) {
                    // Now get the other camera measuremenmt for the point
                    pMeasurement_Cam1 =
                        mvMeasLUTs[iOffDiagLink->cam1Index][pointIndex];

                    if (pMeasurement_Cam1 == NULL)
                        continue;
                    if (pMeasurement_Cam1->bBad)
                        continue;

                    nCurrent1 = iOffDiagLink->cam1Index;
                    nCam1Row = mvCameras[iOffDiagLink->cam1Index].nStartRow;

                    m63_MIJW_x_m3VStarInv =
                        pMeasurement_Cam1->m63W * point.m3VStarInv;
                }
                ...
                for (_r = 0; _r < 6; _r++)
                    for (_c = 0; _c < 6; _c++)
                        mS(nCam1Row + _r, nCam2Row + _c) -=
                            m63_MIJW_x_m3VStarInv(_r, 0) *
                                pMeasurement_Cam2->m63W(_c, 0) +
                            m63_MIJW_x_m3VStarInv(_r, 1) *
                                pMeasurement_Cam2->m63W(_c, 1) +
                            m63_MIJW_x_m3VStarInv(_r, 2) *
                                pMeasurement_Cam2->m63W(_c, 2);
        ...
        for (unsigned int pointIndex = 0; pointIndex < mvPoints.size();
            ...
            for (unsigned int camIndex = 0; camIndex < mvCameras.size();
                 camIndex++) {
                ...
                int _c;
                for (_c = 0; _c < 3; _c++)
                    v3Sum[_c] += pMeasurement->m63W(0, _c) *
                                     mvCamerasUpdate(cam.nStartRow + 0, 0) +
                                 pMeasurement->m63W(1, _c) *
                                     mvCamerasUpdate(cam.nStartRow + 1, 0) +
                                 pMeasurement->m63W(2, _c) *
                                     mvCamerasUpdate(cam.nStartRow + 2, 0) +
                                 pMeasurement->m63W(3, _c) *
                                     mvCamerasUpdate(cam.nStartRow + 3, 0) +
                                 pMeasurement->m63W(4, _c) *
                                     mvCamerasUpdate(cam.nStartRow + 4, 0) +
                                 pMeasurement->m63W(5, _c) *
                                     mvCamerasUpdate(cam.nStartRow + 5, 0);
            }
            cv::Vec<float, 3> v3 = mvPoints[pointIndex].v3EpsilonB - v3Sum;

            //vMapUpdates.slice(i * 3, 3) = mvPoints[i].m3VStarInv * v3;
            mvMapUpdates(pointIndex * 3 + 0, 0) =
                mvPoints[pointIndex].m3VStarInv(0, 0) * v3[0] +
                mvPoints[pointIndex].m3VStarInv(0, 1) * v3[1] +
                mvPoints[pointIndex].m3VStarInv(0, 2) * v3[2];
            mvMapUpdates(pointIndex * 3 + 1, 0) =
                mvPoints[pointIndex].m3VStarInv(1, 0) * v3[0] +
                mvPoints[pointIndex].m3VStarInv(1, 1) * v3[1] +
                mvPoints[pointIndex].m3VStarInv(1, 2) * v3[2];
            mvMapUpdates(pointIndex * 3 + 2, 0) =
                mvPoints[pointIndex].m3VStarInv(2, 0) * v3[0] +
                mvPoints[pointIndex].m3VStarInv(2, 1) * v3[1] +
                mvPoints[pointIndex].m3VStarInv(2, 2) * v3[2];

            //if(isnan(vMapUpdates.slice(i * 3, 3) * vMapUpdates.slice(i * 3, 3)))
            if (isnan(mvMapUpdates(pointIndex * 3 + 0, 0) *
                          mvMapUpdates(pointIndex * 3 + 0, 0) +
                      mvMapUpdates(pointIndex * 3 + 1, 0) *
                          mvMapUpdates(pointIndex * 3 + 1, 0) +
                      mvMapUpdates(pointIndex * 3 + 2, 0) *
        ...
        double dSumSquaredUpdate = cv::norm(mvCamerasUpdate, cv::NORM_L2SQR) +
                                   cv::norm(mvMapUpdates, cv::NORM_L2SQR);
        ...
        for (unsigned int camIndex = 0; camIndex < mvCameras.size();
            ...
            if (mvCameras[camIndex].bFixed)
                mvCameras[camIndex].se3CfWNew = mvCameras[camIndex].se3CfW;
            else
                //mvCameras[j].se3CfWNew = SE3<>::exp(vCamerasUpdate.slice(mvCameras[j].nStartRow, 6)) * mvCameras[j].se3CfW;
                mvCameras[camIndex].se3CfWNew =
                    SE3<>::exp(cv::Vec<float, 6>(
                        mvCamerasUpdate(mvCameras[camIndex].nStartRow + 0, 0),
        ...
        for (unsigned int pointIndex = 0; pointIndex < mvPoints.size();
            ...
            mvPoints[pointIndex].v3PosNew =
                cv::Vec<float, 3>(mvPoints[pointIndex].v3Pos[0] +
                                      mvMapUpdates(pointIndex * 3 + 0, 0),
                                  mvPoints[pointIndex].v3Pos[1] +
                                      mvMapUpdates(pointIndex * 3 + 1, 0),
                                  mvPoints[pointIndex].v3Pos[2] +
                                      mvMapUpdates(pointIndex * 3 + 2, 0));
        ...
        cout << setprecision(1) << "L" << mdLambda << setprecision(3)
             << "\tOld " << dCurrentError << "  New " << dNewError << "  Diff "
    ...
    for (ilMeas = mMeasList.begin(); ilMeas != mMeasList.end(); ilMeas++)
        if (ilMeas->bBad) {
            ...
            mvOutlierMeasurementIdx.push_back(
                make_pair(ilMeas->pointIndex, ilMeas->camIndex));
            mvPoints[ilMeas->pointIndex].nOutliers++;
            mvMeasLUTs[ilMeas->camIndex][ilMeas->pointIndex] = NULL;
        }

    for (unsigned int i = 0; i < vMeasIterators.size(); i++)
        mMeasList.erase(vMeasIterators[i]);
...
double Bundle::FindNewError() {
    ...
    for (ilMeas = mMeasList.begin(); ilMeas != mMeasList.end(); ilMeas++) {
        ...
        // Project the point.
        cv::Vec<float, 3> v3Cam = mvCameras[ilMeas->camIndex].se3CfWNew *
                                  mvPoints[ilMeas->pointIndex].v3PosNew;
        ...
        cv::Vec<float, 2> v2Error =
            ilMeas->dSqrtInvNoise * (ilMeas->v2Found - v2Image);
        double dErrorSquared =
            v2Error[0] * v2Error[0] + v2Error[1] * v2Error[1];
...
void Bundle::GenerateOffDiagScripts() {
    ...
    for (unsigned int pointIndex = 0; pointIndex < mvPoints.size();
        ...
        //                                               -------------------------
        for (set<int>::iterator iCam1Index = point.sCameras.begin();
             iCam1Index != point.sCameras.end(); iCam1Index++) {
            ...
            if (mvCameras[*iCam1Index].bFixed)
                continue;

            BAMeasurement* pCam1Measurement =
                mvMeasLUTs[*iCam1Index][pointIndex];
            ...
            for (set<int>::iterator iCam2Index = point.sCameras.begin();
                 iCam2Index != iCam1Index; iCam2Index++) {
                ...
                if (mvCameras[*iCam2Index].bFixed)
                    continue;

                BAMeasurement* pCam2Measurement =
                    mvMeasLUTs[*iCam2Index][pointIndex];
