...
class ATANCamera {
   public:
    ...
    ATANCamera(std::string sName,
               const cv::Size2i imgsize = cv::Size2i(DEFAULT_IMG_WIDTH,
                                                     DEFAULT_IMG_HEIGHT));

    // Image size get/set: updates the internal projection params to that target image size.
    inline void SetImageSize(const cv::Size2i& vImageSize) {
    ...
    inline cv::Vec2f Project(const cv::Matx<float, 2, 1>& vNormEuc) {
        ...
        mdLastR = cv::norm(
            mvLastCam);  // This is the undistorted (presumably) radius of the normalized Euclidean coordinates
        mbInvalid =
            (mdLastR >
             mdMaxR);  // We cant have a radius beyond the maximum radius
            // (as estimated from image border back-projections/un-projections in refreshparams()
        mdLastFactor = rtrans_factor(
            mdLastR);  // so rtrans_factor is the DISTORTION factor function
        mdLastDistR =
            mdLastFactor *
            mdLastR;  // Get the distorted radius and chache it for potential use in Jacobian computations
        mvLastDistCam =
            mdLastFactor * mvLastCam;  // Now get the distorted coordinates
    ...
    inline cv::Vec2f UnProject(const cv::Matx<float, 2, 1>& v2Im) {
        ...
        double
            dFactor;  // the undistortion factor it should be ru/rd = mdLastR / mdLastDistR
        if (mdLastDistR >
            0.01)  // if very far from the center (hence distortion is probably heavy)
    ...
    cv::Matx<float, 4, 4> MakeUFBLinearFrustumMatrix(
        float near, float far);  // Returns A 4x4 matrix

    // Feedback for Camera Calibrator
    double PixelAspectRatio() { return mvFocal[1] / mvFocal[0]; }

   protected:
    Persistence::pvar3<cv::Vec<float, NUMTRACKERCAMPARAMETERS> >
        mpvvCameraParams;  // The actual camera parameters

    cv::Matx<float, 2, NUMTRACKERCAMPARAMETERS> GetCamParamAnalyticalDerivs();
    cv::Matx<float, 2, NUMTRACKERCAMPARAMETERS>
    GetCameraParameterDerivs();  // 2x NUMTRACKERCAMPARAMETERS
    ...
    float
        mdOnePixelDist;  // z=1 distance covered by a single pixel offset (a rough estimate!)
    ...
    float
        mdDistortionEnabled;  // One or zero depending on if distortion is on or off.
    ...
    inline float rtrans_factor(float r) {
        if (r < 0.001 || mdW == 0.0)
            ...
            return (mdWinv * atan(r * md2Tan) /
                    r);  // 1/w * atan(2*ru*tan(w/2)) / ru
    };
    ...
    inline float invrtrans(float r) {
        ...
        return (
            tan(r * mdW) *
            mdOneOver2Tan);  // mdOneOver2Tan is a radial distortion coefficient (see the paper by Devernay - Faugeras)
    };

    std::string msName;

    friend class
        CameraCalibrator;  // friend declarations allow access to calibration jacobian and camera update function.
    friend class CalibImage;
};

// Some inline projection functions:
inline cv::Vec2f ATANCamera::UFBLinearProject(
    const cv::Matx<float, 2, 1>& camframe) {
    cv::Vec2f v2Res;
...
inline cv::Vec2f ATANCamera::UFBLinearUnProject(
    const cv::Matx<float, 2, 1>& fbframe) {
    cv::Vec2f v2Res;
