...
inline bool HomographyInit::IsHomographyInlier(
    cv::Matx<float, 3, 3> m3Homography, InitializerMatch match) {

    cv::Vec2f v2Projected = CvUtils::pproject(
        m3Homography * CvUtils::backproject(match.v2EucPlaneFirst));
    ...
    cv::Vec2f v2PixelError =
        cv::Vec<float, 2>(match.imPtSecond.x, match.imPtSecond.y) -
        pCamera->Project(v2Projected);
    double dSquaredError =
        v2PixelError[0] * v2PixelError[0] +
        v2PixelError[1] *
            v2PixelError[1];  // again, avoid overloads and inline...
...
inline double HomographyInit::MLESACScore(cv::Matx<float, 3, 3> m3Homography,
    ...
    cv::Vec2f v2PixelError =
        ...
        pCamera->Project(v2Projected);
    double dSquaredError =
        v2PixelError[0] * v2PixelError[0] + v2PixelError[1] * v2PixelError[1];

    // The "robust" Torr-Zisserman error (i.e. e^2 error^2 =  e^2 < T^2 ? e^2 : T^2;
    return dSquaredError > mdMaxPixelErrorSquared ? mdMaxPixelErrorSquared
                                                  : dSquaredError;
}

bool HomographyInit::Compute(vector<InitializerMatch> vMatches,
    ...
    if (mvDecompositions.size() < 2) {
        cout << "bad decompositions! Size of list : " << mvDecompositions.size()
             << endl;
        return false;
    }
    cout << "Good decomposition! number of decompositions : "
         << mvDecompositions.size() << endl;
...
cv::Matx<float, 3, 3> HomographyInit::HomographyFromMatches(
...
void HomographyInit::RefineHomographyWithInliers() {
    // identity prior
    cv::Matx<float, 9, 9> m9Omega =
        cv::Matx<float, 9, 9>::eye();  // Identity for prior
    cv::Vec<float, 9> v9ksi(
        0, 0, 0, 0, 0, 0, 0, 0,
        0);  // zeros in the information matrix (n.b. we are solving for a perturbation Dh)
    ...
    for (unsigned int i = 0; i < mvHomographyInliers.size(); i++) {
        ...
        // First, find error.
        cv::Vec2f v2m1 =
            mvHomographyInliers[i]
                .v2EucPlaneFirst;  // first measurement in normalized Euclidean plane
        cv::Vec3f v3m2tild_pred =
            mm3BestHomography *
            CvUtils::backproject(
                v2m1);  // The unnormalized predicted Euclidean prediction
        cv::Vec2f v2m2_pred = CvUtils::pproject(
            v3m2tild_pred);  // Normalized Euclidean prediction
        ...
        cv::Vec2f v2p2_pred = pCamera->Project(
            v2m2_pred);  // n.b. this projection will now cache the necessary results for GetProjectionDerivs()...
        // caching the correct projection derivatives now!
        mvHomographyInliers[i].m2PixelProjectionGrad =
            pCamera
                ->GetProjectionDerivs();  // This derivative is useless for the mapMaker henceforth.
        ...
        // store the error
        vdErrorSquared.push_back(v2Error[0] * v2Error[0] +
                                 v2Error[1] * v2Error[1]);
        ...
        vmJacobians.push_back(mvHomographyInliers[i].m2PixelProjectionGrad *
                              m29J);
    }
    ...
    cv::Matx<float, 3, 3>
        m3Update;  // create a 3x3 matrix by which to perturb the homography
    m3Update(0, 0) = v9Update(0, 0);
    m3Update(0, 1) = v9Update(1, 0);
    m3Update(0, 2) = v9Update(2, 0);
    m3Update(1, 0) = v9Update(3, 0);
    m3Update(1, 1) = v9Update(4, 0);
    m3Update(1, 2) = v9Update(5, 0);
    m3Update(2, 0) = v9Update(6, 0);
    m3Update(2, 1) = v9Update(7, 0);
    m3Update(2, 2) = v9Update(8, 0);
...
void HomographyInit::BestHomographyFromMatches_MLESAC() {
    ...
    for (int nR = 0; nR < 300; nR++) {
        ...
        cv::Matx<float, 3, 3> m3Homography =
            HomographyFromMatches(vMinimalMatches);
...
void HomographyInit::DecomposeHomography() {
    ...
    if (d1 - d2 < 10e-6 && d2 - d3 < 10e-6) {
        cout << "Homography is a pure rotation or you somehow transported the "
                "camera  across the room! Either way, reconstruction is not "
                "possible!"
             << endl;
        return;
    }
    ...
    cv::Matx<float, 3, 3> H =
        mm3BestHomography;  // NOT a reference; just a shorter name
    ...
    if (alphasq > 10e-5) {
        ...
        // n = v2 x u1
        decomposition1.v3n =
            cv::Vec<float, 3>(-Vt(1, 2) * u1[1] + Vt(1, 1) * u1[2],
    ...
    if (betasq > 10e-5) {
        ...
        decomposition2.v3n =
            cv::Vec<float, 3>(-Vt(1, 2) * u2[1] + Vt(1, 1) * u2[2],
        ...
        decomposition4.se3SecondFromFirst.get_rotation() =
            SO3<>(decomposition2.se3SecondFromFirst.get_rotation());
        decomposition4.se3SecondFromFirst.get_translation() =
            -decomposition2.se3SecondFromFirst.get_translation();
...
void HomographyInit::ChooseBestDecomposition() {
    ...
    for (unsigned int i = 0; i < mvDecompositions.size(); i++) {
        ...
        for (unsigned int m = 0; m < mvHomographyInliers.size(); m++) {
            ...
            cv::Vec<float, 3> v3 =
                CvUtils::backproject(mvHomographyInliers[m].v2EucPlaneFirst);
            double dVisibilityTest = v3[0] * decomp.v3n[0] +
                                     v3[1] * decomp.v3n[1] +
                                     v3[2] * decomp.v3n[2];
    ...
    // with more-less the same performance.
        ...
        double dErrorSquaredLimit =
            mdMaxPixelErrorSquared *
            4;  // recall, "mdMaxPixelErrorSquared" is the squared cutoff bound of MLESAC...
        ...
        for (int i = 0; i < 2; i++) {
            ...
            cv::Matx<float, 3, 3> m3R = mvDecompositions[i]
                                            .se3SecondFromFirst.get_rotation()
                                            .get_matrix();
            // E = [t]x * R
            for (int j = 0; j < 3; j++) {
            ...
            for (unsigned int m = 0; m < mvMatches.size(); m++) {
                ...
                double d =
                    SampsonusError(mvMatches[m].v2EucPlaneSecond, m3Essential,
                                   mvMatches[m].v2EucPlaneFirst);
